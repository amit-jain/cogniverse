...
Forbidden weak assertions: `assert x is not None`, `assert isinstance(out.summary, str) and out.summary.strip()`, `assert "keyword" in output` without bounds, `assert len(hits) >= 1`, `assert count > 0`. Strong assertions name the exact value, exact set, exact dict shape, exact substring with surrounding context, exact length range, exact sentence count, exact persisted row in the backing store. If the strongest assertion you can write is "non-empty string," the contract isn't defined yet — refine it before coding.

**Test the thing the code exists to do, with real content.** The assertion must exercise the actual behaviour the code was written to provide — fed with real data, checked against the real outcome — not that plumbing moved. Image search: query with image A and assert image A comes back as the top hit. A rename mapping: feed a document and assert the value lands under the renamed schema field. Ranking: assert the ORDER of results. "The HTTP call was made with a tensor in it" is not a test of search.

**"It returned something" is not a test.** Results came back, `count > 0`, no exception raised, a field is present — none of that shows the code is CORRECT. Assert WHICH results, in WHAT order, with WHAT exact values. A search test that still passes when the ranking is inverted, or when the wrong document is returned, has tested nothing. Applies equally to a real-service integration test: hitting real Vespa and then asserting only that some hits came back is the same empty assertion with a slower setup.
