...
class TestResultsFormatter:
    ...
    def print_summary(
        self, results: List[Dict[str, Any]], summary_stats: Dict[str, Any] = None
    ):
    ...
    def format_ranking_strategy_table(
        ...
        if show_details and top_n > 0:
            output.append("\n" + "=" * 120)
            output.append(
                f"TOP {min(top_n, len(sorted_strategies))} STRATEGIES - DETAILED PERFORMANCE"
            )
            output.append("=" * 120)
                        ...
                        detail_data.append(
                            {
                                ...
                                "Found": (
                                    "✅"
                                    if result.get("expected_found", False)
                                    else "❌"
                                ),
    ...
    def format_performance_sorted_table(
        self,
        results: List[Dict[str, Any]],
        sort_by: str = "score",
        ...
        sorted_results = sorted(
            results, key=lambda x: x.get(sort_by, 0), reverse=descending
        )
