...
def create_search_services(config, config_manager, schema_loader):
    ...
    for profile in profiles:
        try:
            services[profile] = SearchService(
                config,
                profile,
...
def test_ranking_strategies(query=None, table_output=False, show_analysis=True):
    ...
    auto_csv_path = (
        output_manager.get_test_results_dir() / f"search_client_test_{timestamp}.csv"
    )
    ...
    # Text-only query
    rec1 = client.recommend_strategy(
        "find documents about machine learning", has_embeddings=False
    )
    ...
    rec3 = client.recommend_strategy(
        "person walking", has_embeddings=True, speed_priority=True
    )
    ...
    for strategy, perf in strategy_performance.items():
        if "error" not in perf:
            ...
            formatted_performance[strategy] = {
                "avg_score": perf[
                    "top_score"
                ],  # Using top score as avg for single query
...
def test_default_ranking(query=None):
    ...
    try:
        ...
        results = client.search(
            {"query": test_query, "top_k": 3, "schema": test_schema}
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Test different Vespa ranking strategies"
    )
    parser.add_argument(
        "--query",
        "-q",
    ...
    parser.add_argument(
        "--no-analysis", action="store_true", help="Skip the detailed analysis output"
    )
