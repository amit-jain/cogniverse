...
def _e2e_cluster_exists() -> bool:
    ...
    return listed.returncode == 0 and E2E_CLUSTER_NAME in listed.stdout
...
def e2e_stack(request, resolved_inference_endpoints):
    ...
    Lifecycle:
      ...
      * FRESH — with no existing cluster, or with explicit ``E2E_FRESH=1``,
        stop any dev cluster (RAM + ports), rebuild images, create the cluster,
        deploy, wait, and stamp the fingerprint. An unhealthy or stale existing
        cluster fails with a recovery command instead of being deleted.
      * TEARDOWN — the cluster is LEFT WARM for the next run. Stop it manually
        with ``k3d cluster stop cogniverse-e2e`` after the focused run. Set
        ``E2E_FRESH=1`` on a later run to replace it before deployment.
    ...
    if not force_fresh and _e2e_cluster_reusable(fingerprint):
        ...
        if _e2e_cluster_exists() and not force_fresh:
            pytest.fail(
                f"existing {E2E_CLUSTER_NAME} cluster is unhealthy or has a stale "
                "deploy fingerprint; inspect it, then rerun with E2E_FRESH=1 "
