...
# from the runtime's own profile_selection_agent log against the Modal-served
# chat model: 2.75s mean and 4.0s max over 19 consecutive calls, for the 100
# examples the tab requests. LLM_TIMEOUT's 300s covers the 275s mean
# projection and no variance, so the wait expired mid-generation and the tab's
...
class TestOptimizationOverview:
    ...
    def test_overview_tab(self, page):
        ...
        # it: a locator matches its whole subtree, and the sibling sub-tab
        # bodies rendered alongside Overview carry metrics of their own, so
        # counting from the top panel found seven.
