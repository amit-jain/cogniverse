...
def _write_fact(mm: Mem0MemoryManager, *, subject: str, content: str) -> str:
    ...
    assert provenance["written_by"] == "agent:xtenant"
    assert provenance["derivation_kind"] == DerivationKind.DIRECT_INGEST.value
    assert provenance["confidence"] == 0.9
    assert provenance["derived_from"] == [
        CitationRef.external("xtenant://src").to_dict()
    ]
