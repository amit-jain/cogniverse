...
class GLiNERLabelOptimizer:
    ...
    def __init__(self):
        ...
        self.test_queries = [
            # Video queries
            (
                "Show me videos about AI",
                {"video": True, "text": False, "temporal": None},
            ),
            (
            ...
            (
                ...
                {"video": False, "text": True, "temporal": None},
            ),
            (
                "Show me the report from last week",
                {"video": False, "text": True, "temporal": "last_week"},
            ),
            (
                "I need papers on neural networks",
                {"video": False, "text": True, "temporal": None},
            ),
            (
            ...
            (
                ...
                {"video": True, "text": True, "temporal": "yesterday"},
            ),
            (
                "Search videos and documents about AI",
                {"video": True, "text": True, "temporal": None},
            ),
            # Temporal queries
            (
            ...
            (
                ...
                {"video": True, "text": True, "temporal": "last_month"},
            ),
        ]
    ...
    async def find_optimal_labels(self, gliner_model) -> dict[str, Any]:
        ...
        best_combination = await self._find_best_combination(
            gliner_model, label_performance
        )

        # Step 3: Fine-tune thresholds
        optimal_config = await self._optimize_threshold(gliner_model, best_combination)
        ...
        for category, labels in self.label_pool.items():
            ...
            for label in labels:
                ...
                if label_scores[label]["attempts"] > 0:
                    score = (
                        label_scores[label]["hits"] / label_scores[label]["attempts"]
                    )
    ...
    async def _test_label_combination(
        ...
        for query, expected in self.test_queries:
            ...
            found_video = any(
                e["label"] in self.label_pool["video_indicators"] for e in entities
            )
            found_text = any(
                e["label"] in self.label_pool["document_indicators"] for e in entities
            )
            found_temporal = any(
                e["label"] in self.label_pool["temporal_indicators"] for e in entities
            )
    ...
    async def _optimize_threshold(
        ...
        for threshold in [0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5]:
            accuracy = await self._test_label_combination(
                gliner_model, labels, threshold
            )
