...
class DSPyGLiNEROptimizer:
    ...
    async def optimize_with_mipro(
        ...
        for model_name in self.gliner_models:
            ...
            try:
                ...
                print(
                    f"   🧠 Running MIPROv2 with {len(train_examples)} training examples..."
                )
                ...
                for i, pred in enumerate(predictions[:3]):
                    ...
                    print(
                        f"         Expected: video={pred['expected']['video']}, "
    ...
    async def run_comprehensive_optimization(self) -> dict[str, Any]:
        ...
        print(
            f"📚 Dataset: {len(train_examples)} train, {len(dev_examples)} dev examples"
        )
    ...
    def print_optimization_summary(self, results: dict[str, Any]):
        ...
        for optimizer_name, optimizer_results in results.items():
            ...
            for model_name, result in optimizer_results.items():
                ...
                print(
                    f"   🤖 {model_name.split('/')[-1]}: {accuracy:.1%} ({time_taken:.1f}s)"
                )
