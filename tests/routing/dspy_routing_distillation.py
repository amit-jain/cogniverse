...
class QueryRoutingSignature(dspy.Signature):
    ...
    # Structured outputs
    needs_video_search = dspy.OutputField(desc="Boolean: true if query requires video/visual content search")
    needs_text_search = dspy.OutputField(desc="Boolean: true if query requires text/document search")
    temporal_pattern = dspy.OutputField(desc="Temporal pattern: 'yesterday', 'last_week', 'last_month', or 'none'")
...
class RoutingDistillation:
    ...
    def distill_to_student(self, training_examples: List[dspy.Example]) -> StudentRouter:
    ...
    def evaluate_student(self, student: StudentRouter, test_queries: List[Tuple[str, Dict]]):
        ...
        for query, expected in test_queries:
            try:
                ...
                # Check accuracy
                routing_correct = (
                    prediction.needs_video_search == expected["video"] and
                    prediction.needs_text_search == expected["text"]
                )
                
                temporal_correct = (
                    prediction.temporal_pattern == expected.get("temporal", "none")
                )
                ...
                print(f"\n🔍 Query: '{query}'")
                print(f"   Predicted: video={prediction.needs_video_search}, "
                      f"text={prediction.needs_text_search}, "
                      f"temporal={prediction.temporal_pattern}")
                print(f"   Expected: video={expected['video']}, "
                      f"text={expected['text']}, "
                      f"temporal={expected.get('temporal', 'none')}")
                print(f"   {'✅ Correct' if routing_correct and temporal_correct else '❌ Wrong'}")
...
async def main():
    ...
    distiller.teacher_lm = DummyLM([
        {"needs_video_search": "true", "needs_text_search": "false", 
         "temporal_pattern": "none", "confidence": "0.9",
    ...
    distiller.student_lm = DummyLM([
        {"needs_video_search": "true", "needs_text_search": "false",
         "temporal_pattern": "none", "confidence": "0.8", 
    ...
    test_queries = [
        ...
        ("Search everything from last week", {"video": True, "text": True, "temporal": "last_week"}),
    ]
