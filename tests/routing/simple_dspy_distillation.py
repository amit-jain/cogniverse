...
class SimpleRoutingSignature(dspy.Signature):
    ...
    routing_decision = dspy.OutputField(desc="Routing decision as: video=true/false, text=true/false, temporal=pattern")
...
class SimpleDistillation:
    ...
    def parse_routing_decision(self, decision_text: str) -> Dict[str, Any]:
        ...
        if any(word in text_lower for word in ["video=true", "video: true", "video search", "visual content", "watch", "play"]):
            result["needs_video_search"] = True
            
        # Check for text indicators  
        if any(word in text_lower for word in ["text=true", "text: true", "document", "paper", "article", "report"]):
    ...
    async def generate_training_data(self) -> List[dspy.Example]:
        ...
        for i, query in enumerate(queries):
            ...
            try:
                ...
                routing_data = self.parse_routing_decision(teacher_output.routing_decision)
                ...
                print(f"   ✅ Video: {routing_data['needs_video_search']}, Text: {routing_data['needs_text_search']}, Temporal: {routing_data['temporal_pattern']}")
...
async def main():
    ...
    for query, exp_video, exp_text, exp_temporal in test_queries:
        try:
            ...
            is_correct = (
                routing_data["needs_video_search"] == exp_video and 
                routing_data["needs_text_search"] == exp_text and
                routing_data["temporal_pattern"] == exp_temporal
            )
            ...
            print(f"   Got: video={routing_data['needs_video_search']}, text={routing_data['needs_text_search']}, temporal={routing_data['temporal_pattern']}")
    ...
    print(f"   Can now handle routing with string parsing!")
