...
from cogniverse_core.common.config_utils import get_config  # noqa: E402
...
class StructuredRoutingSignature(dspy.Signature):
    ...
    needs_video_search = dspy.OutputField(
        desc="true if query needs video search, false otherwise"
    )
    needs_text_search = dspy.OutputField(
        desc="true if query needs document/text search, false otherwise"
    )
    temporal_pattern = dspy.OutputField(
        desc="Temporal pattern: yesterday, today, last_week, last_month, or none"
    )
    ...
    async def generate_training_data(self) -> list[dspy.Example]:
        ...
        for i, query in enumerate(queries):
            ...
            try:
                ...
                print(
                    f"   ✅ Video: {needs_video}, Text: {needs_text}, Temporal: {temporal}"
                )
...
async def main():
    ...
    for query, exp_video, exp_text, exp_temporal in test_queries:
        try:
            ...
            is_correct = (
                pred_video == exp_video
                and pred_text == exp_text
                and result.temporal_pattern == exp_temporal
            )
            ...
            print(
                f"   Got: video={pred_video}, text={pred_text}, temporal={result.temporal_pattern}"
            )
    ...
    print("   Can now handle routing with structured outputs!")
