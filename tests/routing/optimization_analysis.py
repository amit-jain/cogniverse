...
class OptimizationAnalysis:
    ...
    def analyze_performance(self):
        ...
        for approach_name, approach_results in self.results.items():
            if approach_name == "simple":
                ...
                accuracies = [
                    result["accuracy"] for result in approach_results.values()
                ]
                eval_times = [
                    result["eval_time"] for result in approach_results.values()
                ]
                ...
                if isinstance(approach_results, list):
                    ...
                    performance_summary[approach_name] = {
                        ...
                        "configs_tested": sum(
                            result.get("details", {}).get("configurations_tested", 0)
        ...
        print(
            f"{'Approach':<20} {'Best Acc':<10} {'Avg Acc':<10} {'Best Time':<12} {'Configs':<10}"
        )
        print("-" * 70)

        for approach_name, stats in performance_summary.items():
            print(
                f"{approach_name:<20} {stats['best_accuracy']:<10.1%} {stats['avg_accuracy']:<10.1%} "
                f"{stats['best_speed']:<12.2f}s {stats['configs_tested']:<10d}"
            )

        # Find the best approach
        best_approach = max(
            performance_summary.items(), key=lambda x: x[1]["best_accuracy"]
        )
        print(
    ...
    def analyze_configuration_insights(self):
        ...
        if "simple" in self.results:
            ...
            sorted_configs = sorted(
                simple_results.items(), key=lambda x: x[1]["accuracy"], reverse=True
            )
    ...
    def generate_recommendations(self):
        ...
        if "simple" in self.results:
            ...
            fastest_config = min(
                simple_results.items(), key=lambda x: x[1]["eval_time"]
            )
            ...
            print(
                f"   🔄 Speed vs Best: {fastest_result['eval_time'] / best_result['eval_time']:.1f}x faster"
            )
            ...
            for config_name, result in simple_results.items():
                ...
                speed_score = 1 / (
                    result["eval_time"] + 0.1
                )  # Invert and add small constant
                balanced_scores[config_name] = (
                    acc_score * 0.7 + speed_score * 0.3,
                    result,
                )
...
def main():
    ...
    if performance:
        ...
        print(
            f"🏆 Best Overall: {best_approach[0]} ({best_approach[1]['best_accuracy']:.1%})"
        )
        ...
        if efficiency_scores:
            ...
            print(
                f"⚡ Most Efficient: {most_efficient[0]} ({most_efficient[1]:.3f} acc/sec)"
            )
