...
def setup_teacher_student_llms(
    ...
    try:
        ...
        if teacher_model in model_names:
            ...
            print(
                f"⚠️ Teacher model {teacher_model} not found. Using {student_model} for both."
            )
            teacher_found = student_model

        if student_model not in model_names:
            print(
                f"⚠️ Student model {student_model} not found. Available: {model_names[:3]}"
            )
...
class QueryRouterSignature(dspy.Signature):
    ...
    query = dspy.InputField(
        desc="User query to analyze for routing and temporal patterns"
    )
    needs_video_search = dspy.OutputField(
        desc="Answer 'true' or 'false': does the query need video/visual content search?"
    )
    needs_text_search = dspy.OutputField(
        desc="Answer 'true' or 'false': does the query need text/document search?"
    )
    temporal_pattern = dspy.OutputField(
        desc="Extract temporal pattern from query (like 'yesterday', 'last_week') or answer 'null' if none found"
    )
    reasoning = dspy.OutputField(
        desc="Brief explanation of your routing and temporal decisions"
    )
...
def create_high_quality_demonstrations():
    ...
    return [
        ...
        create_query_example(
            "Search for reports on quarterly performance", False, True, None
        ),
        create_query_example(
            "Look up articles about machine learning", False, True, None
        ),
        # Multi-modal examples
        create_query_example(
            "Find videos and documents about training", True, True, None
        ),
...
async def teacher_student_optimization():
    ...
    try:
        ...
        print("🎓 Teacher: qwen3:8b (larger, more capable)")
        print("👨‍🎓 Student: qwen3:1.7b (smaller, to be optimized)")
        ...
        for i, example in enumerate(train_examples):
            try:
                ...
                labeled_example = dspy.Example(
                    ...
                    reasoning=getattr(
                        teacher_prediction, "reasoning", "Teacher prediction"
                    ),
                ).with_inputs("query")
        ...
        print(
            f"   📚 Training student with MIPROv2 using {len(full_trainset)} examples"
        )
        print("   🎓 Using qwen3:8b for prompt optimization")
        print("   👨‍🎓 Training qwen3:1.7b for task execution")
        optimized_student_router = teleprompter.compile(
            student_router, trainset=full_trainset
        )
        ...
        if optimized_details["total"] > 0:
            ...
            if optimized_score > student_baseline_score:
                improvement = (
                    (optimized_score - student_baseline_score)
                    / student_baseline_score
                    * 100
                )
                ...
                if teacher_score > 0:
                    teacher_gap = (
                        (teacher_score - optimized_score) / teacher_score * 100
                    )
                ...
                decline = (
                    (student_baseline_score - optimized_score)
                    / student_baseline_score
                    * 100
                )
                print(f"   📉 Performance declined: -{decline:.1f}%")
                print("   🔄 Keeping baseline (MIPROv2 didn't help)")
...
async def main():
    ...
    if final_score and final_score > 0:
        ...
        print("   • Uses qwen3:8b for intelligent prompt optimization")
        print("   • Optimizes qwen3:1.7b for fast inference")
        print("   • Generates better prompts than BootstrapFewShot")
        print("   • Leverages teacher model knowledge in prompt design")
    else:
