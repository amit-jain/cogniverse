...
class CombinedRoutingTester:
    ...
    def calculate_temporal_match(self, expected: str, actual: str) -> bool:
        ...
        if "_to_" in expected_lower and ("between" in actual_lower or "_and_" in actual_lower):
            ...
            if "_to_" in expected_lower:
                ...
                if start_date.replace("-", "_") in actual_lower and end_date.replace("-", "_") in actual_lower:
                    return True
        ...
        if expected == "2024-01-15":
            date_patterns = ["2024-01-15", "01/15/2024", "15/01/2024", "january_15_2024"]
        ...
        for key, variants in relative_matches.items():
            if expected_lower == key and any(variant in actual_lower for variant in variants):
                return True
        
        # Debug output for unmatched patterns
        print(f"         🔍 TEMPORAL MISMATCH: expected='{expected}' vs actual='{actual}'")
        
        return False

    # =================== LLM TESTING METHODS ===================
    
    async def test_query_with_llm_model(self, query: TestQuery, model_name: str) -> TestResult:
        ...
        try:
            ...
            original_extract_json = test_analyzer.inference_engine.extract_json_from_response
            ...
            def debug_extract_json(response_text):
                ...
                if result:
                    print(f"         ✅ JSON extracted: video={result.get('needs_video_search')}, text={result.get('needs_text_search')}, temporal='{result.get('temporal_pattern')}'")
                else:
                    print(f"         ❌ JSON extraction failed")
                return result
            
            test_analyzer.inference_engine.extract_json_from_response = debug_extract_json
            ...
            print(f"         Inference method: {analysis.get('inference_method', 'unknown')}")
            ...
            if needs_video is None or needs_text is None:
                print(f"         ❌ MISSING ROUTING DATA: video={needs_video}, text={needs_text}")
            ...
            if expected_temporal != "none" and not temporal_info:
                print(f"         ❌ MISSING TEMPORAL DATA: expected '{expected_temporal}' but got none")
                analysis['error'] = f"Expected temporal pattern '{expected_temporal}' but extracted none"
                analysis['llm_success'] = False
            elif not temporal_info:
                print(f"         ⚠️ No temporal info extracted (expected: {expected_temporal})")
            else:
            ...
            test_analyzer.inference_engine.extract_json_from_response = original_extract_json
            print()
            ...
            actual_temporal = self.normalize_temporal_llm(analysis.get("temporal_info", {}))
            
            # Calculate accuracy
            routing_correct = actual_routing == query.expected_routing
            temporal_correct = self.calculate_temporal_match(query.expected_temporal, actual_temporal)
    ...
    async def test_llm_model(self, model_name: str, queries: List[TestQuery]) -> List[TestResult]:
    ...
    def convert_queries_to_gliner_format(self, queries: List[TestQuery]) -> List[Tuple]:
        ...
        for query in queries:
            ...
            gliner_queries.append((query.query, expected_video, expected_text, query.expected_temporal))
        return gliner_queries
    
    async def test_query_with_gliner_model(self, analyzer: QueryAnalyzer, query_tuple: Tuple, model_name: str) -> TestResult:
        ...
        try:
            ...
            print(f"      🔍 GLiNER Detection Results:")
            print(f"         Query: {query}")
            ...
            print(f"         needs_video_search: {result.get('needs_video_search', False)}")
            print(f"         needs_text_search: {result.get('needs_text_search', False)}")
            ...
            temporal_correct = self.calculate_temporal_match(expected_temporal, actual_temporal)
    ...
    async def test_gliner_model(self, model_name: str, query_tuples: List[Tuple]) -> List[TestResult]:
        ...
        for i, query_tuple in enumerate(query_tuples, 1):
            ...
            result = await self.test_query_with_gliner_model(analyzer, query_tuple, model_name)
            results.append(result)
    ...
    def calculate_score(self, results: List[TestResult]) -> ModelScore:
        ...
        overall_accuracy = ((routing_correct + temporal_correct) / (total * 2)) * 100 if total > 0 else 0
    ...
    def save_detailed_results(self, all_results: List[TestResult], filename: str = "combined_test_results.csv"):
        """Save detailed results to CSV."""
        with open(filename, 'w', newline='') as csvfile:
            fieldnames = ['model_type', 'model_name', 'query', 'expected_routing', 'expected_temporal', 
                         'actual_routing', 'actual_temporal', 'routing_correct', 
                         'temporal_correct', 'response_time', 'error']
    ...
    def save_summary_report(self, scores: List[ModelScore], filename: str = "combined_comparison_report.json"):
        ...
        report = {
            ...
            "ranking": {
                "by_overall_accuracy": sorted(scores, key=lambda x: x.overall_accuracy, reverse=True),
                "by_routing_accuracy": sorted(scores, key=lambda x: x.routing_accuracy, reverse=True),
                "by_temporal_accuracy": sorted(scores, key=lambda x: x.temporal_accuracy, reverse=True),
        ...
        for category in report["ranking"]:
            report["ranking"][category] = [asdict(score) for score in report["ranking"][category]]
    ...
    def print_results(self, scores: List[ModelScore]):
        ...
        if gliner_scores:
            ...
            for i, score in enumerate(gliner_scores, 1):
                print(f"{i}. {score.model_name.split('/')[-1]}: {score.overall_accuracy:.1f}%")
        ...
        if scores:
            ...
            print("\n🥇 BEST PERFORMERS")
            print("-" * 40)
            print(f"Overall Accuracy: {best_overall.model_type.upper()}:{best_overall.model_name.split('/')[-1] if best_overall.model_type == 'gliner' else best_overall.model_name} ({best_overall.overall_accuracy:.1f}%)")
            print(f"Routing Accuracy: {best_routing.model_type.upper()}:{best_routing.model_name.split('/')[-1] if best_routing.model_type == 'gliner' else best_routing.model_name} ({best_routing.routing_accuracy:.1f}%)")
            print(f"Temporal Accuracy: {best_temporal.model_type.upper()}:{best_temporal.model_name.split('/')[-1] if best_temporal.model_type == 'gliner' else best_temporal.model_name} ({best_temporal.temporal_accuracy:.1f}%)")
            print(f"Fastest Response: {fastest.model_type.upper()}:{fastest.model_name.split('/')[-1] if fastest.model_type == 'gliner' else fastest.model_name} ({fastest.avg_response_time:.2f}s)")
    
    async def run_combined_test(self, llm_models: List[str] = None, test_gliner: bool = True, queries: List[TestQuery] = None):
        ...
        # Test GLiNER models
        if test_gliner:
            available_gliner = self.config.get("query_inference_engine", {}).get("available_gliner_models", [])
                ...
                for model in available_gliner:
                    try:
                        ...
                        if results:  # Only calculate if we got results
                            ...
                            print(f"✅ {model.split('/')[-1]}: {score.overall_accuracy:.1f}% accuracy")
...
async def main():
    ...
    if len(sys.argv) > 1:
        ...
        if arg == "llm-only":
            ...
            await tester.run_combined_test(llm_models=llm_models_to_test, test_gliner=False)
            ...
            print(f"📝 Using {len(quick_queries)} randomly selected queries for quick test")
            
            # Use all models for quick test including GLiNER
            await tester.run_combined_test(llm_models=llm_models_to_test, test_gliner=True, queries=quick_queries)
        else:
            print("❌ Unknown option. Use: llm-only, gliner-only, quick, or no argument for full test")
            return
    else:
