...
class SimpleGLiNEROptimizer:
    ...
    def __init__(self):
        ...
        self.test_configs = [
            ...
            },
            {
                ...
                "labels": ["video_content", "visual_content", "document_content", "text_information", 
            ...
            },
            {
                "name": "domain_focused",
                "model": "urchade/gliner_medium-v2.1",
                "labels": ["video", "document", "footage", "report", "clip", "article",
            ...
            },
            {
                ...
                "labels": ["video_content", "document_content", "temporal_phrase", "search_intent"],
                "threshold": 0.4
            },
            {
                "name": "entity_rich",
                "model": "urchade/gliner_large-v2.1",
                "labels": ["multimedia_content", "textual_content", "visual_media", "written_document",
    ...
    def load_test_queries(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            queries = [
                {"query": "Show me videos of robots", "needs_video": True, "needs_text": False, "temporal_pattern": None},
                {"query": "Find documents about AI", "needs_video": False, "needs_text": True, "temporal_pattern": None},
                {"query": "Search for content about the project", "needs_video": True, "needs_text": True, "temporal_pattern": None},
                {"query": "Videos from yesterday", "needs_video": True, "needs_text": False, "temporal_pattern": "yesterday"},
                {"query": "Find reports from last week", "needs_video": False, "needs_text": True, "temporal_pattern": "last_week"},
                {"query": "Show me clips about machine learning", "needs_video": True, "needs_text": False, "temporal_pattern": None},
                {"query": "Search documents about neural networks", "needs_video": False, "needs_text": True, "temporal_pattern": None},
                {"query": "Find content from today", "needs_video": True, "needs_text": True, "temporal_pattern": "today"},
                {"query": "Show videos and reports about deep learning", "needs_video": True, "needs_text": True, "temporal_pattern": None},
                {"query": "Search for footage from this morning", "needs_video": True, "needs_text": False, "temporal_pattern": "this_morning"},
            ]
        
        return queries
    
    async def evaluate_configuration(self, config: Dict[str, Any], test_queries: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            for query_data in test_queries:
                try:
                    ...
                    video_correct = result.get("needs_video_search", False) == query_data["needs_video"]
                    text_correct = result.get("needs_text_search", False) == query_data["needs_text"]
                    temporal_correct = result.get("temporal_pattern") == query_data["temporal_pattern"]
    ...
    async def run_optimization(self) -> Dict[str, Any]:
        ...
        for config in self.test_configs:
            ...
            print(f"   📈 Accuracy: {accuracy:.1%} ({result['correct']}/{result['total']})")
    ...
    def print_optimization_summary(self, results: Dict[str, Any]):
        ...
        # Filter out errors
        valid_results = {k: v for k, v in results.items() if "accuracy" in v and "error" not in v}
        ...
        for i, (name, result) in enumerate(sorted_results, 1):
            ...
            print(f"{i:2d}. {name:<15} {result['accuracy']:>6.1%} "
        ...
        if best_result["detailed_results"]:
            ...
            for i, detail in enumerate(best_result["detailed_results"][:3]):
                if "error" not in detail:
                    ...
                    print(f"   Score: {detail['score']:.2f} (V:{detail['video_correct']}, T:{detail['text_correct']}, Temp:{detail['temporal_correct']})")
                    print()
    ...
    try:
        ...
        if os.path.exists(manual_results_file):
            ...
            if simple_best > manual_best:
                print(f"   🏆 Simple optimization wins by {simple_best - manual_best:.1%}")
            else:
                print(f"   🏆 Manual optimization wins by {manual_best - simple_best:.1%}")
