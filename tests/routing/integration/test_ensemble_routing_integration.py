...
class TestEnsembleWithRealModels:
    ...
    async def test_ensemble_voting_weighted_decision(self, ensemble_router):
        ...
        assert video_decision.search_modality in [SearchModality.VIDEO, SearchModality.BOTH]
        
        # Test with a general query
        general_decision = await ensemble_router.route("search for information about machine learning")
        
        assert general_decision.routing_method == "ensemble"
        assert general_decision.search_modality in [SearchModality.TEXT, SearchModality.BOTH, SearchModality.VIDEO]
    ...
    async def test_ensemble_strategy_failure_handling(self, ensemble_router):
        ...
        decision = await ensemble_router.route("extremely complex multi-layered query with unusual terminology")
    ...
    async def test_ensemble_confidence_aggregation(self, ensemble_router):
        ...
        assert len(set(confidence_scores)) > 1 or all(0.3 <= score <= 1.0 for score in confidence_scores)
    ...
    async def test_ensemble_vs_tiered_performance(self, ensemble_test_config):
        ...
        tiered_config.pop("ensemble_config", None)  # Remove ensemble config for tiered mode
...
class TestEnsembleConfigurationValidation:
    ...
    async def test_ensemble_with_all_strategies(self):
        ...
        router = ComprehensiveRouter(config)
        decision = await router.route("find videos about machine learning and AI tutorials")
        ...
        assert len(strategies_used) == 3, f"Expected 3 strategies, got {len(strategies_used)}"
...
class TestEnsembleRealWorldScenarios:
    ...
    async def test_mixed_content_queries(self, ensemble_router):
        ...
        for query in mixed_queries:
            ...
            assert decision.search_modality in [SearchModality.BOTH, SearchModality.VIDEO, SearchModality.TEXT]
...
class TestLLMStrictParsing:
    ...
    async def test_llm_strict_json_validation(self):
        ...
        assert decision.search_modality in [SearchModality.VIDEO, SearchModality.TEXT, SearchModality.BOTH]
        assert decision.generation_type in [GenerationType.RAW_RESULTS, GenerationType.SUMMARY, GenerationType.DETAILED_REPORT]
    ...
    def test_llm_rejects_invalid_responses(self):
        ...
        with pytest.raises(ValueError, match="Missing 'generation_type' in LLM response"):
