...
def skip_if_no_provider(span):
    ...
    if span is None or isinstance(span, NoOpSpan):
        pytest.skip(
            "Telemetry provider not available (cogniverse-telemetry-phoenix not installed)"
        )
...
async def test_trace_connectivity_unified_project():
    ...
    with manager.span(
        ...
        if parent_span and hasattr(parent_span, "context"):
            ...
            logger.info(
                f"Parent span created: trace_id={parent_trace_id}, span_id={parent_span_id}"
            )
        ...
        with manager.span(
            ...
            if child_span and hasattr(child_span, "context"):
                ...
                logger.info(
                    f"Child span created: trace_id={child_trace_id}, span_id={child_span_id}"
                )
...
async def test_cross_operation_trace_connectivity():
    ...
    # STEP 1: Request span
    with manager.span("cogniverse.request", tenant_id=tenant_id) as req_span:
            ...
            span_infos.append(
                {
                    ...
                    "parent_id": (
                        req_span.parent.span_id
                        if hasattr(req_span, "parent") and req_span.parent
                        else None
                    ),
                }
            )
        ...
        with manager.span("cogniverse.routing", tenant_id=tenant_id) as routing_span:
                ...
                span_infos.append(
                    {
                        ...
                        "parent_id": (
                            routing_span.parent.span_id
                            if hasattr(routing_span, "parent") and routing_span.parent
                            else None
                        ),
                    }
                )
            ...
            with manager.span("cogniverse.search", tenant_id=tenant_id) as search_span:
                    ...
                    span_infos.append(
                        {
                            ...
                            "parent_id": (
                                search_span.parent.span_id
                                if hasattr(search_span, "parent") and search_span.parent
                                else None
                            ),
                        }
                    )
    ...
    for info in span_infos:
        logger.info(
            f"   {info['name']}: span_id={info['span_id']}, parent_id={info['parent_id']}"
        )
        ...
        if root_span and hasattr(root_span, "context"):
            span_infos.append(
                {
                    ...
                    "parent_id": (
                        root_span.parent.span_id
                        if hasattr(root_span, "parent") and root_span.parent
                        else None
                    ),
                }
            )
            ...
            with manager.span(
                ...
                if routing_span and hasattr(routing_span, "context"):
                    span_infos.append(
                        {
                            ...
                            "parent_id": (
                                routing_span.parent.span_id
                                if hasattr(routing_span, "parent")
                                and routing_span.parent
                                else None
                            ),
                        }
                    )
                    ...
                    # Grandchild: Search
                    with manager.span(
                        "cogniverse.search", tenant_id=tenant_id
                    ) as search_span:
                        if search_span and hasattr(search_span, "context"):
                            span_infos.append(
                                {
                                    ...
                                    "parent_id": (
                                        search_span.parent.span_id
                                        if hasattr(search_span, "parent")
                                        and search_span.parent
                                        else None
                                    ),
                                }
                            )
