...
class TestPhoenixExperimentPlugin:
    ...
    @pytest.mark.unit
    def test_wrap_inspect_task_for_phoenix_basic(
        self, mock_example, mock_search_service
    ):
        ...
        with (
            ...
            assert result["results"]["profile1_strategy1"]["success"] is True
            assert len(result["results"]["profile1_strategy1"]["results"]) == 1
            assert (
                result["results"]["profile1_strategy1"]["results"][0]["video_id"]
                == "video1"
            )
            assert (
                result["results"]["profile1_strategy1"]["results"][0]["score"] == 0.95
            )
    ...
    @pytest.mark.unit
    def test_wrap_inspect_task_multiple_profiles_strategies(
        self, mock_example, mock_search_service
    ):
    ...
    def test_wrap_inspect_task_search_failure(self, mock_example):
        ...
        with (
            ...
            assert result["results"]["profile1_strategy1"]["success"] is False
    ...
    def test_wrap_inspect_task_video_id_extraction(self, mock_example):
        ...
        with (
            ...
            assert (
                formatted_results[0]["video_id"] == "video1"
...
class TestPhoenixExperimentPluginUtilities:
    ...
    def test_get_phoenix_evaluators_custom_evaluator_name(self):
        ...
        config = {
            ...
            "evaluators": {
                "custom_evaluator": {"provider": "openai", "model": "gpt-4"}
            },
        }
