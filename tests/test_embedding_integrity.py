...
class EmbeddingIntegrityTester:
                ...
                try:
                    ...
                    if patches:
                        index[video_id][frame_id] = {
                            "patches": patches,
                            "num_patches": len(
                                doc["fields"]["colpali_embedding"]["blocks"]
                            ),
                        }
    ...
    def test_cross_video_uniqueness(self, index: Dict) -> bool:
        ...
        if not duplicates_found:
            ...
            test_result["details"].append(
                f"✓ No cross-video duplicates found in {comparisons} comparisons"
            )
        else:
                ...
                for patch_id in patches_to_check:
                    if patch_id in emb1 and patch_id in emb2:
                        ...
                        if np.array_equal(patch1, patch2):
                            ...
                            cosine_sim = np.dot(patch1, patch2) / (
                                np.linalg.norm(patch1) * np.linalg.norm(patch2)
                            )
                            if cosine_sim > 0.95:
    ...
    def test_embedding_structure(self, index: Dict) -> bool:
        ...
        if len(embedding_dims) == 1:
            ...
            test_result["details"].append(
                f"✓ Consistent embedding dimension: {expected_dim}"
            )
        else:
            all_good = False
            test_result["details"].append(
        ...
        if len(unique_patch_counts) == 1:
            test_result["details"].append(
                f"✓ Consistent patch count: {unique_patch_counts[0]}"
            )
        elif len(unique_patch_counts) <= 3:
            test_result["details"].append(
                f"⚠️ Variable patch counts (expected for different image sizes): {unique_patch_counts}"
            )
        else:
            test_result["details"].append(
                f"❌ Too many different patch counts: {len(unique_patch_counts)}"
            )
    ...
    def test_statistical_analysis(self, index: Dict) -> bool:
        ...
        if stats["std"] < 0.01:
            test_result["details"] = (
                "❌ Very low standard deviation - embeddings may be too similar"
            )
            all_good = False
        elif stats["unique_values"] < 100:
            test_result["details"] = (
                "❌ Too few unique values - possible quantization issue"
            )
            all_good = False
        else:
        ...
        print(
            f"  Unique values: {stats['unique_values']:,} / {stats['total_values']:,}"
        )
    ...
    def run_all_tests(
        self, exported_file: Optional[str] = None, num_slices: int = 100
    ) -> bool:
        ...
        # Save results
        report_file = (
            self.output_manager.get_test_results_dir()
            / f"embedding_integrity_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
...
def main():
    ...
    parser.add_argument(
        "--sample-size", type=int, help="Limit number of documents to analyze"
    )
    parser.add_argument(
        "--exported-file",
        type=str,
        help="Path to exported Vespa data file (optional - will export if not provided)",
    )
    parser.add_argument(
        "--num-slices",
        type=int,
        default=100,
        help="Number of slices for data export (default: 100)",
    )
    ...
    success = tester.run_all_tests(
        exported_file=args.exported_file, num_slices=args.num_slices
    )
