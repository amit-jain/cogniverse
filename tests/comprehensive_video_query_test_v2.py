...
def calculate_metrics(
    results: List[Dict], expected_videos: List[str], k_values: List[int] = [1, 3, 5, 10]
) -> Dict[str, float]:
    ...
    for k in k_values:
        if k <= len(result_videos):
            recall_k = (
                len(set(result_videos[:k]) & set(expected_videos))
                / len(expected_videos)
                if expected_videos
                else 0
            )
    ...
    for k in k_values:
        if k <= len(result_videos):
            ...
            relevances = [
                1 if vid in expected_videos else 0 for vid in result_videos[:k]
            ]
            ...
            ideal_relevances = [1] * min(len(expected_videos), k) + [0] * max(
                0, k - len(expected_videos)
            )
    ...
    for strategy, strategy_name in strategies_to_test:
        ...
        for query_data in queries:
            ...
            try:
                ...
                query_result = {
                    ...
                    "metrics": metrics,
                    "top_result_correct": (
                        results[0]["video_id"] in expected_videos if results else False
                    ),
                }
                ...
                if "query(qtb)" in error_msg and len(error_msg) > 500:
                    ...
                    if start_idx != -1 and end_idx != -1 and end_idx - start_idx > 100:
                        embedding_preview = (
                            error_msg[start_idx : start_idx + 50]
                            + "... (truncated) ..."
                            + error_msg[end_idx - 20 : end_idx + 1]
                        )
                        error_msg = (
                            error_msg[:start_idx]
                            + embedding_preview
                ...
                print(
                    f"❌ Search failed for query '{query}' with strategy '{strategy}': {error_msg}"
                )
                query_result = {
                    "query": query,
                    "expected": expected_videos,
        ...
        if all_metrics:
            ...
            for metric_name in all_metric_names:
                ...
                values = [
                    m.get(metric_name, 0) for m in all_metrics if metric_name in m
                ]
                if values:
...
def create_metrics_summary_table(all_results: List[Dict]) -> pd.DataFrame:
    ...
    for profile_results in all_results:
        ...
        row = {
            "Profile": profile,
            "Model": (
                profile_results.get("model", "N/A")[:30] + "..."
                if profile_results.get("model", "")
                and len(profile_results.get("model", "")) > 30
                else profile_results.get("model", "N/A")
            ),
        }
...
def main():
    ...
    if args.output_format == "table":
        ...
        csv_path = (
            output_dir
            / f"comprehensive_v2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        )
    ...
    if args.test_multiple_strategies:
        ...
        for result in all_results:
            if "strategies" in result and len(result["strategies"]) > 1:
                ...
                print(
                    tabulate(
                        strategy_df, headers="keys", tablefmt="grid", showindex=False
                    )
                )
                print()
