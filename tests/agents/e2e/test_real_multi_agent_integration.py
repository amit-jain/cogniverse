...
class TestOllamaAvailability:
    ...
    def test_ollama_model_available(self):
        ...
        try:
            ...
            response = requests.get(f"{TEST_CONFIG['ollama_base_url'].replace('/v1', '')}/api/tags", timeout=5)
            if response.status_code != 200:
                pytest.skip("Ollama server not available - skipping real integration tests")
            ...
            if TEST_CONFIG["ollama_model"] not in model_names:
                pytest.skip(f"Model {TEST_CONFIG['ollama_model']} not available in Ollama - skipping real integration tests")
                
            logger.info(f"✅ Ollama available with model: {TEST_CONFIG['ollama_model']}")
            
        except Exception as e:
            pytest.skip(f"Could not connect to Ollama: {e} - skipping real integration tests")
    ...
    async def test_real_query_analysis_with_local_llm(self):
        ...
        for test_case in test_queries:
            ...
            if "expected_video_search" in test_case:
                assert result["needs_video_search"] == test_case["expected_video_search"]
...
class TestRealAgentRoutingIntegration:
    ...
    async def test_real_agent_routing_with_local_llm(self):
        ...
        for test_case in test_cases:
            ...
            # Verify expected outcomes
            assert test_case["expected_agent"] in routing_decision["primary_agent"].lower()
            assert test_case["expected_workflow"] in routing_decision["recommended_workflow"].lower()
            
            # Verify confidence is reasonable
            confidence = float(routing_decision["routing_confidence"])
            assert 0.0 <= confidence <= 1.0
            assert confidence > 0.5, "Confidence should be reasonably high for clear test cases"
            
            # Verify reasoning is provided
            assert len(routing_decision["reasoning"]) > 10, "Reasoning should be substantive"
...
class TestRealAgentSpecializationIntegration:
    ...
    async def test_real_summarizer_agent_with_local_llm(self):
        ...
        assert len(summary_result["summary"]) < len(test_content), "Summary should be shorter than original"
    ...
    async def test_real_detailed_report_agent_with_local_llm(self):
        ...
        assert len(report_result["executive_summary"]) > 50, "Executive summary should be comprehensive"
        assert len(report_result["detailed_findings"]) > 100, "Detailed findings should be thorough"
        assert len(report_result["recommendations"]) > 30, "Recommendations should be actionable"
...
class TestRealDSPyOptimizationIntegration:
    ...
    @pytest.mark.asyncio
    @pytest.mark.timeout(TEST_CONFIG["test_timeout"] * 2)  # DSPy optimization takes longer
    async def test_real_dspy_optimization_pipeline(self):
        ...
        try:
            query_examples = training_data["query_analysis"][:2]  # Use only 2 examples for speed
    ...
    async def test_real_agent_with_dspy_integration(self):
        ...
        result_default = await analyzer.analyze_query("Show me videos of cats")
...
class TestRealEndToEndWorkflow:
    ...
    @pytest.mark.asyncio
    @pytest.mark.timeout(TEST_CONFIG["test_timeout"] * 2)  # Allow more time for full workflow
    async def test_real_multi_agent_workflow(self):
        ...
        test_query = "Give me a summary of recent developments in artificial intelligence"
...
class TestRealPerformanceComparison:
    ...
    async def test_agent_performance_comparison(self):
        ...
        avg_default_time = sum(p["response_time"] for p in performance_comparison["default"]) / len(test_queries)
        avg_optimized_time = sum(p["response_time"] for p in performance_comparison["optimized"]) / len(test_queries)
        
        avg_default_quality = sum(p["result_quality"] for p in performance_comparison["default"]) / len(test_queries)
        avg_optimized_quality = sum(p["result_quality"] for p in performance_comparison["optimized"]) / len(test_queries)
        
        logger.info(f"Default agent - Avg time: {avg_default_time:.2f}s, Avg quality: {avg_default_quality}")
        logger.info(f"Optimized agent - Avg time: {avg_optimized_time:.2f}s, Avg quality: {avg_optimized_quality}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        pytest.main([f"-v", f"tests/agents/e2e/test_real_multi_agent_integration.py::{test_class}"])
    else:
