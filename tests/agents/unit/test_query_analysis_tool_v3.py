...
from src.app.agents.query_analysis_tool_v3 import (
    ...
    QueryAnalysisResult, create_enhanced_query_analyzer
)
...
class TestQueryAnalysisToolV3:
    ...
    @patch('src.app.agents.query_analysis_tool_v3.get_config')
    def test_analyzer_with_routing_agent(self, mock_get_config, mock_routing_agent, mock_config):
    ...
    @patch('src.app.agents.query_analysis_tool_v3.RoutingAgent')
    @patch('src.app.agents.query_analysis_tool_v3.get_config')
    def test_analyzer_routing_agent_failure(self, mock_get_config, mock_routing_agent, mock_config):
    ...
    def test_clean_query(self, mock_get_config, mock_config):
        ...
        assert analyzer._clean_query("Multiple   spaces   here") == "multiple spaces here"
    ...
    def test_detect_intents_simple(self, mock_get_config, mock_config):
        ...
        primary, secondary = analyzer._detect_intents("find videos about cats", thinking_phase)
        assert primary == QueryIntent.SEARCH
        
        # Test summarize intent
        thinking_phase = {"query_type_indicators": {"summary": True}}
        primary, secondary = analyzer._detect_intents("summarize the content", thinking_phase)
    ...
    def test_detect_intents_complex(self, mock_get_config, mock_config):
        ...
        primary, secondary = analyzer._detect_intents(
            "analyze recent videos and create detailed report", 
            thinking_phase
        )
        
        assert primary == QueryIntent.REPORT
        assert QueryIntent.ANALYZE in secondary
    ...
    def test_detect_modality_requirements_default(self, mock_get_config, mock_config):
        ...
        requirements = analyzer._detect_modality_requirements("generic query", thinking_phase)
    ...
    @patch('src.app.agents.query_analysis_tool_v3.get_config')
    def test_detect_modality_requirements_video_only(self, mock_get_config, mock_config):
        """Test modality detection for video-only queries"""
        mock_get_config.return_value = mock_config
        analyzer = QueryAnalysisToolV3()
        
        thinking_phase = {"modality_hints": {"video": True, "text": False}}
        requirements = analyzer._detect_modality_requirements("show video clips", thinking_phase)
        
        assert requirements["video"] is True
        assert requirements["text"] is False
    
    @patch('src.app.agents.query_analysis_tool_v3.get_config')
    def test_detect_modality_requirements_visual_analysis(self, mock_get_config, mock_config):
        """Test modality detection enabling visual analysis"""
        mock_get_config.return_value = mock_config
        analyzer = QueryAnalysisToolV3()
        
        thinking_phase = {"modality_hints": {"video": True}}
        requirements = analyzer._detect_modality_requirements("analyze visual content", thinking_phase)
    ...
    def test_extract_entities_and_keywords(self, mock_get_config, mock_config):
        ...
        assert "about" not in keywords  # Stop word removed
    ...
    async def test_expand_query_basic(self, mock_get_config, mock_config):
        ...
        expansions = await analyzer._expand_query(
            "find videos about cats", None, {}
        )
        
        # Should include synonym expansions
        assert any("search" in exp for exp in expansions)  # find -> search
        assert any("clip" in exp for exp in expansions)    # videos -> clip
    
    @patch('src.app.agents.query_analysis_tool_v3.get_config')
    @pytest.mark.asyncio
    async def test_expand_query_with_context(self, mock_get_config, mock_config, sample_context):
        """Test query expansion with context"""
        mock_get_config.return_value = mock_config
        analyzer = QueryAnalysisToolV3()
        
        expansions = await analyzer._expand_query(
            "latest research", sample_context, {}
        )
        
        # Should include context-based expansions
        assert any("AI" in exp for exp in expansions)  # From conversation history
    ...
    @patch('src.app.agents.query_analysis_tool_v3.get_config')
    @pytest.mark.asyncio
    async def test_determine_workflow_without_routing_agent(self, mock_get_config, mock_config):
    ...
    @patch('src.app.agents.query_analysis_tool_v3.get_config')
    @pytest.mark.asyncio
    async def test_determine_workflow_with_routing_agent(self, mock_get_config, mock_config, sample_routing_analysis):
    ...
    @patch('src.app.agents.query_analysis_tool_v3.get_config')
    @pytest.mark.asyncio
    async def test_thinking_phase_complex_query(self, mock_get_config, mock_config, sample_context):
    ...
    async def test_analyze_complex_query(self, mock_get_config, mock_config):
        ...
        assert result.complexity_level in [QueryComplexity.MODERATE, QueryComplexity.COMPLEX]
    ...
    async def test_analyze_with_expansion(self, mock_get_config, mock_config):
        ...
        assert any("search" in query for query in result.expanded_queries)  # Synonym expansion
    
    @patch('src.app.agents.query_analysis_tool_v3.get_config')
    @pytest.mark.asyncio
    async def test_analyze_with_context(self, mock_get_config, mock_config, sample_context):
    ...
    async def test_analyze_error_handling(self, mock_get_config, mock_config):
        ...
        with patch.object(analyzer, '_thinking_phase', side_effect=Exception("Test error")):
...
class TestQueryAnalysisToolV3EdgeCases:
    ...
    async def test_analyze_very_long_query(self, mock_get_config, mock_config):
        ...
        long_query = "analyze " + " ".join(["word"] * 50) + " and create detailed report"
        result = await analyzer.analyze(long_query)
        
        assert result.thinking_phase["query_length"] > 10
        assert "Long query suggests complex information need" in result.thinking_phase["reasoning"]
        assert result.complexity_level in [QueryComplexity.MODERATE, QueryComplexity.COMPLEX]
