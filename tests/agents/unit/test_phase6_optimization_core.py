"""
Core functionality tests for Phase 6 optimization components.

Focused tests for the essential functionality without complex mocking.
"""
...
class TestAdvancedRoutingOptimizerCore:
    ...
    def test_config_customization(self):
        ...
        config = AdvancedOptimizerConfig(
            learning_rate=0.01,
            optimizer_strategy="gepa",
            force_optimizer="simba"
        )
    ...
    async def test_record_experience_basic(self):
        ...
        config = AdvancedOptimizerConfig(min_experiences_for_training=100)  # High threshold
    ...
    async def test_get_baseline_recommendations(self):
        ...
        with patch('src.app.routing.advanced_optimizer.SentenceTransformer'):
            ...
            recommendations = optimizer._get_baseline_recommendations(
                query="Find video about machine learning",
                entities=[],
                relationships=[]
            )
...
class TestSIMBAIntegration:
    ...
    def test_simba_config_creation(self):
        ...
        assert config.max_patterns_stored > 0
...
class TestAdaptiveThresholdLearner:
    ...
    def test_threshold_config_creation(self):
        """Test threshold configuration creation."""
        from src.app.routing.adaptive_threshold_learner import ThresholdConfig
        
        config = ThresholdConfig()
        assert config.initial_routing_threshold > 0
        assert config.initial_confidence_threshold > 0
    ...
    def test_threshold_learner_creation(self):
        ...
        assert learner.current_routing_threshold == config.initial_routing_threshold
        assert learner.current_confidence_threshold == config.initial_confidence_threshold
...
class TestPhase6Integration:
    ...
    async def test_end_to_end_workflow_simulation(self):
        ...
        with patch('src.app.routing.advanced_optimizer.SentenceTransformer'):
            ...
            assert status["optimizer_ready"] is True  # Should be ready now
