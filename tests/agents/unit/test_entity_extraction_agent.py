...
class TestEntityExtractionAgentIntegration:
    """Integration tests for EntityExtractionAgent"""
    ...
    async def test_full_extraction_workflow(self, entity_agent):
        ...
        org_entities = [e for e in result.entities if e.type == "ORG"]
        place_entities = [e for e in result.entities if e.type == "PLACE"]
        concept_entities = [e for e in result.entities if e.type == "CONCEPT"]

        assert len(org_entities) == 1
        assert org_entities[0].text == "Apple Inc"
        assert len(place_entities) == 1
        assert place_entities[0].text == "Cupertino"
        assert len(concept_entities) == 1
        assert concept_entities[0].text == "iPhone 15"

        # Verify dominant types
        assert "ORG" in result.dominant_types
        assert "PLACE" in result.dominant_types
        assert "CONCEPT" in result.dominant_types
    ...
    async def test_a2a_task_processing(self, entity_agent):
        ...
        assert a2a_output["status"] == "success"
        assert a2a_output["entity_count"] == 2
        assert all(e["type"] == "PLACE" for e in a2a_output["entities"])
