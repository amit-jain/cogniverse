...
from src.app.agents.a2a_routing_agent import A2ARoutingAgent, AgentEndpoint, RoutingResult
...
class TestA2ARoutingAgent:
    ...
    def test_a2a_routing_agent_initialization(self, mock_get_config, mock_registry_class, mock_routing_agent):
    ...
    def test_extract_query_and_context_with_data_part(self):
        ...
        message = A2AMessage(
            role="user",
            parts=[DataPart(data={"query": "test query", "top_k": 10, "user_id": "test_user"})]
        )
    ...
    def test_extract_query_and_context_with_text_part(self):
        ...
        message = A2AMessage(
            role="user",
            parts=[TextPart(text="find videos about machine learning")]
        )
        task = Task(id="test_task", messages=[message])
    ...
    def test_extract_query_and_context_no_query(self):
        ...
        message = A2AMessage(
            role="user",
            parts=[DataPart(data={"other_field": "value"})]
        )
        task = Task(id="test_task", messages=[message])
    ...
    async def test_send_to_agent_http_error(self, mock_registry_class, mock_get_config):
        ...
        agent.http_client.post = AsyncMock(side_effect=HTTPStatusError(
...
class TestA2ARoutingAgentIntegration:
    ...
    async def test_process_task_success(self, mock_registry_class, mock_get_config):
        ...
        mock_routing_agent.analyze_and_route = AsyncMock(return_value={
        ...
        message = A2AMessage(
            role="user",
            parts=[DataPart(data={"query": "find AI videos", "top_k": 5})]
        )
        task = Task(id="test_task", messages=[message])
    ...
    @patch('src.app.agents.agent_registry.AgentRegistry')
    @pytest.mark.asyncio
    async def test_process_task_routing_failure(self, mock_registry_class, mock_get_config):
        ...
        mock_routing_agent = Mock()
        mock_routing_agent.analyze_and_route = AsyncMock(side_effect=Exception("Routing failed"))
        ...
        message = A2AMessage(
            role="user",
            parts=[DataPart(data={"query": "test query"})]
        )
        task = Task(id="test_task", messages=[message])
