...
class TestVLMInterface:
    ...
    @pytest.mark.asyncio
    async def test_analyze_visual_content(self, mock_predict, mock_dspy_settings, mock_get_config):
...
class TestSummarizerAgent:
    ...
    @pytest.mark.asyncio
    async def test_process_a2a_task_success(self, mock_cot, mock_vlm_class, mock_get_config):
...
class TestSummarizerAgentCoreFunctionality:
    ...
    def agent_with_mocks(self):
        ...
        with patch("src.app.agents.summarizer_agent.get_config") as mock_config, \
            ...
            mock_vlm = Mock()
            mock_vlm.analyze_visual_content = AsyncMock(return_value={
    ...
    @pytest.mark.ci_fast
    @pytest.mark.asyncio
    async def test_thinking_phase_functionality(self, agent_with_mocks, sample_summary_request):
    ...
    @pytest.mark.ci_fast
    def test_extract_themes_functionality(self, agent_with_mocks, sample_summary_request):
    ...
    @pytest.mark.ci_fast
    def test_categorize_content_functionality(self, agent_with_mocks, sample_summary_request):
    ...
    @pytest.mark.ci_fast
    def test_calculate_relevance_scores_functionality(self, agent_with_mocks, sample_summary_request):
    ...
    @pytest.mark.ci_fast
    def test_identify_visual_elements_functionality(self, agent_with_mocks, sample_summary_request):
        """Test visual element identification"""
        agent = agent_with_mocks
        
        visual_elements = agent._identify_visual_elements(sample_summary_request.search_results)
        
        assert isinstance(visual_elements, list)
        # Should identify visual content from video results

    @pytest.mark.ci_fast
    def test_generate_brief_summary_logic(self, agent_with_mocks, sample_summary_request):
        ...
        mock_prediction = Mock()
        mock_prediction.summary = "Brief summary of AI technology content including demos and tutorials."
        ...
        brief_summary = agent._generate_brief_summary(sample_summary_request, results, thinking_phase)
        ...
        assert any(word in brief_summary.lower() for word in ["ai", "technology", "demo", "tutorial", "research", "brief"])
    ...
    def test_extract_key_points_logic(self, agent_with_mocks, sample_summary_request):
        ...
        key_points = agent._extract_key_points(sample_summary_request, thinking_phase, mock_summary)
    ...
    @pytest.mark.ci_fast
    @pytest.mark.asyncio
    async def test_generate_summary_workflow(self, agent_with_mocks, sample_summary_request):
        ...
        with patch('dspy.ChainOfThought') as mock_cot:
            ...
            mock_prediction.key_insights = "AI advancement, Technical education, Research findings"
    ...
    @pytest.mark.ci_fast
    @pytest.mark.asyncio
    async def test_summarize_with_routing_decision_functionality(self, agent_with_mocks):
        ...
        search_results = [{"title": "AI Demo", "content_type": "video", "relevance": 0.9}]
        ...
        with patch('dspy.ChainOfThought') as mock_cot:
            ...
            result = await agent.summarize_with_routing_decision(routing_decision, search_results)
