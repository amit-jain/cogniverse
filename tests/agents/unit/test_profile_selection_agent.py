...
class TestProfileSelectionArtifactLoading:
    ...
    def test_no_telemetry_skips_loading(self, profile_agent):
        ...
        profile_agent._load_artifact()
        assert profile_agent.artifact_load_status == "no_telemetry"

    @pytest.mark.asyncio
    async def test_absent_artifact_records_no_artifact(self, profile_agent):
        ...
        assert profile_agent.artifact_load_status == "no_artifact"
    ...
    async def test_artifact_load_failure_surfaces_error_status(
        ...
        assert profile_agent.artifact_load_status == "error"
        assert (
            "ProfileSelectionAgent artifact load failed; using defaults" in caplog.text
        )
