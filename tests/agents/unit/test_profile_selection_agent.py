...
class TestProfileSelectionAgentIntegration:
    """Integration tests for ProfileSelectionAgent"""
    ...
    @pytest.mark.asyncio
    async def test_modality_detection_accuracy(self, profile_agent):
        """Test modality detection for different query types"""
        test_cases = [
            ("Show me videos", "video", "video_search"),
            ("Find pictures of cats", "image", "image_search"),
            ("Search for documents", "video", "text_search"),  # Defaults to video
        ]
        ...
        for query, expected_modality, expected_intent in test_cases:
            ...
            assert result.modality == expected_modality, f"Failed for query: {query}"
            assert result.query_intent == expected_intent, f"Failed for query: {query}"
    ...
    async def test_a2a_task_processing(self, profile_agent):
        ...
        assert a2a_output["status"] == "success"
        assert a2a_output["selected_profile"] == "video_colpali_large"
