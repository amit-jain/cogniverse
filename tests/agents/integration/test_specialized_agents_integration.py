...
class TestSummarizerAgentOpenAIIntegration:
    ...
    @pytest.mark.asyncio
    async def test_summarizer_with_qwen_comprehensive(self, ollama_config, sample_search_results, mock_ollama_client):
        """Test SummarizerAgent with Qwen model for comprehensive summary"""
        with patch('src.app.agents.summarizer_agent.get_config') as mock_config:
            mock_config.return_value = {
                **ollama_config,
                "summarizer_model": "qwen:7b"
            }
            
            with patch.object(SummarizerAgent, '_create_ollama_client', return_value=mock_ollama_client):
    ...
    @pytest.mark.asyncio
    async def test_summarizer_a2a_with_ollama(self, ollama_config, sample_search_results, mock_ollama_client):
    ...
    @pytest.mark.asyncio
    async def test_summarizer_thinking_phase_with_ollama(self, ollama_config, sample_search_results, mock_ollama_client):
...
class TestDetailedReportAgentOllamaIntegration:
    ...
    @pytest.mark.asyncio
    async def test_detailed_report_with_qwen(self, ollama_config, sample_search_results, mock_ollama_client):
        """Test DetailedReportAgent with Qwen model"""
        with patch('src.app.agents.detailed_report_agent.get_config') as mock_config:
            mock_config.return_value = {
                **ollama_config,
                "report_model": "qwen:7b"
            }
            
            with patch.object(DetailedReportAgent, '_create_ollama_client', return_value=mock_ollama_client):
    ...
    @pytest.mark.asyncio
    async def test_detailed_report_technical_analysis(self, ollama_config, sample_search_results, mock_ollama_client):
        ...
        with patch('src.app.agents.detailed_report_agent.get_config') as mock_config:
            ...
            with patch.object(DetailedReportAgent, '_create_ollama_client', return_value=mock_ollama_client):
                ...
                technical_section = next(
                    (section for section in result.technical_details 
    ...
    @pytest.mark.asyncio
    async def test_detailed_report_a2a_with_ollama(self, ollama_config, sample_search_results, mock_ollama_client):
    ...
    @pytest.mark.asyncio
    async def test_detailed_report_pattern_identification(self, ollama_config, sample_search_results, mock_ollama_client):
        ...
        with patch('src.app.agents.detailed_report_agent.get_config') as mock_config:
            ...
            with patch.object(DetailedReportAgent, '_create_ollama_client', return_value=mock_ollama_client):
                ...
                patterns_finding = next(
                    (finding for finding in result.detailed_findings 
...
class TestCrossAgentIntegration:
    ...
    @pytest.mark.asyncio
    async def test_summarizer_to_detailed_report_workflow(self, ollama_config, sample_search_results, mock_ollama_client):
        ...
        with patch('src.app.agents.summarizer_agent.get_config') as mock_config1, \
            ...
            mock_config1.return_value = {**ollama_config, "summarizer_model": "smollm3:8b"}
            mock_config2.return_value = {**ollama_config, "report_model": "qwen:7b"}
            
            # Initialize agents
            with patch.object(SummarizerAgent, '_create_ollama_client', return_value=mock_ollama_client), \
                 patch.object(DetailedReportAgent, '_create_ollama_client', return_value=mock_ollama_client):
    ...
    @pytest.mark.asyncio
    async def test_model_switching_integration(self, ollama_config, sample_search_results, mock_ollama_client):
...
class TestOllamaConfigurationIntegration:
    ...
    @pytest.mark.asyncio
    async def test_ollama_model_availability_check(self, ollama_config, mock_ollama_client):
        """Test checking Ollama model availability"""
        # Mock model list endpoint
        with patch.object(mock_ollama_client, 'list', new_callable=AsyncMock) as mock_list:
    ...
    async def test_ollama_error_handling(self, ollama_config, mock_ollama_client):
        ...
        with patch.object(mock_ollama_client, 'chat', side_effect=Exception("Connection refused")):
            with patch('src.app.agents.summarizer_agent.get_config') as mock_config:
                mock_config.return_value = ollama_config
