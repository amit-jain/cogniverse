...
class TestPerformanceBenchmarks:
    ...
    def test_component_initialization_times(self):
        ...
        for component_name, component_class in components_to_test:
            ...
            try:
                ...
                end_time = time.time()
                initialization_time = end_time - start_time
                print(f"⚠️  {component_name}: {initialization_time:.3f}s (with error: {e})")
                initialization_times[component_name] = initialization_time
        
        # Performance assertions
        for component_name, init_time in initialization_times.items():
            # Components should initialize within reasonable time
            assert init_time < 30.0, f"{component_name} took too long to initialize: {init_time:.3f}s"
    ...
    def test_routing_agent_performance(self):
        ...
        with patch('src.common.config.get_config') as mock_config:
            ...
            for query in test_queries:
                start_time = time.time()
                try:
                    result = asyncio.run(routing_agent.route_query(query))
                    end_time = time.time()
            ...
            if response_times:
                ...
                print(f"Routing performance - Avg: {avg_response_time:.3f}s, Max: {max_response_time:.3f}s, Min: {min_response_time:.3f}s")
                
                # Performance assertions
                assert avg_response_time < 10.0, f"Average response time too high: {avg_response_time:.3f}s"
                assert max_response_time < 20.0, f"Max response time too high: {max_response_time:.3f}s"
    ...
    def test_relationship_extraction_performance(self):
        ...
        for query in test_queries:
            ...
            try:
                ...
                print(f"Query '{query[:30]}...': {extraction_time:.3f}s ({len(entities)} entities, {len(relationships)} relationships)")
        ...
        if extraction_times:
            ...
            assert avg_extraction_time < 15.0, f"Average extraction time too high: {avg_extraction_time:.3f}s"
    ...
    def test_query_enhancement_performance(self):
        ...
        for test_case in test_cases:
            ...
            try:
                ...
                print(f"Enhancement '{test_case['query'][:30]}...': {enhancement_time:.3f}s")
        ...
        if enhancement_times:
            ...
            # Performance assertion
            assert avg_enhancement_time < 10.0, f"Average enhancement time too high: {avg_enhancement_time:.3f}s"
    ...
    def test_memory_usage_during_operations(self):
        ...
        print(f"Operational memory growth (excluding model loading): {operational_growth:.1f} MB")
        
        # Memory leak detection - operational growth should be minimal
        assert operational_growth < 1000, f"Potential memory leak detected: {operational_growth:.1f} MB growth during operations"
        
        # Total memory usage should be reasonable but we allow for ML models
        assert total_growth < 6000, f"Total memory growth extremely high: {total_growth:.1f} MB - investigate"
        
        # Warn about memory usage patterns
        if operational_growth > 200:
            print(f"⚠️  Potential memory leak: {operational_growth:.1f} MB growth during operations")
        if total_growth > 3000:
            print(f"🔥 High total memory usage: {total_growth:.1f} MB - ML models loaded")
    ...
    def test_concurrent_request_capacity(self):
        ...
        with patch('src.common.config.get_config') as mock_config:
            ...
            try:
                ...
                # Performance assertions
                assert total_time < 30.0, f"Concurrent processing took too long: {total_time:.3f}s"
                assert success_count >= len(queries) * 0.5, f"Too many failures: {success_count}/{len(queries)}"
...
class TestAdaptiveThresholdPerformance:
    ...
    def test_threshold_learning_performance(self):
        ...
        for i in range(num_samples):
            start_time = time.time()
            try:
                asyncio.run(learner.record_performance_sample(
                    ...
                    user_satisfaction=0.7 + (i % 30) / 100.0  # Varying satisfaction
                ))
                end_time = time.time()
        ...
        if sample_times:
            ...
            print(f"Threshold learning performance - Avg: {avg_sample_time:.4f}s, Max: {max_sample_time:.4f}s")
            print(f"Processed {num_samples} samples")
            
            # Performance assertions
            assert avg_sample_time < 0.1, f"Average sample processing too slow: {avg_sample_time:.4f}s"
            assert max_sample_time < 1.0, f"Max sample processing too slow: {max_sample_time:.4f}s"
            
            # Test threshold retrieval performance
            start_time = time.time()
            threshold = learner.get_threshold_value("routing_confidence")
            end_time = time.time()
            retrieval_time = end_time - start_time
            
            print(f"Threshold retrieval time: {retrieval_time:.4f}s")
            assert retrieval_time < 0.01, f"Threshold retrieval too slow: {retrieval_time:.4f}s"
