...
class MockOpenAIClient:
    ...
    def __init__(self, api_key: str, base_url: str = None):
        ...
        self.chat.completions.create = AsyncMock(
            side_effect=self.chat_completions_create
        )
...
class TestSummarizerAgentOpenAIIntegration:
    ...
    async def test_summarizer_with_small_model(
        ...
        with patch("src.app.agents.summarizer_agent.get_config") as mock_config:
            ...
            # Mock the OpenAI import during VLM initialization
            with patch("builtins.__import__") as mock_import:
                # Mock the import of openai to return our mock module
...
class TestDetailedReportAgentOpenAIIntegration:
    ...
    async def test_detailed_report_with_medium_model(
        ...
        with patch("src.app.agents.detailed_report_agent.get_config") as mock_config:
            ...
            with patch(
                "src.app.agents.detailed_report_agent.openai.OpenAI",
                return_value=mock_openai_client,
            ):
...
class TestOpenAIConfigurationIntegration:
    ...
    async def test_openai_client_initialization(
        ...
        response = await mock_openai_client.chat_completions_create(
            "smollm3:8b", messages
        )
    ...
    async def test_openai_error_handling(
        ...
        with patch("src.app.agents.summarizer_agent.get_config") as mock_config:
            ...
            with patch(
                ...
                request = SummaryRequest(
                    query="test error handling", search_results=[], summary_type="brief"
                )
