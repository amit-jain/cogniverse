...
from src.app.agents.routing_agent import RoutingAgent, app, startup_event, routing_agent
...
class TestRoutingAgentIntegration:
    ...
    def test_routing_agent_with_real_config_file(self, mock_get_config, test_config, routing_config_file):
        ...
        assert len(agent.router.strategies) > 0  # Should have at least fallback strategy
    ...
    @patch('src.app.agents.routing_agent.get_config')
    @pytest.mark.asyncio 
    async def test_routing_agent_context_propagation(self, mock_get_config, test_config):
...
# Removed unused fixture
    ...
    def test_health_check_with_initialized_agent(self, test_client):
        ...
        mock_agent = Mock()
        mock_agent.agent_registry = {"video_search": "http://localhost:8002", "text_search": None}
    ...
    def test_process_a2a_task(self, test_client):
        ...
        message = A2AMessage(
            role="user",
            parts=[DataPart(data={"query": "Find machine learning videos", "top_k": 5})]
        )
    ...
    def test_process_a2a_task_with_text_part(self, test_client, mock_initialized_agent):
        ...
        message = A2AMessage(
            parts=[TextPart(text="Summarize recent AI developments")]
        )
