...
from src.app.agents.dspy_agent_optimizer import (
    ...
    DSPyQueryAnalysisOptimizer, DSPyAgentRoutingOptimizer,
    DSPySummaryOptimizer, DSPyDetailedReportOptimizer
)
...
class TestDSPyOptimizerIntegration:
    ...
    @pytest.mark.asyncio
    async def test_pipeline_optimization_with_mocked_teleprompter(self, mock_openai_compatible_api):
        ...
        with patch('dspy.LM') as mock_lm_class:
            with patch('dspy.teleprompt.BootstrapFewShot') as mock_teleprompter:
                ...
                for module_name in ['query_analysis', 'agent_routing', 'summary_generation', 'detailed_report']:
                    mock_module = Mock()
                    mock_module.__class__.__name__ = f"Optimized{module_name.title()}Module"
                    mock_compiled_modules[module_name] = mock_module
                
                mock_teleprompter_instance = Mock()
                mock_teleprompter_instance.compile.side_effect = lambda module, **kwargs: mock_compiled_modules.get(
                ...
                with patch.object(DSPyAgentOptimizerPipeline, 'optimize_module') as mock_optimize:
                    mock_optimize.side_effect = lambda module_name, *args, **kwargs: Mock(name=f"compiled_{module_name}")
    ...
    @pytest.mark.asyncio
    async def test_module_optimization_with_training_data(self, mock_openai_compatible_api):
        ...
        with patch.object(DSPyAgentOptimizerPipeline, 'optimize_module') as mock_optimize:
            ...
            optimized_module = pipeline.optimize_module(
                ...
                query_examples[3:4] if len(query_examples) > 3 else None  # Use 4th as validation
            )
    ...
    def test_prompt_saving_and_loading(self, temp_optimized_prompts_dir):
        ...
        for module_name in ['query_analysis', 'agent_routing', 'summary_generation', 'detailed_report']:
            mock_module = Mock()
            mock_module.generate_analysis = Mock() if 'analysis' in module_name else None
            ...
            if hasattr(mock_module, 'generate_analysis') and mock_module.generate_analysis:
                mock_module.generate_analysis.signature = f"Mock {module_name} signature"
        ...
        for module_name in mock_modules.keys():
            prompt_file = output_dir / f"{module_name}_prompts.json"
...
class TestDSPyAgentIntegration:
    ...
    def test_detailed_report_agent_with_optimized_prompts(self, temp_optimized_prompts_dir):
    ...
    def test_query_analysis_tool_with_optimized_prompts(self, temp_optimized_prompts_dir):
...
class TestDSPyEndToEndOptimization:
    ...
    @pytest.mark.asyncio
    async def test_optimization_to_agent_integration_pipeline(self, temp_optimized_prompts_dir, mock_openai_compatible_api):
            ...
            with patch('dspy.teleprompt.BootstrapFewShot') as mock_teleprompter:
                ...
                def create_mock_module(module_type):
                    ...
                    if module_type == 'query_analysis':
                        ...
                        mock_module.generate_analysis.signature = "Optimized query analysis signature"
                ...
                mock_teleprompter_instance = Mock()
                mock_teleprompter_instance.compile.side_effect = lambda module, **kwargs: compiled_modules.get(
                ...
                with patch.object(Path, 'exists') as mock_exists:
                    ...
                    agents_to_test = [
                        ('query_analysis', QueryAnalysisToolV3, lambda: QueryAnalysisToolV3(enable_agent_integration=False)),
                    ]
    ...
    async def test_performance_comparison_optimized_vs_default(self):
        ...
        with patch('src.app.agents.query_analysis_tool_v3.RoutingAgent'):
            ...
            default_prompt = agent_default.get_optimized_analysis_prompt(test_query, "test context")
            optimized_prompt = agent_optimized.get_optimized_analysis_prompt(test_query, "test context")
            ...
            assert default_metadata['enabled'] == False
            assert optimized_metadata['enabled'] == True
