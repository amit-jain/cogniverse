...
class TestProcessingConfig:
    ...
    def test_processing_config_creation(self):
        ...
        config = ProcessingConfig(
            process_type="frame_based", model_name="test_model", backend="vespa"
        )
...
class TestEmbeddingGenerator:
    ...
    def basic_config(self):
        return {"schema_name": "test_schema", "model_config": {}}
    ...
    @pytest.fixture
    def noop_profile_config(self):
        """Profile config for tests that don't trigger model loading."""
        return {
            "process_type": "frame_based",
            "embedding_model": "test_model",
            "embedding_type": "frame_based",
    ...
    def test_initialization_no_model_load(
        ...
        assert generator.process_type == "frame_based"
        assert generator.model_name == "vidore/colsmol-500m"
    ...
    def test_initialization_with_model_load(
        ...
        assert generator.videoprism_loader == mock_model
        assert generator.model is None

        mock_get_model.assert_called_once_with(
            "custom_model", profile_config, mock_logger
        )
    ...
    def test_should_load_model_conditions(
        ...
        load_cases = [
            ("direct_video_global", "direct_video_segment"),
            ("direct_video_local", "direct_video_segment"),
            ("video_chunks", "video_chunks"),
        ]

        for process_type, embedding_type in load_cases:
            profile_config = {
        ...
        no_load_cases = ["frame_based", "single_vector", "other_type"]

        for process_type in no_load_cases:
            profile_config = {
                "process_type": process_type,
                "embedding_model": "test_model",
                "embedding_type": "frame_based",
    ...
    def test_generate_embeddings_unknown_processing_type(
        ...
        generator = EmbeddingGenerator(
            basic_config, mock_logger, {"process_type": "frame_based", "embedding_model": "test_model", "embedding_type": "frame_based"}, mock_backend_client
        )
    ...
    def test_generate_embeddings_processing_method_dispatch(
        ...
        with (
            patch.object(
                generator,
                "_generate_frame_based_embeddings",
                return_value=expected_result,
            ) as mock_frame,
            patch.object(
                generator,
                "_generate_single_vector_embeddings",
                return_value=expected_result,
            ) as mock_single,
            patch.object(
                generator,
                "_generate_video_chunks_embeddings",
                return_value=expected_result,
            ) as mock_chunks,
            patch.object(
                generator,
                "_generate_direct_video_embeddings",
                return_value=expected_result,
            ) as mock_direct,
        ):
    ...
    def test_generate_embeddings_uses_fallback_processing_type(
        ...
        with patch.object(
            generator, "_generate_frame_based_embeddings", return_value=expected_result
        ) as mock_frame:
    ...
    def test_generate_embeddings_logs_info(
        ...
        with patch.object(
            ...
            mock_logger.info.assert_any_call(
                "Starting embedding generation for video: test_video"
            )
    ...
    def test_get_video_path_search_video_dir(
        ...
        generator = EmbeddingGenerator(
            config_with_dir, mock_logger, {"process_type": "frame_based", "embedding_model": "test_model", "embedding_type": "frame_based"}, mock_backend_client
        )
    ...
    def test_process_video_segment_videoprism(
        ...
        generator.embedding_processor.process_videoprism_segment.return_value = (
            embeddings_result
        )
    ...
    def test_direct_video_embeddings_success(
        ...
        with (
            patch.object(generator, "_get_video_path", return_value=video_path),
