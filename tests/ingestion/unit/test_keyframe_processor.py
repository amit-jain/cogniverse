...
class TestKeyframeProcessor:
    ...
    def test_processor_initialization(self, mock_logger):
        ...
        processor = KeyframeProcessor(
            mock_logger, threshold=0.95, max_frames=100, fps=2.0
        )
    ...
    def test_from_config_factory_method(self, mock_logger):
        ...
        config = {"threshold": 0.85, "max_frames": 50, "fps": 0.5}
    ...
    def test_extract_keyframes_fps_mode(
        self, mock_cv2, fps_processor, temp_dir, sample_video_path
    ):
    ...
    @patch("cv2.VideoCapture")
    def test_extract_keyframes_histogram_mode(
        self, mock_cv2, processor, temp_dir, sample_video_path
    ):
        ...
        with (
            ...
            def mock_compare(h1, h2, method):
                ...
                return (
                    0.5 if call_count % 30 == 0 else 0.95
                )  # Every 30th frame is different
