...
class TestChunkProcessor:
    ...
    def no_cache_processor(self, mock_logger):
        ...
        return ChunkProcessor(
            mock_logger, chunk_duration=15.0, chunk_overlap=1.0, cache_chunks=False
        )
    ...
    def test_extract_chunks_success(
        self, mock_extractor_class, processor, temp_dir, sample_video_path
    ):
    ...
    def test_extract_chunks_with_caching(
        ...
        cached_result = {
            "video_id": "test_video",
            "chunks": [
                {"start_time": 0.0, "end_time": 30.0, "filename": "cached_chunk.mp4"}
            ],
            "total_chunks": 1,
        }

        with patch.object(
            processor, "_get_cache_manager", return_value=mock_cache_manager
        ):
    ...
    @patch("src.app.ingestion.processors.video_chunk_extractor.VideoChunkExtractor")
    def test_extract_chunks_no_caching(
        self, mock_extractor_class, no_cache_processor, temp_dir, sample_video_path
    ):
        ...
        with patch(
            ...
            result = no_cache_processor.extract_chunks(
        ...
        assert (
            not hasattr(no_cache_processor, "_get_cache_manager")
            or no_cache_processor._get_cache_manager is None
        )
    ...
    @patch("src.app.ingestion.processors.video_chunk_extractor.VideoChunkExtractor")
    def test_extract_chunks_extractor_error(
        self, mock_extractor_class, processor, temp_dir, sample_video_path
    ):
    ...
    def test_get_config(self, processor):
        ...
        expected = {"chunk_duration": 30.0, "chunk_overlap": 2.0, "cache_chunks": True}
