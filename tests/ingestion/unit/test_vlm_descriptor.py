...
class TestVLMDescriptor:
    ...
    def sample_keyframes_metadata(self, tmp_path):
        ...
        return {
            ...
            "keyframes": [
                {
                    "frame_id": "frame_001",
                    "path": str(frame1_path),
                    "timestamp": 10.5
                },
                {
                    "frame_id": "frame_002", 
                    "path": str(frame2_path),
                    "timestamp": 20.0
                }
            ]
        }
    ...
    @patch('requests.get')
    @patch('time.sleep')
    def test_ensure_service_running_start_success(self, mock_sleep, mock_get, mock_exists, mock_run, vlm_descriptor):
    ...
    @patch('pathlib.Path.exists')
    @patch('requests.get')
    def test_ensure_service_running_start_failure(self, mock_get, mock_exists, mock_run, vlm_descriptor):
    ...
    @patch('pathlib.Path.exists')
    @patch('requests.get')
    def test_ensure_service_running_no_service_file(self, mock_get, mock_exists, vlm_descriptor):
    ...
    def test_generate_descriptions_empty_keyframes(self, vlm_descriptor, empty_keyframes_metadata):
    ...
    @patch('json.dump')
    @patch('time.time')
    def test_generate_descriptions_success(self, mock_time, mock_json_dump, mock_file, 
                                         mock_mkdir, mock_get_output_manager, 
                                         vlm_descriptor, sample_keyframes_metadata):
        """Test successful description generation."""
        # Mock output manager
        mock_output_manager = Mock()
        mock_processing_dir = Mock()
        mock_processing_dir.__truediv__ = Mock(return_value=Path("/test/descriptions/test_video_123.json"))
    ...
    @patch('json.dump')
    @patch('time.time')
    def test_generate_descriptions_with_output_dir(self, mock_time, mock_json_dump, 
                                                  mock_file, mock_mkdir, vlm_descriptor, 
                                                  sample_keyframes_metadata, tmp_path):
        ...
        with patch.object(vlm_descriptor, '_process_vlm_batch') as mock_batch:
            ...
            result = vlm_descriptor.generate_descriptions(sample_keyframes_metadata, output_dir)
    ...
    @patch('json.dump')
    @patch('time.time')
    def test_generate_descriptions_auto_start(self, mock_time, mock_json_dump, mock_file,
                                            mock_mkdir, mock_get_output_manager,
                                            vlm_descriptor_auto_start, sample_keyframes_metadata):
        ...
        with patch.object(vlm_descriptor_auto_start, '_ensure_service_running') as mock_ensure:
            with patch.object(vlm_descriptor_auto_start, '_process_vlm_batch') as mock_batch:
                mock_batch.return_value = {"frame_001": "Auto-started description"}
                
                vlm_descriptor_auto_start.generate_descriptions(sample_keyframes_metadata)
    ...
    @patch('requests.post')
    @patch('os.unlink')
    def test_process_vlm_batch_success(self, mock_unlink, mock_post, mock_zipfile, 
                                      mock_tempfile, vlm_descriptor, tmp_path):
    ...
    @patch('requests.post')
    @patch('os.unlink')
    def test_process_vlm_batch_api_error(self, mock_unlink, mock_post, mock_zipfile,
                                        mock_tempfile, vlm_descriptor, tmp_path):
    ...
    @patch('requests.post')
    @patch('os.unlink')
    def test_process_vlm_batch_connection_error(self, mock_unlink, mock_post, mock_zipfile,
                                               mock_tempfile, vlm_descriptor, tmp_path):
    ...
    @patch('requests.post')
    @patch('os.unlink')
    def test_process_vlm_batch_connection_error_auto_start_retry(self, mock_unlink, mock_post, 
                                                               mock_zipfile, mock_tempfile, 
                                                               vlm_descriptor_auto_start, tmp_path):
        ...
        mock_response_success.json.return_value = {"descriptions": {"frame_001": "Retry success"}}
    ...
    @patch('requests.post')
    def test_process_single_frame_connection_error(self, mock_post, vlm_descriptor, tmp_path):
    ...
    @patch('json.dump')
    @patch('time.time')
    def test_generate_descriptions_large_batch(self, mock_time, mock_json_dump, mock_file,
                                             mock_mkdir, mock_get_output_manager,
                                             vlm_descriptor, tmp_path):
        """Test description generation with multiple batches."""
        # Mock output manager
        mock_output_manager = Mock()
        mock_processing_dir = Mock()
        mock_processing_dir.__truediv__ = Mock(return_value=Path("/test/descriptions/large_video.json"))
        mock_output_manager.get_processing_dir.return_value = mock_processing_dir
        mock_get_output_manager.return_value = mock_output_manager
        ...
        with patch.object(vlm_descriptor, '_process_vlm_batch', side_effect=mock_batch_side_effect):
    ...
    def test_batch_endpoint_url_transformation(self, vlm_descriptor):
        ...
        batch_endpoint = original_endpoint.replace("generate-description", "upload-and-process-frames")
