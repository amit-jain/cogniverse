...
class TestEndToEndVideoProcessing:
    ...
    def test_complete_frame_based_pipeline(self, mock_embedding_class, mock_audio_class, 
                                         mock_keyframe_class, mock_logger, frame_based_strategy_set,
        ...
        with patch('src.app.ingestion.processor_manager.pkgutil.iter_modules') as mock_iter:
            ...
            keyframe_result = keyframe_processor.extract_keyframes(sample_video_path, temp_dir)
            ...
            mock_keyframe.extract_keyframes.assert_called_once_with(sample_video_path, temp_dir)
    ...
    @patch('src.app.ingestion.processors.audio_processor.AudioProcessor')
    def test_complete_chunk_based_pipeline(self, mock_audio_class, mock_chunk_class, 
                                         mock_logger, chunk_based_strategy_set,
                                         temp_dir, sample_video_path):
        ...
        with patch('src.app.ingestion.processor_manager.pkgutil.iter_modules') as mock_iter:
            ...
            # Process video
            chunk_result = chunk_processor.extract_chunks(sample_video_path, video_duration=20.0)
            audio_result = audio_processor.transcribe_audio(sample_video_path)
    ...
    def test_pipeline_error_propagation(self, mock_logger, frame_based_strategy_set, 
                                      temp_dir, sample_video_path):
    ...
    def test_pipeline_partial_failure_handling(self, mock_logger, frame_based_strategy_set,
                                              temp_dir, sample_video_path):
    ...
    @patch('src.app.ingestion.processors.keyframe_processor.KeyframeProcessor')
    @patch('src.app.ingestion.processors.audio_processor.AudioProcessor') 
    def test_pipeline_data_flow_validation(self, mock_audio_class, mock_keyframe_class, 
                                         mock_logger, frame_based_strategy_set,
                                         temp_dir, sample_video_path):
        ...
        with patch('src.app.ingestion.processor_manager.pkgutil.iter_modules') as mock_iter:
            ...
            keyframes = keyframe_processor.extract_keyframes(sample_video_path, temp_dir)
    ...
    def test_pipeline_concurrent_processing_safety(self, mock_logger, frame_based_strategy_set,
                                                  temp_dir, sample_video_path):
        ...
        class ThreadSafeProcessor:
            ...
            def extract_keyframes(self, *args, **kwargs):
                with self._lock:
                    ...
                    return {"video_id": f"video_{self.call_count}", "total_keyframes": 1}
