"""Unit coverage for the golden-set ground-truth admin upload route."""
...
async def test_upload_persists_canonical_blob_and_activates_version(monkeypatch):
    ...
    assert stub.activate_calls == [("config", "golden_set_ground_truth", 1)]
...
async def test_upload_rejects_blank_query_with_400(monkeypatch):
    ...
    try:
        response = await _put(
            app,
            "/admin/tenants/acme/golden_set_ground_truth",
    ...
    assert response.json() == {
        "detail": (
            "golden_set_ground_truth row 1 query must be non-empty after "
