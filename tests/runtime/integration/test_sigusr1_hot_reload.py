...
class TestInferenceServiceUrlStartup:
    @pytest.mark.asyncio
    async def test_malformed_urls_fail_before_backend_startup(
        ...
        async def _unexpected_backend_wait(url):
            backend_waits.append(url)
            raise AssertionError("backend startup ran before URL validation")

        monkeypatch.setattr(
            runtime_main, "_wait_for_backend_ready", _unexpected_backend_wait
        )

        with pytest.raises(
            ValueError,
            match=r"^INFERENCE_SERVICE_URLS must be a JSON object of root HTTP\(S\) URLs$",
        ):
            async with runtime_main.lifespan(FastAPI()):
                pytest.fail("malformed inference URLs reached a running lifespan")

        assert backend_waits == []

    @pytest.mark.asyncio
    async def test_valid_urls_are_normalized_before_backend_startup(
        self, lifespan_env, monkeypatch
    ):
        import cogniverse_runtime.main as runtime_main
        from cogniverse_foundation.config.bootstrap import (
            parse_inference_service_urls as parse_urls,
        )

        raw = '{"denseon":"http://denseon:8000/"}'
        monkeypatch.setenv("INFERENCE_SERVICE_URLS", raw)
        monkeypatch.setenv("BACKEND_URL", "http://127.0.0.1")
        monkeypatch.setenv("BACKEND_PORT", "17999")
        events = []

        def _record_parse(value):
            parsed = parse_urls(value)
            events.append(("parse", value, parsed))
            return parsed

        class BackendReached(RuntimeError):
            pass

        async def _stop_at_backend(url):
            events.append(("backend", url))
            raise BackendReached(url)

        monkeypatch.setattr(runtime_main, "parse_inference_service_urls", _record_parse)
        monkeypatch.setattr(runtime_main, "_wait_for_backend_ready", _stop_at_backend)

        with pytest.raises(BackendReached):
            async with runtime_main.lifespan(FastAPI()):
                pytest.fail("sentinel backend boundary did not stop startup")

        assert events == [
            ("parse", raw, {"denseon": "http://denseon:8000"}),
            ("backend", "http://127.0.0.1:17999"),
        ]
