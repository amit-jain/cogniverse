...
async def test_cold_cache_failed_dependency_settles_waiters_and_retries(
    ...
    # The revision slots are read concurrently; the failure names the logical
    # blob, whichever slot's read failed first.
    assert failure.value.dataset == managers[TENANT_A]._blob_dataset_name(
        "config", "cold_agent"
    )
    assert failure.value.dataset == "dspy-config-acme:acme-cold_agent"
    assert type(failure.value.__cause__) is httpx.ConnectError
