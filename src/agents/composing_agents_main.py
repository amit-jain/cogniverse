...
class EnhancedA2AClientTool(BaseTool):
    ...
    async def execute(self, query: str, top_k: int = 10, start_date: str = None, end_date: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"⏱️ [{self.name}] Result formatting time: {format_end - format_start:.3f}s")
...
class QueryAnalysisTool(BaseTool):
    ...
    async def execute(self, query: str) -> Dict[str, Any]:
        ...
        logger.info(f"⏱️ [QueryAnalyzer] Temporal extraction time: {temporal_end - temporal_start:.3f}s")
        ...
        if self.inference_mode == "gliner_only" and hasattr(self, 'gliner_model') and self.gliner_model:
            ...
            try:
                ...
                logger.info(f"⏱️ [QueryAnalyzer] GLiNER inference time: {gliner_end - gliner_start:.3f}s")
...
async def route_and_execute_query(query: str, top_k: int = 10) -> Dict[str, Any]:
    ...
    try:
        ...
        if analysis.get("needs_text_search", False):
            logger.info("📄 Step 2b: TextSearchAgent needed but not available (Elasticsearch not configured)")
        ...
        if not analysis.get("needs_video_search", False) and not analysis.get("needs_text_search", False):
            logger.info("❓ No specific routing determined, defaulting to VideoSearchAgent")
...
def start_web_interface():
    ...
    logger.info(f"🌐 Web Interface: http://localhost:{config.get('composing_agent_port', 8000)}")
