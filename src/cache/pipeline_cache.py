...
class VideoArtifacts:
    ...
    def is_complete(self, pipeline_config: Dict[str, Any]) -> bool:
        ...
        if pipeline_config.get("transcribe_audio", False) and not self.audio_transcript:
            return False
        if pipeline_config.get("generate_descriptions", False) and not self.frame_descriptions:
            return False
...
class PipelineArtifactCache:
    ...
    def __init__(self, cache_manager: CacheManager, ttl: int = 604800, profile: Optional[str] = None):  # 7 days default
        """
    ...
    def _generate_video_key(self, video_path: str, video_hash: Optional[str] = None) -> str:
    ...
    def _generate_artifact_key(self, video_key: str, artifact_type: str, **kwargs) -> str:
    ...
    async def set_keyframes(
        ...
        # Store metadata
        metadata_key = self._generate_artifact_key(
            video_key,
            "keyframes",
            **params
        )
        ...
        if keyframe_images:
            for frame_info in keyframes_metadata.get("keyframes", []):
                ...
                if str(frame_id) in keyframe_images:
                    ...
                    image_data = cv2.imencode('.jpg', keyframe_images[str(frame_id)])[1].tobytes()
    ...
    async def get_all_artifacts(
        ...
        if pipeline_config.get("transcribe_audio", False):
            artifacts.audio_transcript = await self.get_transcript(
                video_path,
                model_size=pipeline_config.get("whisper_model", "base")
            )
        
        if pipeline_config.get("generate_descriptions", False):
            artifacts.frame_descriptions = await self.get_descriptions(
                video_path,
                model_name=pipeline_config.get("vlm_model", "Qwen/Qwen2-VL-2B-Instruct"),
...
class FileSystemPipelineCache(PipelineArtifactCache):
    ...
    async def get_descriptions_from_disk(self, video_id: str) -> Optional[Dict[str, Any]]:
        """Load descriptions directly from disk"""
        descriptions_file = self.base_path / "descriptions" / f"{video_id}_descriptions.json"
