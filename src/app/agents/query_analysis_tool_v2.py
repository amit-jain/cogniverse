...
class QueryAnalysisToolV2(BaseTool):
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.optimizer = None
        if self.routing_config.optimization_config.get(
            ...
            for tier, strategy in self.router.strategies.items():
                self.optimizers[tier] = AutoTuningOptimizer(
                    strategy=strategy, config=self.routing_config.optimization_config
                )
    ...
    def _convert_routing_decision(self, query: str, decision) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            "needs_video_search": decision.search_modality
            in [SearchModality.VIDEO, SearchModality.BOTH],
            "needs_text_search": decision.search_modality
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "queries_per_minute": (
                (self.total_queries / uptime * 60) if uptime > 0 else 0
            ),
...
async def integrate_with_composing_agent():
    ...
    actual_routing = {"search_modality": "video", "generation_type": "detailed_report"}

    await query_analyzer.provide_feedback(
        query=query, predicted=analysis, actual=actual_routing, user_satisfaction=0.9
    )
    ...
    if query_analyzer.total_queries % 1000 == 0:
        query_analyzer.export_metrics(
            f"routing_metrics_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
