...
async def get_agent_card():
    return {
        ...
        "url": "/tasks/send", "version": "1.0", "protocol": "a2a",
        "protocol_version": "0.2.1", "capabilities": ["tasks/send"],
        "skills": [
            {
...
async def send_task(task: Task):
    ...
    try:
        search_results = text_agent.search(query=query, top_k=query_data.get("top_k", 10))
    except Exception as e:
...
if __name__ == "__main__":
    print("--- To run the Text Search Agent server, use: uvicorn text_agent_server:app --reload --port 8002 ---")
