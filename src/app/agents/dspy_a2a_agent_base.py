...
from src.tools.a2a_utils import A2AClient, A2AMessage, AgentCard, DataPart, TextPart
...
class DSPyA2AAgentBase(ABC):
    ...
    def _setup_a2a_endpoints(self):
        ...
        async def handle_task(task: Dict[str, Any]):
            ...
            try:
                ...
                logger.debug(f"Converted A2A task to DSPy input: {list(dspy_input.keys())}")
                
                # Step 2: Process with DSPy 3.0 module
                dspy_output = await self._process_with_dspy(dspy_input)
                logger.debug(f"DSPy processing completed")
                ...
                logger.error(f"Task processing failed after {processing_time:.3f}s: {e}")
    ...
    def _a2a_to_dspy_input(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            raise DSPyA2AConversionError(f"Failed to convert A2A task to DSPy input: {e}")
    ...
    async def call_other_agent(self, 
                              agent_url: str, 
                              query: str, 
                              **kwargs) -> Dict[str, Any]:
        """
...
class SimpleDSPyA2AAgent(DSPyA2AAgentBase):
    ...
    def _dspy_to_a2a_output(self, dspy_output: Any) -> Dict[str, Any]:
        ...
        return {
            "status": "success",
            "response": str(dspy_output.response if hasattr(dspy_output, 'response') else str(dspy_output)),
            "agent": self.agent_name
        }
...
if __name__ == "__main__":
    ...
    print("Endpoints:")
    print(f"  - Agent Card: http://localhost:8001/agent.json")
    print(f"  - Health: http://localhost:8001/health")  
    print(f"  - Metrics: http://localhost:8001/metrics")
    print(f"  - Tasks: POST http://localhost:8001/tasks/send")
