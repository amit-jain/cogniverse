...
class QueryAnalysisToolV3:
    ...
    async def analyze(self, query: str, context: Optional[QueryContext] = None) -> QueryAnalysisResult:
        """
        ...
        try:
            ...
            primary_intent, secondary_intents = self._detect_intents(query, thinking_phase)
            
            # Phase 4: Complexity assessment
            complexity_level = self._assess_complexity(query, primary_intent, secondary_intents)
            
            # Phase 5: Modality detection
            modality_requirements = self._detect_modality_requirements(query, thinking_phase)
            ...
            if self.enable_query_expansion:
                expanded_queries = await self._expand_query(query, context, thinking_phase)
            
            # Phase 9: Workflow determination
            workflow_info = await self._determine_workflow(
                query, primary_intent, secondary_intents, complexity_level, thinking_phase
            )
    ...
    async def _thinking_phase(self, query: str, context: Optional[QueryContext]) -> Dict[str, Any]:
        """
        ...
        complex_terms = ["analyze", "compare", "detailed", "comprehensive", "report", "explain"]
        thinking["complexity_signals"] = [term for term in complex_terms if term in words]
        ...
        temporal_terms = ["recent", "latest", "new", "today", "yesterday", "last", "since", "before", "after"]
        thinking["temporal_indicators"] = [term for term in temporal_terms if term in words]
        ...
        if len(thinking["complexity_signals"]) > 1:
            reasoning_parts.append(f"Multiple complexity signals: {', '.join(thinking['complexity_signals'])}")
        
        if thinking["modality_hints"]["video"] and thinking["modality_hints"]["text"]:
            reasoning_parts.append("Multimodal query requiring both video and text search")
        
        if thinking["temporal_indicators"]:
            reasoning_parts.append(f"Temporal constraints detected: {', '.join(thinking['temporal_indicators'])}")
        
        if thinking["query_type_indicators"]["report"]:
            reasoning_parts.append("Detailed reporting required based on indicator words")
        elif thinking["query_type_indicators"]["summary"]:
            reasoning_parts.append("Summarization required based on indicator words")
        
        thinking["reasoning"] = ". ".join(reasoning_parts) if reasoning_parts else "Simple search query detected"
    ...
    def _detect_intents(self, query: str, thinking_phase: Dict[str, Any]) -> Tuple[QueryIntent, List[QueryIntent]]:
        """
        ...
        if thinking_phase.get("complexity_signals"):
            intent_scores[QueryIntent.ANALYZE] += len(thinking_phase["complexity_signals"])
        
        if thinking_phase.get("temporal_indicators"):
            intent_scores[QueryIntent.TEMPORAL] += 1
        
        if thinking_phase.get("modality_hints", {}).get("video") and thinking_phase.get("modality_hints", {}).get("text"):
    ...
    def _assess_complexity(self, query: str, primary_intent: QueryIntent, 
                         secondary_intents: List[QueryIntent]) -> QueryComplexity:
        """
    ...
    def _detect_modality_requirements(self, query: str, thinking_phase: Dict[str, Any]) -> Dict[str, bool]:
        """
        ...
        if any(term in query.lower() for term in ["analyze", "detailed", "visual", "image"]):
    ...
    def _extract_entities_and_keywords(self, query: str) -> Tuple[List[Dict[str, Any]], List[str]]:
        """
        ...
        for word in capitalized:
            if word.lower() not in ["show", "find", "get", "what", "when", "where", "how"]:
                entities.append({
                    "text": word,
                    "type": "proper_noun",
                    "confidence": 0.6
    ...
    async def _expand_query(self, query: str, context: Optional[QueryContext], 
                          thinking_phase: Dict[str, Any]) -> List[str]:
        """
        ...
        for original, synonym_list in synonyms.items():
            if original in words:
                for synonym in synonym_list[:2]:  # Limit to 2 synonyms
                    ...
                    if expanded != query and len(expansions) < self.max_expanded_queries:
    ...
    async def _determine_workflow(self, query: str, primary_intent: QueryIntent,
                                secondary_intents: List[QueryIntent],
                                complexity_level: QueryComplexity,
                                thinking_phase: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        elif primary_intent == QueryIntent.SUMMARIZE or thinking_phase.get("query_type_indicators", {}).get("summary"):
        ...
        if workflow_type == "summary":
            steps.append({
                ...
                "parameters": {
                    "query": query,
                    "summary_type": "comprehensive" if complexity_level == QueryComplexity.COMPLEX else "brief"
                }
            })
    ...
    def _calculate_confidence(self, primary_intent: QueryIntent, complexity_level: QueryComplexity,
                            num_entities: int, num_keywords: int) -> float:
        """
        ...
        if primary_intent in [QueryIntent.SEARCH, QueryIntent.SUMMARIZE, QueryIntent.REPORT]:
            base_confidence += 0.2
    ...
    def _log_analysis_result(self, result: QueryAnalysisResult, execution_time: float):
        ...
        logger.info(f"   ├─ Secondary Intents: {[i.value for i in result.secondary_intents]}")
        ...
        logger.info(f"   └─ Entities: {len(result.entities)}, Keywords: {len(result.keywords)}")
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "analyses_per_minute": (self.total_analyses / uptime * 60) if uptime > 0 else 0,
            "configuration": {
