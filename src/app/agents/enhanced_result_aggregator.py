...
class EnhancedResultAggregator:
    ...
    def __init__(self, **kwargs):
        ...
        self.enhancement_engine = ResultEnhancementEngine(**kwargs.get('enhancement_config', {}))
    ...
    async def aggregate_and_enhance(self, request: AggregationRequest) -> AggregatedResult:
        """
        ...
        """
        logger.info(f"Aggregating results for query: '{request.routing_decision.query}'")
        ...
        try:
            ...
            agent_results = await self._invoke_agents_parallel(request, enhanced_results)
            ...
            enhancement_stats = self.enhancement_engine.get_enhancement_statistics(enhanced_results)
    ...
    async def _enhance_search_results(self, request: AggregationRequest) -> List[EnhancedResult]:
        ...
        # Enhance results
        enhanced_results = self.enhancement_engine.enhance_results(results_to_enhance, context)
    ...
    async def _invoke_agents_parallel(
        ...
        if request.include_detailed_report and 'detailed_report' not in agents_to_invoke:
        ...
        try:
            ...
            logger.warning("Agent invocation timeout - some agents may not have completed")
            agent_results_list = []
    ...
    async def _invoke_single_agent(
        ...
        try:
            ...
            result_data = await self._simulate_agent_invocation(agent_name, agent_request_data)
    ...
    def _prepare_agent_request_data(
        ...
        base_data = {
            ...
            'enhanced_result_metadata': [result.enhancement_metadata for result in enhanced_results]
        }
    ...
    async def _simulate_agent_invocation(
        ...
        if agent_name == 'summarizer':
            return {
                ...
                'key_entities': [e['text'] for e in request_data['routing_decision']['entities'][:3]],
                'key_relationships': [f"{r['subject']} {r['relation']} {r['object']}" for r in request_data['routing_decision']['relationships'][:2]],
                'enhancement_applied': True,
                'confidence': 0.85
            }
        elif agent_name == 'detailed_report':
            return {
                ...
                'entity_analysis': {
                    'entities_identified': len(request_data['routing_decision']['entities']),
                    'relationships_identified': len(request_data['routing_decision']['relationships'])
                },
                'recommendations': ['Leverage relationship context for improved accuracy', 'Consider entity-driven result filtering'],
                'enhancement_applied': True
            }
        else:
            return {
    ...
    def get_aggregation_summary(self, result: AggregatedResult) -> Dict[str, Any]:
        ...
        return {
            ...
            'enhancement_rate': result.enhancement_statistics.get('enhancement_rate', 0) if result.enhancement_statistics else 0,
