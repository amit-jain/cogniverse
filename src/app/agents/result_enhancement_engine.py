...
class ResultEnhancementEngine:
    ...
    def __init__(self, **kwargs):
        ...
        self.contextual_connection_boost = kwargs.get("contextual_connection_boost", 0.10)
        self.max_total_boost = kwargs.get("max_total_boost", 0.50)
        
        # Thresholds
        self.entity_confidence_threshold = kwargs.get("entity_confidence_threshold", 0.5)
        self.relationship_confidence_threshold = kwargs.get("relationship_confidence_threshold", 0.6)
        self.connection_strength_threshold = kwargs.get("connection_strength_threshold", 0.3)
    ...
    def _enhance_single_result(
        ...
        relationship_matches = self._find_relationship_matches(result_text, context.relationships)
        ...
        enhancement_metadata = {
            ...
            'enhancement_factors': {
                ...
                'relationship_boost': len(relationship_matches) * self.relationship_match_boost,
                'connection_boost': len(contextual_connections) * self.contextual_connection_boost
            }
        }
    ...
    def _extract_result_text(self, result: Dict[str, Any]) -> str:
        ...
        # Frame descriptions
        if 'frame_descriptions' in result and isinstance(result['frame_descriptions'], list):
            text_parts.extend([str(desc) for desc in result['frame_descriptions']])
        
        # Segment descriptions
        if 'segment_descriptions' in result and isinstance(result['segment_descriptions'], list):
    ...
    def _find_entity_matches(
        ...
        for entity in entities:
            ...
            if (entity_text and 
                entity_confidence >= self.entity_confidence_threshold and
                entity_text in result_text):
                ...
                for entity_match_2 in entity_matches[i+1:]:
                    ...
                    if strength >= self.connection_strength_threshold:
                        connections.append({
                            ...
                            'strength': strength,
                            'description': f"Co-occurring entities in result context"
                        })
    ...
    def _calculate_cooccurrence_strength(
        ...
        import re
        entity_1_positions = [m.start() for m in re.finditer(re.escape(entity_1_text), result_text)]
        entity_2_positions = [m.start() for m in re.finditer(re.escape(entity_2_text), result_text)]
    ...
    def _create_fallback_enhanced_result(self, result: Dict[str, Any]) -> EnhancedResult:
    ...
    def get_enhancement_statistics(self, enhanced_results: List[EnhancedResult]) -> Dict[str, Any]:
        ...
        avg_enhancement_score = sum(r.enhancement_score for r in enhanced_results) / total_results
        avg_boost = sum(r.enhancement_metadata.get('boost_applied', 0) for r in enhanced_results) / total_results
        
        entity_matches = sum(len(r.entity_matches) for r in enhanced_results)
        relationship_matches = sum(len(r.relationship_matches) for r in enhanced_results)
        contextual_connections = sum(len(r.contextual_connections) for r in enhanced_results)
        ...
        return {
            ...
            'avg_relationship_matches_per_result': round(relationship_matches / total_results, 2),
            'avg_connections_per_result': round(contextual_connections / total_results, 2)
        }