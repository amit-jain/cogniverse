...
class A2ARoutingAgent:
    ...
    async def process_task(self, task: Task) -> RoutingResult:
        ...
        try:
            ...
            routing_analysis = await self.routing_agent.analyze_and_route(query, context)
    ...
    async def _send_to_agent(
        ...
        try:
            ...
            raise Exception(f"Agent {agent_endpoint.name} returned {e.response.status_code}")
        except Exception as e:
    ...
    def _aggregate_results(
        ...
        if workflow_type == "summary":
            ...
            if report_response:
                ...
                final_result["detailed_report"] = self._create_fallback_report(search_results)
    ...
    def _create_fallback_summary(self, search_results: List[Dict[str, Any]]) -> str:
        ...
        return f"Found {num_results} results from {len(sources)} sources. " + \
    ...
    def _create_fallback_report(self, search_results: List[Dict[str, Any]]) -> str:
        ...
        report_parts = [
            f"Detailed Analysis of {len(search_results)} Results:",
            ""
        ]
        ...
        for i, result in enumerate(search_results[:3]):  # Top 3 results
            ...
            if "metadata" in result:
                ...
                if "start_time" in metadata:
                    report_parts.append(f"   Time Range: {metadata.get('start_time')} - {metadata.get('end_time', 'end')}")
            ...
            try:
                ...
                if agent_endpoint:
                    ...
                    if response.status_code == 200:
                        ...
                        health_status["downstream_agents"][agent_name] = f"unhealthy (status: {response.status_code})"
                        health_status["status"] = "degraded"
                else:
                    health_status["downstream_agents"][agent_name] = "not_registered"
                    
            except Exception as e:
                health_status["downstream_agents"][agent_name] = f"unreachable ({str(e)})"
                health_status["status"] = "degraded"
