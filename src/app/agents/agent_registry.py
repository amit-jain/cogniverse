...
class AgentRegistry:
    ...
    def _initialize_from_config(self):
        ...
        if video_agent_url:
            self.register_agent(AgentEndpoint(
                name="video_search",
                url=video_agent_url,
                capabilities=["video_search", "multimodal_search", "temporal_search"]
            ))
        ...
        self.register_agent(AgentEndpoint(
            ...
            capabilities=["query_routing", "workflow_coordination", "agent_orchestration"]
        ))
    ...
    async def health_check_agent(self, agent_name: str) -> bool:
        ...
        try:
            ...
            if is_healthy:
                ...
                logger.warning(f"Agent {agent_name} health check failed: status {response.status_code}")
    ...
    async def health_check_all(self) -> Dict[str, bool]:
        ...
        if tasks:
            agent_names = [name for name, agent in self.agents.items() if agent.needs_health_check()]
    ...
    def get_registry_stats(self) -> Dict[str, any]:
        ...
        for capability, agent_names in self.capabilities.items():
            healthy_agents = [name for name in agent_names if self.agents[name].is_healthy()]
