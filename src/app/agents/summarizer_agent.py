...
class VLMInterface:
    ...
    async def analyze_visual_content(self, image_paths: List[str], query: str) -> Dict[str, Any]:
        """
    ...
    def _mock_visual_analysis(self, image_paths: List[str], query: str) -> Dict[str, Any]:
        """Mock visual analysis for testing"""
        return {
            "descriptions": [f"Visual content analysis for {Path(p).name}" for p in image_paths],
    ...
    async def _openai_visual_analysis(self, image_paths: List[str], query: str) -> Dict[str, Any]:
...
Format your response as JSON with keys: descriptions, themes, key_objects, emotions, relevance_to_query, insights"""
    ...
    async def _anthropic_visual_analysis(self, image_paths: List[str], query: str) -> Dict[str, Any]:
...
class SummarizerAgent:
    ...
    async def summarize(self, request: SummaryRequest) -> SummaryResult:
        ...
        try:
            ...
            if request.include_visual_analysis and self.visual_analysis_enabled:
                visual_insights = await self._analyze_visual_content(request, thinking_phase)
            
            # Phase 3: Generate summary based on analysis
            summary = await self._generate_summary(request, thinking_phase, visual_insights)
    ...
    async def _thinking_phase(self, request: SummaryRequest) -> ThinkingPhase:
        ...
        logger.info(f"Thinking phase complete. Found {len(key_themes)} themes, {len(content_categories)} categories")
    ...
    def _identify_visual_elements(self, search_results: List[Dict[str, Any]]) -> List[str]:
    ...
    def _generate_reasoning(self, request: SummaryRequest, themes: List[str], 
                          categories: List[str], relevance_scores: Dict[str, float]) -> str:
...
""".strip()
    ...
    async def _analyze_visual_content(self, request: SummaryRequest, 
        ...
        try:
            visual_analysis = await self.vlm.analyze_visual_content(image_paths, request.query)
            
            insights = visual_analysis.get("insights", [])
            descriptions = visual_analysis.get("descriptions", [])
            
            # Combine insights and descriptions
            visual_insights = insights + [f"Visual: {desc}" for desc in descriptions[:3]]
    ...
    async def _generate_summary(self, request: SummaryRequest, 
                              thinking_phase: ThinkingPhase, 
                              visual_insights: List[str]) -> str:
    ...
    def _generate_brief_summary(self, request: SummaryRequest, 
        ...
        return f"Found {result_count} results for '{request.query}' covering {themes}. " \
               f"Content includes {', '.join(thinking_phase.content_categories)} " \
               f"with varying relevance to the search query."
    
    def _generate_bullet_summary(self, request: SummaryRequest, 
                                results: List[Dict[str, Any]], 
                                thinking_phase: ThinkingPhase) -> str:
    ...
    def _generate_comprehensive_summary(self, request: SummaryRequest, 
                                      results: List[Dict[str, Any]], 
                                      thinking_phase: ThinkingPhase,
                                      visual_insights: List[str]) -> str:
        ...
        # Conclusion
        avg_relevance = sum(
            thinking_phase.relevance_scores.values()
        ) / len(thinking_phase.relevance_scores) if thinking_phase.relevance_scores else 0
    ...
    def _extract_key_points(self, request: SummaryRequest, 
                           thinking_phase: ThinkingPhase, 
        ...
        if thinking_phase.key_themes:
            key_points.append(f"Main themes: {', '.join(thinking_phase.key_themes[:3])}")
        
        # Content type points
        if thinking_phase.content_categories:
            key_points.append(f"Content types: {', '.join(thinking_phase.content_categories)}")
        ...
        if thinking_phase.visual_elements:
            key_points.append(f"Visual content: {', '.join(thinking_phase.visual_elements)}")
        
        return key_points
    
    def _calculate_confidence(self, request: SummaryRequest, 
                            thinking_phase: ThinkingPhase) -> float:
        ...
        if thinking_phase.relevance_scores:
            avg_relevance = sum(thinking_phase.relevance_scores.values()) / len(thinking_phase.relevance_scores)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--vlm-model", type=str, default="gpt-4-vision-preview", help="VLM model to use")
