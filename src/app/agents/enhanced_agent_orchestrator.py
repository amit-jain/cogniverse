...
class EnhancedAgentOrchestrator:
    ...
    def __init__(self, **kwargs):
        ...
        # Configuration
        self.default_profiles = kwargs.get('default_profiles', ['video_colpali_smol500_mv_frame'])
    ...
    async def process_complete_pipeline(self, request: ProcessingRequest) -> ProcessingResult:
        """
        ...
        try:
            ...
            search_results = await self._perform_enhanced_search(request, routing_decision)
    ...
    async def _perform_enhanced_routing(self, request: ProcessingRequest) -> RoutingDecision:
        ...
        try:
            ...
            logger.info(f"Routing complete: {routing_decision.recommended_agent} "
                ...
                for strategy in strategies:
                    try:
                        results = await self.vespa_client.query(
                            ...
                            profile=profile,
                            strategy=strategy,
                            top_k=request.top_k // len(profiles)  # Distribute across profiles
                        )
    ...
    def _generate_processing_summary(
        ...
        aggregation_summary = self.result_aggregator.get_aggregation_summary(aggregated_result)
        
        return {
            'query_analysis': {
                'original_query': request.query,
                'enhanced_query': routing_decision.enhanced_query,
                'query_enhanced': routing_decision.enhanced_query != routing_decision.query,
            ...
            'agent_processing': {
                ...
                'detailed_report_generated': aggregation_summary.get('has_detailed_report', False)
            },
            'performance_metrics': {
                'total_processing_time': total_time,
...
async def process_routing_only(query: str, enable_relationship_extraction: bool = True, enable_query_enhancement: bool = True):
    """Process only the routing phase"""
    if not orchestrator:
        raise HTTPException(status_code=503, detail="Orchestrator not initialized")
    
    try:
        routing_decision = await orchestrator.routing_agent.analyze_and_route_with_relationships(
            query=query,
