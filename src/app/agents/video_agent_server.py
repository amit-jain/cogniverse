...
class VideoSearchAgent:
    ...
    def __init__(self, **kwargs):
        ...
        if active_profile and active_profile in profiles:
            model_name = profiles[active_profile].get(
                "embedding_model", config.get("colpali_model", "vidore/colsmol-500m")
            )
        ...
        try:
            ...
            print(
                f"Successfully initialized Vespa search client at {vespa_url}:{vespa_port}"
            )
            print(
                f"Active profile: {active_profile}, Embedding type: {self.embedding_type}"
            )
        except Exception as e:
            print(
                f"Could not initialize Vespa search client. Please ensure Vespa is running. Error: {e}"
            )
    ...
    def _search_vespa(
        self, query: str, top_k: int, start_date: Optional[str], end_date: Optional[str]
    ) -> List[Dict[str, Any]]:
        if not self.vespa_client:
            raise ConnectionError("Vespa search client is not available.")

        logger.info("📺 VIDEO AGENT: Received search request")
        ...
        try:
            ...
            logger.info(
                f"   Embedding shape: {query_embeddings.shape if query_embeddings is not None else 'None'}"
            )
            ...
            logger.info("🔍 Forwarding to Vespa search client...")
            logger.info(f"   Search params: {search_params}")
            logger.info(
                f"   Embeddings provided: {'Yes' if query_embeddings is not None else 'No'}"
            )
            ...
            logger.info(
                f"✅ Vespa search completed: {len(results)} hits in {search_time:.3f}s"
            )
            logger.info(f"   Total request time: {total_time:.3f}s")

            if results:
                logger.info(
                    f"   Score range: {results[0]['relevance']:.3f} to {results[-1]['relevance']:.3f}"
                )
                ...
                for i, result in enumerate(results[:3]):
                    logger.info(
                        f"   Result {i+1}: {result['video_id']} frame {result['frame_id']} (score: {result['relevance']:.3f})"
                    )
...
async def get_agent_card():
    return {
        ...
        "description": "Finds video clips based on text and time filters.",
        "url": "/tasks/send",
        "version": "1.0",
        "protocol": "a2a",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--profile",
        type=str,
        help="Video processing profile to use (sets VIDEO_PROFILE env var)",
    )
    parser.add_argument(
    ...
    print("--- Vespa backend configured ---")
    print(
        f"--- To run manually: uvicorn video_agent_server:app --reload --port {args.port} ---"
    )
