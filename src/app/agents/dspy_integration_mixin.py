...
class DSPyIntegrationMixin:
        ...
        try:
            ...
            for prompt_file in search_paths:
                if prompt_file.exists():
                    ...
                    logger.info(f"Loaded DSPy optimized prompts for {agent_type} from {prompt_file}")
                    break
            else:
                logger.info(f"No DSPy optimized prompts found for {agent_type}, using original prompts")
    ...
    def get_optimized_prompt(self, prompt_key: str, default_prompt: str = "") -> str:
        ...
        try:
            ...
            if 'signature' in compiled_prompts:
                ...
                return self._extract_prompt_from_signature(signature_text, prompt_key, default_prompt)
    ...
    def _extract_prompt_from_signature(self, signature_text: str, prompt_key: str, default_prompt: str) -> str:
    ...
    def get_dspy_metadata(self) -> Dict[str, Any]:
        ...
        if self.dspy_enabled:
            ...
            metadata['prompt_keys'] = list(self.dspy_optimized_prompts.get('compiled_prompts', {}).keys())
    ...
    def apply_dspy_optimization(self, prompt_template: str, context: Dict[str, Any]) -> str:
        """
    ...
    async def test_dspy_optimization(self, sample_input: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for prompt_key in ['system', 'analysis', 'routing', 'summary']:
                optimized = self.get_optimized_prompt(prompt_key, f"default_{prompt_key}_prompt")
...
4. Content type needs"""
        ...
        return self.apply_dspy_optimization(default_prompt, {
            'query': query,
            'context': context
        })
...
class DSPyRoutingMixin(DSPyIntegrationMixin):
    ...
    def get_optimized_routing_prompt(self, query: str, analysis_result: Dict[str, Any], 
...
class DSPySummaryMixin(DSPyIntegrationMixin):
    ...
    def get_optimized_summary_prompt(self, content: str, summary_type: str, 
...
class DSPyDetailedReportMixin(DSPyIntegrationMixin):
    ...
    def get_optimized_report_prompt(self, search_results: list, query_context: str,
