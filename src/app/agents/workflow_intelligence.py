...
class WorkflowOptimizationSignature(dspy.Signature):
    ...
    workflow_history: str = dspy.InputField(desc="JSON string of historical workflow executions")
    current_query: str = dspy.InputField(desc="Current query to optimize workflow for")
    agent_performance: str = dspy.InputField(desc="JSON string of agent performance metrics")
    ...
    expected_improvement: float = dspy.OutputField(desc="Expected performance improvement percentage")
...
class TemplateGeneratorSignature(dspy.Signature):
    ...
    successful_workflows: str = dspy.InputField(desc="JSON string of successful similar workflows")
    ...
    task_sequence: List[Dict[str, str]] = dspy.OutputField(desc="Optimized task sequence")
    applicability_criteria: List[str] = dspy.OutputField(desc="When to use this template")
...
class WorkflowIntelligence:
    ...
    def _create_fallback_modules(self) -> None:
        ...
        class FallbackOptimizerModule(dspy.Module):
            def forward(self, workflow_history: str, current_query: str, agent_performance: str):
    ...
    def _initialize_persistence(self) -> None:
        ...
        try:
            self.db_connection = sqlite3.connect("workflow_intelligence.db", check_same_thread=False)
            self._create_tables()
    ...
    def _load_historical_data(self) -> None:
        ...
        try:
            ...
            cursor.execute("SELECT * FROM workflow_executions ORDER BY timestamp DESC LIMIT ?", 
    ...
    async def record_workflow_execution(self, workflow_plan: WorkflowPlan) -> None:
        ...
        try:
            ...
            execution = WorkflowExecution(
                ...
                execution_time=(workflow_plan.end_time - workflow_plan.start_time).total_seconds() if workflow_plan.end_time and workflow_plan.start_time else 0.0,
            ...
            self.logger.debug(f"Recorded workflow execution: {workflow_plan.workflow_id}")
    ...
    async def optimize_workflow_plan(
        ...
        try:
            ...
            if template_match:
                ...
                self.logger.info(f"Applied template '{template_match.name}' for optimization")
                return optimized_plan
            
            # Use DSPy for intelligent optimization
            optimized_plan = await self._dspy_optimize_workflow(query, initial_plan, optimization_context)
    ...
    async def _dspy_optimize_workflow(
        ...
        try:
            ...
            performance_data = {
                name: {
                    "success_rate": perf.successful_executions / max(perf.total_executions, 1),
            ...
            optimization_reasoning = getattr(optimization_result, "reasoning", "DSPy optimization")
            expected_improvement = getattr(optimization_result, "expected_improvement", 0.0)
    ...
    def _create_optimized_plan(
        ...
        for i, agent_name in enumerate(optimized_sequence):
            ...
            if agent_name == "video_search_agent":
                ...
                task_query = f"Generate detailed report for: {initial_plan.original_query}"
                dependencies = {f"optimized_task_{j}" for j in range(i) if j >= 0}
            else:
                task_query = initial_plan.original_query
                dependencies = set()
        ...
        optimized_plan.execution_order = self._calculate_execution_order(optimized_tasks)
    ...
    def _find_matching_template(self, query: str) -> Optional[WorkflowTemplate]:
        ...
        for template in self.workflow_templates.values():
            ...
            if weighted_score > best_score and weighted_score > 0.6:  # Minimum similarity threshold
                best_score = weighted_score
                best_match = template
        
        return best_match

    def _apply_template(self, initial_plan: WorkflowPlan, template: WorkflowTemplate) -> WorkflowPlan:
    ...
    def _optimize_for_performance(self, plan: WorkflowPlan) -> WorkflowPlan:
        ...
        for task in plan.tasks:
            ...
            if agent_name in self.agent_performance:
                ...
                success_rate = perf.successful_executions / max(perf.total_executions, 1)
                time_factor = 1.0 / (1.0 + perf.average_execution_time)  # Inverse of time
                confidence_factor = perf.average_confidence
                
                composite_score = (success_rate * 0.4 + time_factor * 0.3 + confidence_factor * 0.3)
            ...
            if task.status == TaskStatus.COMPLETED:
                ...
                if task.end_time and task.start_time:
                    ...
                    perf.average_execution_time = (
                        (perf.average_execution_time * (perf.total_executions - 1) + task_time) /
    ...
    async def _evaluate_template_creation(self, execution: WorkflowExecution) -> None:
        ...
        similar_executions = [
            exec for exec in self.workflow_history
            if (exec.success and 
                exec.query_type == execution.query_type and
                exec.agent_sequence == execution.agent_sequence and
                exec != execution)
        ]
    ...
    async def _create_workflow_template(
        ...
        try:
            ...
            template = WorkflowTemplate(
                ...
                name=getattr(template_result, "template_name", f"Template for {seed_execution.query_type}"),
                description=getattr(template_result, "template_description", "Generated workflow template"),
                query_patterns=[seed_execution.query_type],
                task_sequence=getattr(template_result, "task_sequence", []),
                expected_execution_time=statistics.mean([exec.execution_time for exec in [seed_execution] + similar_executions]),
    ...
    def _classify_query_type(self, query: str) -> str:
        ...
        if any(word in query_lower for word in ["video", "show", "watch", "visual", "footage"]):
            return "video_search"
        ...
        elif any(word in query_lower for word in ["report", "detailed", "comprehensive", "document"]):
        ...
        # Multi-step queries
        elif any(word in query_lower for word in ["then", "and", "followed by", "after", "next"]):
            return "multi_step"
    ...
    def _calculate_parallel_efficiency(self, workflow_plan: WorkflowPlan) -> float:
        ...
        max_task_time = max(
            ...
            for task in workflow_plan.tasks
            if task.end_time and task.start_time
        ) if workflow_plan.tasks else 0.0
        
        # Calculate actual execution time
        actual_time = (
            (workflow_plan.end_time - workflow_plan.start_time).total_seconds()
            if workflow_plan.end_time and workflow_plan.start_time else 0.0
        )
        
        # Efficiency = theoretical_minimum / actual_time
        return max_task_time / actual_time if actual_time > 0 else 0.0

    def _get_relevant_history(self, query: str, limit: int = 20) -> List[WorkflowExecution]:
    ...
    def get_intelligence_statistics(self) -> Dict[str, Any]:
        ...
        stats.update({
            ...
            "success_rate": (
                ...
                len(self.workflow_history) if self.workflow_history else 0.0
            ),
            "average_execution_time": (
                statistics.mean([exec.execution_time for exec in self.workflow_history]) 
                if self.workflow_history else 0.0
            ),
            "query_types": dict(defaultdict(int, {
                exec.query_type: sum(1 for e in self.workflow_history if e.query_type == exec.query_type)
