...
class A2AQueryRequest(BaseModel):
    ...
    routing_options: Optional[Dict[str, Any]] = Field(None, description="Routing configuration")
...
class A2AQueryResponse(BaseModel):
    ...
    # Enhanced fields
    enhanced_query: Optional[str] = Field(None, description="Enhanced query with relationship context")
    fallback_agents: List[str] = Field(default_factory=list, description="Fallback agent recommendations")
    
    # Orchestration fields
    needs_orchestration: bool = Field(False, description="Whether orchestration is recommended")
    workflow_id: Optional[str] = Field(None, description="Workflow identifier if orchestrated")
    ...
    routing_method: str = Field(..., description="Routing method used (enhanced/fallback)")
...
class OrchestrationRequest(BaseModel):
    ...
    user_id: Optional[str] = Field(None, description="User identifier")
    preferences: Optional[Dict[str, Any]] = Field(None, description="User preferences")
    force_orchestration: bool = Field(False, description="Force orchestration even for simple queries")
...
class OrchestrationResponse(BaseModel):
    ...
    result: Optional[Dict[str, Any]] = Field(None, description="Final orchestrated result")
...
class A2AEnhancedGateway:
    ...
    def _initialize_enhanced_system(self, config: Optional[EnhancedRoutingConfig]) -> None:
    ...
    def _create_fastapi_app(self) -> FastAPI:
        ...
        async def orchestrate_query(request: OrchestrationRequest) -> OrchestrationResponse:
        ...
        async def get_statistics():
            ...
            if self.enhanced_system_available:
                gateway_stats["enhanced_routing_stats"] = self.enhanced_router.get_routing_statistics()
            
            # Add orchestration stats if available
            if self.enable_orchestration and hasattr(self, 'orchestrator'):
                gateway_stats["orchestration_stats"] = self.orchestrator.get_orchestration_statistics()
    ...
    async def _handle_routing_request(self, request: A2AQueryRequest) -> A2AQueryResponse:
        ...
        try:
            ...
            # Fallback to original routing system
                ...
                # Ultimate fallback
                return self._create_emergency_response(request, start_time, "No routing systems available")
    ...
    async def _enhanced_routing(self, request: A2AQueryRequest, start_time: datetime) -> A2AQueryResponse:
        ...
        response = A2AQueryResponse(
            ...
            needs_orchestration=routing_decision.routing_metadata.get("needs_orchestration", False),
    ...
    async def _fallback_routing(self, request: A2AQueryRequest, start_time: datetime) -> A2AQueryResponse:
        ...
        try:
            ...
            if hasattr(self.fallback_router, 'analyze_and_route'):
                ...
                result = await self.fallback_router.analyze_and_route(request.query, request.context)
            else:
    ...
    def _create_emergency_response(
        ...
        if any(word in query_lower for word in ["video", "visual", "image", "watch", "show"]):
        ...
        elif any(word in query_lower for word in ["report", "analysis", "detailed", "comprehensive"]):
    ...
    async def _handle_orchestration_request(self, request: OrchestrationRequest) -> OrchestrationResponse:
    ...
    def start_server(self, host: str = "0.0.0.0", port: Optional[int] = None) -> None:
        ...
        self.logger.info(f"Enhanced routing: {'enabled' if self.enhanced_system_available else 'disabled'}")
        self.logger.info(f"Orchestration: {'enabled' if self.enable_orchestration else 'disabled'}")
        self.logger.info(f"Fallback routing: {'enabled' if self.enable_fallback else 'disabled'}")
        ...
        for i, request in enumerate(test_requests, 1):
            ...
            try:
                ...
                if response.needs_orchestration:
                    ...
                    orch_response = await gateway._handle_orchestration_request(orchestration_request)
                    print(f"  Orchestration: {orch_response.status} (workflow: {orch_response.workflow_id})")
