...
class FrameSegmentationStrategy(BaseStrategy):
    ...
    def __init__(self, fps: float = 1.0, threshold: float = 0.999, max_frames: int = 3000):
        self.fps = fps
...
class ChunkSegmentationStrategy(BaseStrategy):
    ...
    def __init__(self, chunk_duration: float = 30.0, chunk_overlap: float = 0.0, cache_chunks: bool = True):
...
class SingleVectorSegmentationStrategy(BaseStrategy):
    ...
    def __init__(self, strategy: str = "sliding_window", segment_duration: float = 6.0,
                 segment_overlap: float = 1.0, sampling_fps: float = 2.0,
                 max_frames_per_segment: int = 12, store_as_single_doc: bool = False):
    ...
    async def segment(self, video_path: Path, pipeline_context: Any, 
                     transcript_data: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        if hasattr(pipeline_context, 'processor_manager'):
            ...
            single_vector_processor = pipeline_context.processor_manager.get_processor('single_vector')
            ...
            if single_vector_processor:
                ...
                return {
                    "single_vector_processing": processed_data_serializable
                }
            
            # Fallback: check for old-style processor
            elif hasattr(pipeline_context, 'single_vector_processor') and pipeline_context.single_vector_processor:
...
class VLMDescriptionStrategy(BaseStrategy):
    ...
    async def generate_descriptions(self, keyframes_data: Dict[str, Any],
                                   video_path: Path, pipeline_context: Any,
                                   cached_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
...
class MultiVectorEmbeddingStrategy(BaseStrategy):
    ...
    async def generate_embeddings_with_processor(self, results: Dict[str, Any],
        ...
        wrapped_results = {
            "video_id": pipeline_context.video_path.stem if hasattr(pipeline_context, 'video_path') else "unknown",
            "video_path": str(pipeline_context.video_path) if hasattr(pipeline_context, 'video_path') else "",
            "results": results
        }
...
class SingleVectorEmbeddingStrategy(BaseStrategy):
    ...
    async def generate_embeddings_with_processor(self, results: Dict[str, Any],
        ...
        if 'single_vector_processing' in results:
            wrapped_results['results']['single_vector_processing'] = results['single_vector_processing']
