...
class VideoChunkExtractor:
    ...
    def extract_chunks(
        ...
        self.logger.info(f"Video duration: {duration:.2f}s, chunk duration: {self.chunk_duration}s")
        ...
        for idx, start_time in enumerate(chunk_starts):
            ...
            if self.cache_chunks and chunk_path.exists():
                ...
                self.logger.info(f"Extracting chunk {idx}: {start_time:.2f}s - {end_time:.2f}s")
