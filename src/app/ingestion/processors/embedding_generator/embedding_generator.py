...
class EmbeddingGenerator(BaseEmbeddingGenerator):
    ...
    def _generate_video_chunks_embeddings(
        ...
        documents = []
        for segment_idx, (segment, embeddings) in enumerate(zip(segments, embeddings_list)):
            if embeddings is None:
                continue
            ...
            if 'segment_description' in segment:
                additional_fields['segment_description'] = segment['segment_description']
            additional_fields['total_segments'] = num_segments
            additional_fields['segment_duration'] = segment['end_time'] - segment['start_time']
        ...
        self.logger.info(f"✅ Successfully fed {documents_fed}/{len(documents)} documents for {video_id}")
