...
class KeyframeProcessor(BaseProcessor):
    ...
    def __init__(self, logger: logging.Logger, threshold: float = 0.999, 
                 max_frames: int = 3000, fps: Optional[float] = None):
        """
    ...
    @classmethod
    def from_config(cls, config: Dict[str, Any], logger: logging.Logger) -> 'KeyframeProcessor':
    ...
    def extract_keyframes(self, video_path: Path, output_dir: Path = None) -> Dict[str, Any]:
        """Extract keyframes from video using specified method."""
        self.logger.info(f"📸 Extracting keyframes from: {video_path.name} (mode: {self.extraction_mode})")
        ...
        if self.extraction_mode == 'fps' and self.fps:
            return self._extract_keyframes_fps(video_path, keyframes_dir, metadata_file, video_id)
        else:
            return self._extract_keyframes_histogram(video_path, keyframes_dir, metadata_file, video_id)
    
    def _extract_keyframes_fps(self, video_path: Path, keyframes_dir: Path, 
                              metadata_file: Path, video_id: str) -> Dict[str, Any]:
    ...
    def _extract_keyframes_histogram(self, video_path: Path, keyframes_dir: Path,
                                   metadata_file: Path, video_id: str) -> Dict[str, Any]:
        ...
        self.logger.info(f"   ✅ Extracted {len(keyframes)} keyframes using histogram method in {extraction_time:.2f}s")
