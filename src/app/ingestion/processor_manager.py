...
class ProcessorManager:
    ...
    def __init__(self, logger: logging.Logger, plugin_dir: Optional[Path] = None):
        ...
        self.logger.info(f"🔍 Discovered {len(self._processor_classes)} processor types")
    ...
    def initialize_from_strategies(self, strategy_set):
        ...
        for strategy in strategies:
            if isinstance(strategy, BaseStrategy):
                ...
                self.logger.info(f"🔧 {strategy_name} requires: {list(requirements.keys())}")
        ...
        for processor in self._processors.values():
            if hasattr(processor, 'cleanup'):
                try:
                    ...
                    self.logger.error(f"Error cleaning up {processor.get_processor_name()}: {e}")
