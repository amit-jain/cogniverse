...
class RoutingOptimizer:
        ...
        if isinstance(config, dict):
            ...
            for k, v in config.items():
                if k in valid_fields:
                    ...
                    if k in ['checkpoint_dir', 'metrics_export_dir'] and isinstance(v, str):
                        v = Path(v)
                    filtered_config[k] = v
            self.config = OptimizationConfig(**filtered_config) if filtered_config else OptimizationConfig()
        else:
    ...
    def track_performance(self, query: str, predicted: RoutingDecision, 
    ...
    async def optimize(self):
        ...
        if not self.baseline_metrics or current_metrics.accuracy > self.baseline_metrics.accuracy:
            self.baseline_metrics = current_metrics
        
        # Export metrics
        self._export_metrics(current_metrics)
        
        logger.info(f"Optimization complete. Current accuracy: {current_metrics.accuracy:.2%}")
...
class AutoTuningOptimizer(RoutingOptimizer):
    ...
    def __init__(self, strategy: RoutingStrategy, config: Optional[OptimizationConfig] = None):
        """
    ...
    async def optimize(self):
        ...
        logger.info(f"Running auto-tuning optimization attempt #{self.optimization_attempts}")
    ...
    async def _optimize_gliner(self):
        ...
        logger.info(f"Optimized GLiNER threshold: {best_threshold:.2f} (score: {best_score:.3f})")
    ...
    async def _optimize_gliner_labels(self):
        ...
        for record in list(self.performance_history)[-100:]:  # Last 100 records
            if record.get("predicted", {}).get("entities_detected"):
                ...
                is_correct = record.get("actual") and record["predicted"]["search_modality"] == record["actual"]["search_modality"]
        ...
        if label_effectiveness:
            sorted_labels = sorted(label_effectiveness.items(), key=lambda x: x[1], reverse=True)
    ...
    async def _optimize_keyword(self):
        ...
        if keyword_effectiveness:
            sorted_keywords = sorted(keyword_effectiveness.items(), key=lambda x: x[1], reverse=True)
    ...
    async def _optimize_ensemble_weights(self):
        ...
        for strategy_name, scores in strategy_scores.items():
            if scores:
                ...
                new_weights[strategy_name] = max(0.1, avg_score)  # Minimum weight of 0.1
    ...
    async def _optimize_confidence_thresholds(self):
        ...
        logger.info(f"Optimized confidence threshold: {best_threshold} (score: {best_score:.3f})")
