...
class BasicQueryAnalysisSignature(dspy.Signature):
    ...
    # Search requirements
    needs_video_search: bool = dspy.OutputField(desc="Whether video content search is needed")
    needs_text_search: bool = dspy.OutputField(desc="Whether text/document search is needed") 
    needs_multimodal: bool = dspy.OutputField(desc="Whether multimodal processing is required")
    ...
    confidence_score: float = dspy.OutputField(desc="Confidence in routing decision (0.0-1.0)")
...
class EntityExtractionSignature(dspy.Signature):
    ...
    domain_context: Optional[str] = dspy.InputField(desc="Domain context for better extraction")
    ...
    key_entities: List[str] = dspy.OutputField(desc="Most important entities for the query")
    
    # Context analysis
    domain_classification: str = dspy.OutputField(desc="Identified domain/topic of the query")
...
class RelationshipExtractionSignature(dspy.Signature):
    ...
    entities: List[Dict[str, Any]] = dspy.InputField(desc="Previously extracted entities")
    linguistic_context: Optional[str] = dspy.InputField(desc="Linguistic analysis context")
    ...
    relationship_types: List[str] = dspy.OutputField(desc="Types of relationships found")
    semantic_connections: List[str] = dspy.OutputField(desc="Semantic connections between entities")
    
    # Analysis outputs
    query_structure: str = dspy.OutputField(desc="Grammatical/semantic structure analysis")
    complexity_indicators: List[str] = dspy.OutputField(desc="Factors indicating query complexity")
...
class QueryEnhancementSignature(dspy.Signature):
    ...
    relationships: List[Dict[str, Any]] = dspy.InputField(desc="Extracted relationship tuples")
    search_context: str = dspy.InputField(desc="Target search system context")
    
    # Enhancement outputs
    enhanced_query: str = dspy.OutputField(desc="Query enhanced with relationship context")
    semantic_expansions: List[str] = dspy.OutputField(desc="Additional terms for better retrieval")
    relationship_phrases: List[str] = dspy.OutputField(desc="Natural language relationship descriptions")
    
    # Strategy outputs
    enhancement_strategy: str = dspy.OutputField(desc="Strategy used for enhancement")
    search_operators: List[str] = dspy.OutputField(desc="Suggested search operators (AND, OR)")
...
class MultiAgentOrchestrationSignature(dspy.Signature):
    ...
    agent_capabilities: Dict[str, List[str]] = dspy.InputField(desc="Capabilities of each agent")
    ...
    # Success criteria
    success_metrics: Dict[str, Any] = dspy.OutputField(desc="How to measure task success")
    fallback_strategy: str = dspy.OutputField(desc="What to do if primary plan fails")
    expected_processing_time: float = dspy.OutputField(desc="Estimated total processing time")
...
class AdvancedRoutingSignature(dspy.Signature):
    ...
    user_preferences: Optional[Dict[str, Any]] = dspy.InputField(desc="User preferences")
    system_state: Optional[Dict[str, Any]] = dspy.InputField(desc="Current system state")
    
    # Comprehensive analysis
    query_analysis: Dict[str, Any] = dspy.OutputField(desc="Complete query analysis results")
    extracted_entities: List[Dict[str, Any]] = dspy.OutputField(desc="All extracted entities")
    extracted_relationships: List[Dict[str, Any]] = dspy.OutputField(desc="All relationship tuples")
    enhanced_query: str = dspy.OutputField(desc="Relationship-enhanced query")
    
    # Routing decision
    routing_decision: Dict[str, Any] = dspy.OutputField(desc="Complete routing decision")
    agent_workflow: List[Dict[str, Any]] = dspy.OutputField(desc="Step-by-step agent workflow")
    optimization_suggestions: List[str] = dspy.OutputField(desc="Suggestions for better performance")
    
    # Confidence and quality
    overall_confidence: float = dspy.OutputField(desc="Overall confidence in routing decision")
...
class MetaRoutingSignature(dspy.Signature):
    ...
    query_characteristics: Dict[str, Any] = dspy.InputField(desc="Query complexity and features")
    routing_history: List[Dict[str, Any]] = dspy.InputField(desc="Recent routing decisions")
    performance_metrics: Dict[str, Any] = dspy.InputField(desc="System performance data")
...
class AdaptiveThresholdSignature(dspy.Signature):
    ...
    recent_performance: List[Dict[str, Any]] = dspy.InputField(desc="Recent routing performance data")
    query_patterns: List[Dict[str, Any]] = dspy.InputField(desc="Patterns in recent queries")
    user_feedback: Optional[List[Dict[str, Any]]] = dspy.InputField(desc="User satisfaction scores")
    system_metrics: Dict[str, Any] = dspy.InputField(desc="System-wide performance metrics")
    
    # Adaptive thresholds
    fast_path_threshold: float = dspy.OutputField(desc="Optimal fast path confidence threshold")
    slow_path_threshold: float = dspy.OutputField(desc="Optimal slow path confidence threshold")
    escalation_threshold: float = dspy.OutputField(desc="When to escalate between tiers")
    
    # Learning insights
    performance_insights: List[str] = dspy.OutputField(desc="Key performance insights")
    optimization_opportunities: List[str] = dspy.OutputField(desc="Areas for improvement")
    confidence_explanation: str = dspy.OutputField(desc="Reasoning for threshold values")
...
def validate_signature_output(signature_output: dspy.Prediction, 
