...
class AdvancedRoutingOptimizer:
    ...
    def _initialize_advanced_components(self):
        ...
        try:
            ...
            class RoutingPolicySignature(dspy.Signature):
                ...
                enhanced_query = dspy.InputField(desc="Enhanced query with relationship context")
            ...
            class OptimizedRoutingPolicy(dspy.Module):
                ...
                def forward(self, query, entities=None, relationships=None, enhanced_query=None):
    ...
    def _initialize_confidence_calibrator(self):
        ...
        try:
            class ConfidenceCalibratorSignature(dspy.Signature):
                ...
                raw_confidence = dspy.InputField(desc="Raw confidence from routing model")
                query_complexity = dspy.InputField(desc="Complexity indicators of the query")
                historical_accuracy = dspy.InputField(desc="Historical accuracy for similar queries")
                
                calibrated_confidence = dspy.OutputField(desc="Calibrated confidence score (0-1)")
            ...
            class ConfidenceCalibrator(dspy.Module):
                ...
                def forward(self, raw_confidence, query_complexity=0.5, historical_accuracy=0.7):
    ...
    async def record_routing_experience(
        ...
        logger.debug(f"Recorded routing experience: reward={reward:.3f}, agent={chosen_agent}")
        return reward
    ...
    def _compute_reward(
        ...
        if user_satisfaction is not None:
            ...
            total_weight = self.config.search_quality_weight + self.config.agent_success_weight
        ...
        if processing_time > 0:
            ...
            time_penalty = self.config.processing_time_penalty * (1.0 - 1.0 / (1.0 + processing_time / 10.0))
    ...
    def _update_metrics(self, experience: RoutingExperience):
        ...
        if self.metrics.avg_reward == 0.0:
            ...
            self.metrics.avg_reward = (1 - alpha) * self.metrics.avg_reward + alpha * experience.reward
        ...
        if len(self.experiences) > 1:
            confidence_predictions = [exp.routing_confidence for exp in self.experiences[-100:]]
            actual_outcomes = [1.0 if exp.agent_success else 0.0 for exp in self.experiences[-100:]]
    ...
    async def _run_optimization_step(self):
        ...
        try:
            ...
            if self.routing_policy and training_examples:
                ...
                logger.info(f"GRPO optimization step {self.training_step} complete. Epsilon: {self.current_epsilon:.3f}")
    ...
    def _get_historical_accuracy_for_query_type(self, query: str) -> float:
        ...
        for exp in self.experiences[-200:]:  # Last 200 experiences
            ...
            similarity = len(query_words.intersection(exp_words)) / max(len(query_words), len(exp_words), 1)
        ...
        success_rate = sum(1 for exp in similar_experiences if exp.agent_success) / len(similar_experiences)
        return success_rate
    
    def _apply_baseline_improvements(self, baseline_prediction: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if agent in self.metrics.agent_preferences:
            ...
            improved["confidence"] = max(0.0, min(1.0, original_confidence + confidence_boost))
    ...
    def _add_exploration_noise(self, baseline_prediction: Dict[str, Any]) -> Dict[str, Any]:
        ...
        original_confidence = baseline_prediction.get("confidence", 0.7)
        exploration["confidence"] = max(0.0, min(1.0, original_confidence + confidence_noise))
        ...
        if np.random.random() < 0.1:
            ...
            current_agent = baseline_prediction.get("recommended_agent", "video_search_agent")
            other_agents = [a for a in agents if a != current_agent]
            if other_agents:
                exploration["recommended_agent"] = np.random.choice(other_agents)
                exploration["reasoning"] = f"Exploration: trying {exploration['recommended_agent']}"
    ...
    def _load_stored_data(self):
        ...
        try:
            ...
            if metrics_file.exists():
                ...
                self.metrics.total_experiences = metrics_dict.get('total_experiences', 0)
                self.metrics.avg_reward = metrics_dict.get('avg_reward', 0.0)
                self.metrics.successful_routes = metrics_dict.get('successful_routes', 0)
                self.metrics.failed_routes = metrics_dict.get('failed_routes', 0)
                self.metrics.confidence_accuracy = metrics_dict.get('confidence_accuracy', 0.0)
                self.metrics.agent_preferences = metrics_dict.get('agent_preferences', {})
                self.metrics.query_type_accuracy = metrics_dict.get('query_type_accuracy', {})
                ...
                self.current_epsilon = metrics_dict.get('current_epsilon', self.config.exploration_epsilon)
    ...
    def get_optimization_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'metrics': {
                ...
                'failed_routes': self.metrics.failed_routes,
                'success_rate': round(
                    self.metrics.successful_routes / max(1, self.metrics.total_experiences), 3
                ),
