...
class GLiNERRelationshipExtractor:
    ...
    def extract_entities(
        ...
        if labels is None:
            ...
            labels = [
                ...
                "TECHNOLOGY", "CONCEPT", "ACTION", "OBJECT", "ANIMAL",
                "SPORT", "ACTIVITY", "TOOL", "VEHICLE", "MATERIAL"
            ]
    ...
    def _calculate_relationship_confidence(
        ...
        relationship_indicators = [
            "with", "using", "showing", "playing", "performing",
            "demonstrating", "teaching", "learning", "in", "at", "for"
        ]
...
class SpaCyDependencyAnalyzer:
    ...
    def _load_spacy_model(self):
        ...
        try:
            ...
            logger.error(f"spaCy model {self.model_name} not found. Please install with: python -m spacy download {self.model_name}")
            self.nlp = None
        except Exception as e:
    ...
    def _find_subject_object(self, verb_token: Token) -> Tuple[Optional[Token], Optional[Token]]:
        """
    ...
    def _calculate_complexity_score(self, doc: Doc) -> float:
        ...
        factors = {
            ...
            "unique_pos_tags": len(set(token.pos_ for token in doc)) / 17.0,  # 17 universal POS tags
            "dependency_depth": self._max_dependency_depth(doc) / 10.0,
            "entity_density": len(doc.ents) / len(doc) if len(doc) > 0 else 0
        }
        
        # Weighted average
        weights = {"sentence_length": 0.3, "unique_pos_tags": 0.3, "dependency_depth": 0.25, "entity_density": 0.15}
    ...
    def _max_dependency_depth(self, doc: Doc) -> int:
        ...
        return max((get_depth(token) for token in doc if token.dep_ == "ROOT"), default=0)
...
class RelationshipExtractorTool:
    ...
    async def extract_comprehensive_relationships(
        ...
        try:
            ...
            entity_relationships = self.gliner_extractor.infer_relationships_from_entities(
                text, entities
            )
            ...
            semantic_relationships = self.spacy_analyzer.extract_semantic_relationships(text)
            ...
            relationship_types = list(set(rel["relation"] for rel in deduplicated_relationships))
            semantic_connections = self._generate_semantic_connections(entities, deduplicated_relationships)
            ...
            return {
                ...
                "confidence": self._calculate_overall_confidence(entities, deduplicated_relationships)
            }
    ...
    def _identify_complexity_indicators(
        ...
        if len(relationships) > 2:
            indicators.append(f"Multiple relationships ({len(relationships)} relations)")
        ...
        if complexity_score > 0.7:
            indicators.append(f"High grammatical complexity (score: {complexity_score:.2f})")
