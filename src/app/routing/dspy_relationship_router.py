...
class DSPyEntityExtractorModule(dspy.Module):
    ...
    def forward(
        ...
        try:
            ...
            key_entities = [e["text"] for e in sorted(entities, key=lambda x: x["confidence"], reverse=True)[:5]]
            
            # Domain classification based on entity types
            domain_classification = self._classify_domain(entity_types, query)
            
            # Calculate entity density
            entity_density = len(entities) / len(query.split()) if query.split() else 0.0
    ...
    def _classify_domain(self, entity_types: List[str], query: str) -> str:
        ...
        if any(t in entity_types for t in ["TECHNOLOGY", "PRODUCT", "TOOL"]):
            if any(word in query_lower for word in ["ai", "machine learning", "algorithm", "robot"]):
                return "artificial_intelligence"
            else:
                return "technology"
        
        # Sports domain
        if any(t in entity_types for t in ["SPORT", "ACTIVITY"]) or \
           any(word in query_lower for word in ["playing", "game", "sport", "competition"]):
            return "sports"
        
        # Entertainment domain
        if any(word in query_lower for word in ["video", "show", "movie", "entertainment"]):
            return "entertainment"
        
        # Education domain
        if any(word in query_lower for word in ["learn", "teach", "education", "tutorial", "how to"]):
            return "education"
...
class DSPyBasicRoutingModule(dspy.Module):
    ...
    def _analyze_query_characteristics(
        ...
        if any(word in query_lower for word in ["compare", "vs", "versus", "difference"]):
            ...
            intent = "analyze"
        elif any(word in query_lower for word in ["report", "details", "comprehensive"]):
            intent = "report"
        ...
        if len(query.split()) > 10:
            complexity = "moderate"
        if len(query.split()) > 20 or any(word in query_lower for word in ["complex", "detailed", "comprehensive"]):
            complexity = "complex"
        
        # Search requirements
        needs_video = any(word in query_lower for word in [
            "video", "show", "demonstration", "visual", "watch", "footage", "clip"
        ])
        
        needs_text = any(word in query_lower for word in [
            "text", "document", "article", "paper", "read", "written"
        ])
        
        needs_multimodal = needs_video and (
            any(word in query_lower for word in ["describe", "explain", "analyze", "caption"])
        )
...
class DSPyAdvancedRoutingModule(dspy.Module):
    ...
    def _create_routing_decision(
        ...
        if basic_analysis.complexity_level == "complex":
            ...
            if generation_type == "detailed_report" and primary_agent != "detailed_report":
    ...
    def _create_agent_workflow(self, routing_decision: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
    ...
    def _generate_optimization_suggestions(
        ...
        if len(relationship_analysis.relationships) > 3:
            suggestions.append("Rich relationship context available for enhanced search")
        elif len(relationship_analysis.relationships) == 0:
            suggestions.append("No relationships detected - consider semantic expansion")
    ...
    def _generate_reasoning_chain(
        ...
        reasoning.append(f"Identified {len(relationship_analysis.relationships)} relationships")
        reasoning.append(f"Domain classification: {entity_analysis.domain_classification}")
        reasoning.append(f"Recommended agent: {basic_analysis.recommended_agent}")
        reasoning.append(f"Overall confidence: {self._calculate_overall_confidence(basic_analysis, entity_analysis, relationship_analysis)}")
