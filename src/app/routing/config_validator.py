    ...
    def validate_ensemble_config(ensemble_config: dict[str, Any] | None) -> None:
        ...
        for strategy in enabled_strategies:
            if not isinstance(strategy, str):
                raise ValueError(
                    f"Strategy name must be a string, got: {type(strategy)}"
                )
        ...
        if voting_method not in valid_voting_methods:
            raise ValueError(
                f"Invalid voting_method '{voting_method}'. Valid methods: {valid_voting_methods}"
            )
        ...
        if "min_agreement" in ensemble_config:
            ...
            if not (0.0 <= min_agreement <= 1.0):
                raise ValueError(
                    "ensemble_config.min_agreement must be between 0.0 and 1.0"
                )
        ...
        if "strategy_weights" in ensemble_config:
            ...
            if not isinstance(strategy_weights, dict):
                raise ValueError(
                    "ensemble_config.strategy_weights must be a dictionary"
                )

            for strategy, weight in strategy_weights.items():
                if not isinstance(strategy, str):
                    raise ValueError(
                        f"Strategy weight key must be a string, got: {type(strategy)}"
                    )
                if strategy not in valid_strategies:
                    raise ValueError(
                        f"Invalid strategy in weights '{strategy}'. Valid strategies: {valid_strategies}"
                    )
                if not isinstance(weight, (int, float)):
                    raise ValueError(
                        f"Strategy weight must be a number, got: {type(weight)}"
                    )
                if weight <= 0:
