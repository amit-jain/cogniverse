    ...
    def _initialize_backends(self):
        ...
        for backend_config in sorted_configs:
            try:
                ...
                logger.info(
                    f"Initialized {backend_config.get('backend_type', 'unknown')} cache backend"
                )
            except Exception as e:
                logger.error(
                    f"Failed to initialize {backend_config.get('backend_type', 'unknown')}: {e}"
                )
    ...
    async def cleanup_expired(self) -> int:
        ...
        for backend in self.backends:
            try:
                ...
                logger.warning(
                    f"Error cleaning expired from {backend.__class__.__name__}: {e}"
                )
