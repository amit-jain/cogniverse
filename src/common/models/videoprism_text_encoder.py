...
class VideoPrismTextEncoder:
    ...
    def _initialize_model(self):
        ...
        with self._cache_lock:
            if self.model_name in self._model_cache:
                logger.info(
                    f"[{self.correlation_id}] Using cached model for {self.model_name}"
                )
                self.tokenizer, self.model, self.state = self._model_cache[
                    self.model_name
                ]
            else:
                logger.info(
                    f"[{self.correlation_id}] Loading new model {self.model_name}"
                )
                self._load_model()
    ...
    def _encode_impl(self, text: str) -> np.ndarray:
        ...
        try:
            ...
            # Tokenize text
            text_ids, text_paddings = self._vp.tokenize_texts(
                self.tokenizer, [formatted_text]
            )
            ...
            logger.info(
                f"[{self.correlation_id}] Raw embeddings shape: {embeddings.shape}, expected dim: {self.embedding_dim}"
            )
            ...
            logger.info(
                f"[{self.correlation_id}] Final embeddings shape: {embeddings.shape}"
            )
...
def create_text_encoder(
    model_name: str, embedding_dim: int, **kwargs
