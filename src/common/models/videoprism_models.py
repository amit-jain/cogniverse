...
class SimpleVideoPrismModel:
    ...
    def load_model(self):
        ...
        try:
            ...
            if not videoprism_path:
                raise ImportError(
                    "VideoPrism repository path not configured. Set videoprism_repo_path in config.json or VIDEOPRISM_REPO_PATH environment variable"
                )
            ...
            raise RuntimeError(
                f"VideoPrism model loading failed - cannot proceed with random embeddings: {e}"
            )
    ...
    def preprocess_video(self, frames: np.ndarray) -> np.ndarray:
        ...
        # Normalize to [0, 1]
        frames = (
            frames.astype(np.float32) / 255.0 if frames.dtype == np.uint8 else frames
        )
