...
from src.common.utils.retry import RetryConfig, retry_with_backoff
...
class VideoPrismLoader:
    ...
    def embeddings_to_vespa_format(
        ...
        binarized_vectors = np.packbits(np.where(vectors > 0, 1, 0), axis=1).astype(
            np.int8
        )
    ...
    def process_entire_video(
        self, video_path: Path, sampling_fps: float = 1.0
    ) -> Dict[str, Any]:
        """
        ...
        if num_frames_to_sample > max_frames:
            logger.warning(
                f"Limiting frames from {num_frames_to_sample} to {max_frames} (max_frames config)"
            )
    ...
    def process_video_segment(
        ...
        if num_frames_to_sample >= total_frames_in_segment:
            ...
            frame_indices = np.linspace(
                start_frame, end_frame - 1, num_frames_to_sample, dtype=int
            ).tolist()
        ...
        logger.info(
            f"Extracted {len(frames)} frames from segment {start_time}-{end_time}s (sampling at {sampling_fps} FPS)"
        )
...
class VideoPrismGlobalLoader(VideoPrismLoader):
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized VideoPrism LVT loader for global embeddings: {actual_model}"
        )
    ...
    def embeddings_to_vespa_format(
        ...
        logger.info(
            f"Converting global embedding with {embedding_dim} dimensions to Vespa format"
        )
    ...
    def extract_embeddings(self, frames: List[np.ndarray]) -> Dict[str, Any]:
        ...
        # Run inference using our model
        logger.info(
            f"Running VideoPrism LVT inference on {len(frames)} frames for global embedding"
        )
        result = self.model.extract_embeddings(video_input)
        embeddings_np = result["embeddings"]
        ...
        if len(embeddings_np.shape) > 1 and embeddings_np.shape[0] > 1:
            logger.info(
                f"Averaging {embeddings_np.shape[0]} tokens to create global embedding"
            )
