        ...
        for i, result in enumerate(results[:3], 1):  # Top 3 results for multimodal eval
            if isinstance(result, dict):
                ...
                if not frame_path and "frame_id" in result:
                    ...
                    frame_path = (
                        f"data/frames/{video_id}/frame_{result.get('frame_id', 0)}.jpg"
                    )
            else:
                video_id = getattr(
                    result, "video_id", getattr(result, "source_id", "unknown")
                )
...
Explanation: Your visual assessment"""
        ...
        response = await self._call_llm(
            prompt, system_prompt, images=frame_paths if frame_paths else None
        )
...
Format: Score: X/10"""
        ...
        if expected_videos and retrieved_ids:
            precision = len(set(retrieved_ids[:5]) & set(expected_videos)) / min(
                5, len(retrieved_ids)
            )
            recall = len(set(retrieved_ids) & set(expected_videos)) / len(
                expected_videos
            )
            f1 = (
...
class SyncLLMHybridEvaluator(Evaluator, LLMJudgeBase):
    ...
    async def _evaluate_async(
        ...
        if expected:
            tasks.append(
                self.reference_based._evaluate_async(input, output, expected, **kwargs)
            )
            use_reference = True
        else:
            use_reference = False
        ...
        if use_reference:
            ...
            if reference_result.metadata:
                metadata.update(
                    {f"ref_{k}": v for k, v in reference_result.metadata.items()}
                )
        else:
