...
class VisualRelevanceEvaluator(Evaluator):
    ...
    def evaluate(self, *, input=None, output=None, **kwargs) -> EvaluationResult:
        ...
        if final_score >= 0.8:
            ...
            explanation = f"Strong visual match (avg: {avg_score:.2f}, max: {max_score:.2f})"
        elif final_score >= 0.6:
            label = "relevant"
            explanation = f"Good visual match (avg: {avg_score:.2f}, max: {max_score:.2f})"
        elif final_score >= 0.4:
            label = "partially_relevant"
            explanation = f"Partial visual match (avg: {avg_score:.2f}, max: {max_score:.2f})"
        else:
            label = "not_relevant"
            explanation = f"Poor visual match (avg: {avg_score:.2f}, max: {max_score:.2f})"
    ...
    def _evaluate_frame(self, result: Dict, query_embedding: np.ndarray) -> Optional[float]:
        """
    ...
    def _compute_max_sim(self, query_emb: np.ndarray, frame_emb: np.ndarray) -> float:
        ...
        frame_norm = frame_emb / (np.linalg.norm(frame_emb, axis=1, keepdims=True) + 1e-8)
...
def create_visual_evaluators(model_name: str = "vidore/colsmol-500m") -> List[Evaluator]:
    """
    ...
    """
    return [
        VisualRelevanceEvaluator(model_name)
    ]