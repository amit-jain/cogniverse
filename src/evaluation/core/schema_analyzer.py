...
class DefaultSchemaAnalyzer(SchemaAnalyzer):
    ...
    def extract_item_id(self, document: Any) -> Optional[str]:
        ...
        if isinstance(document, dict):
            ...
            id_fields = ['id', '_id', 'item_id', 'document_id', 'doc_id', 
                        'video_id', 'image_id', 'audio_id', 'file_id',
                        'source_id', 'uid', 'key', 'product_id', 'sku']
...
class SchemaAnalyzerRegistry:
    ...
    def get_analyzer(self, schema_name: str, schema_fields: Dict[str, Any]) -> SchemaAnalyzer:
        """
        ...
        for analyzer in self._analyzers:
            if analyzer.can_handle(schema_name, schema_fields):
                logger.info(f"Using {analyzer.__class__.__name__} for schema {schema_name}")
                return analyzer
    ...
    def register_plugin(self, plugin_module: str):
        ...
        try:
            ...
            for name in dir(module):
                ...
                if isinstance(obj, type) and issubclass(obj, SchemaAnalyzer) and obj != SchemaAnalyzer:
                    self.register(obj())
...
def get_schema_analyzer(schema_name: str, schema_fields: Dict[str, Any]) -> SchemaAnalyzer:
    """Get the appropriate analyzer for a schema."""
