...
class ExperimentTracker:
    ...
    def _register_evaluator_plugins(self):
        ...
        if self.enable_llm_evaluators:
            ...
            try:
                from src.evaluation.plugins.visual_evaluator import VisualEvaluatorPlugin
                plugin = VisualEvaluatorPlugin()
                register_plugin("visual_evaluator", plugin)
                logger.info(f"Registered visual evaluator plugin with {self.evaluator_name}")
            except Exception as e:
    ...
    def get_experiment_configurations(
        ...
        for profile in profiles_to_test:
            try:
                ...
                # Filter strategies
                if strategies:
                    strategies_to_use = [s for s in available_strategies if s in strategies]
                elif all_strategies:
                    strategies_to_use = available_strategies
                else:
                    strategies_to_use = [s for s in available_strategies if s in common_strategies]
    ...
    def create_or_get_dataset(
        ...
        if csv_path:
            dataset_id = self.dataset_manager.create_from_csv(
                csv_path=csv_path,
                dataset_name=dataset_name or f"experiment_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
    ...
    def run_all_experiments(
        ...
        if not self.configurations:
            raise ValueError("No configurations set. Call get_experiment_configurations first.")
        
        all_experiments = []
        total_experiments = sum(len(config["strategies"]) for config in self.configurations)
        experiment_count = 0
        
        print(f"\n{'='*80}")
        print(f"PHOENIX EXPERIMENTS WITH VISUALIZATION")
        print(f"{'='*80}")
        ...
        if self.enable_quality_evaluators:
            ...
            print(f"Quality Evaluators: ❌ DISABLED")
    ...
    def create_visualization_tables(
        ...
        for profile, stats in profiles.items():
            profile_summary.append({
                ...
                "Failed": stats["failed"],
                "Success Rate": f"{(stats['success']/stats['total']*100):.1f}%" if stats['total'] > 0 else "0%"
            })
    ...
    def print_visualization(
        ...
        if dataset_url:
            print(f"\n🔗 Dataset: {dataset_url}")
        print(f"🔗 Experiments Project: http://localhost:6006/projects/{self.experiment_project_name}")
...
if __name__ == "__main__":
    ...
    import argparse
    parser = argparse.ArgumentParser(description="Run experiments with new evaluation framework")
    ...
    parser.add_argument("--quality-evaluators", action="store_true", help="Enable quality evaluators")
    parser.add_argument("--llm-evaluators", action="store_true", help="Enable LLM evaluators")
