...
class DiversityRerankingStrategy(RerankingStrategy):
    ...
    async def rerank(
        ...
        config = config or {}
        lambda_param = config.get("diversity_lambda", 0.5)  # Balance between relevance and diversity
        ...
        while remaining:
            ...
            if best_idx >= 0:
                ...
                selected['mmr_score'] = best_mmr if reranked else selected.get('score', 0.0)
    ...
    def _extract_content(self, result: Dict) -> str:
        ...
        content_fields = ['content', 'text', 'description', 'summary', 'title', 'caption']
        contents = []
    ...
    def _calculate_idf(self, documents: List[List[str]], vocab: List[str]) -> List[float]:
...
class ContentSimilarityRerankingStrategy(RerankingStrategy):
    ...
    async def rerank(
        ...
        if not searchable_fields:
            ...
            searchable_fields = ["content", "text", "description", "summary", "title", "caption"]
        ...
        reranked = sorted(
            scored_results, 
            key=lambda x: x['content_similarity_score'], 
            reverse=True
        )
    ...
    def _calculate_tfidf_similarity(
        ...
        for term in vocab:
            ...
            # IDF (simplified)
            docs_with_term = (1 if term in query_words else 0) + (1 if term in content_words else 0)
    ...
    def _calculate_enhanced_keyword_similarity(
        self, 
        query: str, 
        content: str
    ) -> float:
...
class TemporalRerankingStrategy(RerankingStrategy):
    ...
    async def rerank(
        ...
        if temporal_intent['type'] in ['latest', 'recent', 'newest']:
            ...
            reranked = sorted(
                scored_results,
                key=lambda x: (x['temporal_score'], -x.get('temporal_value', float('inf')))
            )
        else:
            # Sort by temporal relevance score
            reranked = sorted(
                scored_results,
                key=lambda x: x['temporal_score'],
                reverse=True
            )
    ...
    def _score_temporal_relevance(
        ...
        if intent_type in ['latest', 'recent', 'newest']:
            ...
            import math
            normalized = 1.0 / (1.0 + math.exp(-temporal_value / 1000000))  # Scale by typical timestamp range
            return normalized
            ...
            import math
            normalized = 1.0 / (1.0 + math.exp(temporal_value / 1000000))  # Inverse of recency
            return normalized
            ...
            try:
                ...
                if year_diff == 0:
                    ...
                    return 0.6 - (year_diff - 2) * 0.1  # 0.6 for 2 years, 0.5 for 3, 0.4 for 4, 0.3 for 5
                else:
            ...
            median_timestamp = config.get('median_timestamp', 1700000000)  # Unix timestamp around 2023
    ...
    async def rerank(
        ...
        for strategy in self.strategies:
            try:
                ...
                strategy_name = strategy.__class__.__name__.replace("RerankingStrategy", "").lower()
        ...
        logger.info(f"Hybrid reranked {len(results)} results using {len(strategy_results)} strategies")
        return reranked
