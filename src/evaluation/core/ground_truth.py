...
class SchemaAwareGroundTruthStrategy(GroundTruthStrategy):
    ...
    async def extract_ground_truth(
        ...
        if not backend:
            return {
                ...
                "confidence": 0.0,
                "source": "no_backend",
                "metadata": {"error": "No backend provided for ground truth extraction"}
            }
    ...
    async def _discover_schema_fields(
        ...
        if hasattr(backend, 'get_schema_info'):
            try:
                schema_info = await self._safe_async_call(backend.get_schema_info(schema_name))
                if schema_info:
            ...
            try:
                ...
                if sample_results:
                    ...
                    if any(discovered.values()):
                        logger.info(f"Discovered fields from sample query: {discovered}")
                        return discovered
            except Exception as e:
        ...
        if "fields" in schema_info:
            for field_name, field_info in schema_info["fields"].items():
                ...
                if "id" in field_name.lower() or field_info.get("is_id"):
                    ...
                    if "content" in field_name.lower() or "description" in field_name.lower():
    ...
    def _categorize_fields(self, field_list: List[str]) -> Dict[str, List[str]]:
        ...
        for field in field_list:
            ...
            # Temporal fields
            elif any(time_term in field_lower for time_term in ["time", "date", "timestamp", "created", "updated"]):
                fields["temporal_fields"].append(field)
            # Content fields
            elif any(content_term in field_lower for content_term in ["content", "body", "text", "description"]):
                fields["content_fields"].append(field)
            # Numeric fields
            elif any(num_term in field_lower for num_term in ["score", "rank", "count", "size", "length"]):
                fields["numeric_fields"].append(field)
            # Text fields
            elif any(text_term in field_lower for text_term in ["title", "name", "label", "summary"]):
    ...
    def _infer_fields_from_results(self, result: Any) -> Dict[str, List[str]]:
        ...
        for field_name in all_fields:
            value = getattr(result, field_name, None) if hasattr(result, field_name) else result.get(field_name)
    ...
    async def _search_with_constraints(
        ...
        try:
            ...
            if constraints.get("temporal_constraints"):
                search_params["temporal_constraints"] = constraints["temporal_constraints"]
            
            # Execute search
            results = await self._safe_async_call(
                backend.search(**search_params)
            )
    ...
    def _calculate_confidence(
        ...
        extraction_rate = (total_results - num_errors) / total_results if total_results > 0 else 0.0
        ...
        if constraints.get("field_constraints") and len(constraints["field_constraints"]) > 2:
            ...
            query_specificity = specificity_scores.get(query_type, specificity_scores["generic"])
