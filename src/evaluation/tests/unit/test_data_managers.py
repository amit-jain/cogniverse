...
class TestDatasetManager:
    ...
    def test_update_dataset(self, manager):
        ...
        new_queries = [
            {"query": "new query", "expected_videos": ["v10"]}
        ]
    ...
    def test_export_dataset(self, manager):
        ...
        with patch('builtins.open', mock_open()) as mock_file:
            ...
            handle = mock_file()
            written_content = ''.join(str(call.args[0]) for call in handle.write.call_args_list if call.args)
...
class TestTraceManager:
    ...
    def test_get_recent_traces(self, manager):
        ...
        assert timedelta(hours=1, minutes=30) < time_diff < timedelta(hours=2, minutes=30)
    ...
    def test_mark_trace_evaluated(self, manager):
        ...
        success = manager.mark_trace_evaluated(
            trace_id="trace1",
            evaluation_scores={"mrr": 0.8, "recall": 0.7}
        )
    ...
    def test_get_trace_statistics(self, manager):
        ...
        df = pd.DataFrame([
            {"trace_id": "t1", "attributes.metadata.profile": "p1", "duration_ms": 100},
            {"trace_id": "t2", "attributes.metadata.profile": "p1", "duration_ms": 200},
            {"trace_id": "t3", "attributes.metadata.profile": "p2", "duration_ms": 150}
        ])
        manager.storage.client.get_spans_dataframe.return_value = df
