...
class TestVideoRetrievalScorer:
    ...
    async def test_calculate_mrr(self, scorer):
        ...
        retrieved = [{"video_id": "video1"}, {"video_id": "video2"}, {"video_id": "video3"}]
    ...
    async def test_calculate_map(self, scorer):
        ...
        retrieved = [{"video_id": "video1"}, {"video_id": "video2"}, {"video_id": "video3"}, {"video_id": "video4"}]
        expected = ["video1", "video3"]
    ...
    async def test_phoenix_logging(self, scorer):
        ...
        with patch('src.evaluation.inspect_tasks.scorers.px') as mock_px:
            state = Mock()
            state.metadata = {
...
class TestTemporalAccuracyScorer:
    ...
    async def test_temporal_scorer_calculation(self):
        ...
        assert "overlap" in score.explanation.lower() or "range" in score.explanation.lower()
...
class TestAlignmentScorer:
    ...
    async def test_alignment_scorer_calculation(self):
        ...
        assert "alignment" in score.explanation.lower() or "correct" in score.explanation.lower()
...
class TestFailureAnalysisScorer:
    ...
    async def test_failure_analysis_with_errors(self):
        ...
        assert "error" in score.explanation.lower() or "failure" in score.explanation.lower()
