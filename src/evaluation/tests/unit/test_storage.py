    ...
    def test_log_experiment_results(self, mock_phoenix_client, config):
        ...
        with patch('src.evaluation.data.storage.px.Client', return_value=mock_phoenix_client):
            with patch('src.evaluation.data.storage.trace'):
                ...
                storage.tracer = Mock()
                storage.tracer.start_as_current_span.return_value.__enter__ = Mock(return_value=mock_span)
                storage.tracer.start_as_current_span.return_value.__exit__ = Mock(return_value=None)
    ...
    def test_get_traces_with_trace_ids(self, mock_phoenix_client, config):
        ...
        with patch('src.evaluation.data.storage.px.Client', return_value=mock_phoenix_client):
            with patch('src.evaluation.data.storage.trace'):
                ...
                assert len(df) == 2
                assert mock_phoenix_client.get_spans_dataframe.call_count == 3  # 1 for init + 2 for traces
    ...
    def test_health_check_reconnection_failure(self):
        ...
        with patch('src.evaluation.data.storage.px.Client', return_value=mock_client):
            with patch('src.evaluation.data.storage.trace'):
                ...
                with patch.object(storage, '_initialize_connection', side_effect=Exception("Cannot reconnect")):
                    storage._perform_health_check()
