...
def evaluate_span_quality(
    span_data_path: Optional[str] = None,
    ...
    solvers = [
        RetrievalQualitySolver(
            evaluators={name: create_reference_free_evaluators()[name] 
    ...
    return Task(
        ...
        scorer=scorer,
        metadata={
            "task_type": "span_quality_evaluation",
            "num_spans": len(samples)
        }
    )


@task
def experiment_comparison_task(
    profiles: List[str],
    strategies: List[str]
) -> Task:
    """
...
async def demonstrate_complete_evaluation_flow():
    ...
    experiment_results = await orchestrator.experiment_manager.run_comparison_experiments(
