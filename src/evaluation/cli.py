...
from src.evaluation.data import PhoenixStorage, DatasetManager, TraceManager
...
@cli.command()
@click.option('--mode', type=click.Choice(['experiment', 'batch', 'live']), required=True,
              help='Evaluation mode')
@click.option('--dataset', required=True, help='Phoenix dataset name')
@click.option('--profiles', '-p', multiple=True, help='Video processing profiles (for experiment mode)')
@click.option('--strategies', '-s', multiple=True, help='Ranking strategies (for experiment mode)')
@click.option('--trace-ids', '-t', multiple=True, help='Specific trace IDs (for batch mode)')
@click.option('--config', type=click.Path(exists=True), help='Configuration file path')
@click.option('--output', '-o', type=click.Path(), help='Output file for results')
def evaluate(mode, dataset, profiles, strategies, trace_ids, config, output):
    """
    ...
    if mode == 'experiment':
        if not profiles or not strategies:
            raise click.UsageError("--profiles and --strategies required for experiment mode")
...
@click.option('--queries-json', type=click.Path(exists=True), help='JSON file with queries')
...
def list_traces(hours, limit):
    ...
    try:
        ...
        for trace in traces[:10]:  # Show first 10
            ...
            click.echo(f"  Results: {len(trace.get('results', []))}, Duration: {trace['duration_ms']}ms")
            click.echo("-" * 80)
...
def test():
    ...
    try:
        dataset_id = dataset_manager.create_test_dataset()
