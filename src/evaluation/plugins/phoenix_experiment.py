...
class PhoenixExperimentPlugin:
    ...
    def run_inspect_with_phoenix_tracking(
        ...
        experiment_name = f"inspect_eval_{dataset_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        
        logger.info(f"Running Inspect AI evaluation tracked by Phoenix experiment: {experiment_name}")
