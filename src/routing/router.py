...
class ComprehensiveRouter:
    ...
    def _initialize_strategies(self):
        ...
        if tier_config.get("enable_langextract", True):
            self.strategies[RoutingTier.LANGEXTRACT] = LangExtractRoutingStrategy(langextract_cfg)
    ...
    async def route(self, query: str, context: Optional[Dict[str, Any]] = None) -> RoutingDecision:
        ...
        if isinstance(self.config, dict):
            ...
            fast_threshold = self.config.tier_config.get("fast_path_confidence_threshold", 0.7)
            slow_threshold = self.config.tier_config.get("slow_path_confidence_threshold", 0.6)
            langextract_threshold = self.config.tier_config.get("langextract_confidence_threshold", 0.5)
        else:
            ...
            langextract_threshold = getattr(self.config, 'langextract_confidence_threshold', 0.5)
        ...
        if RoutingTier.FAST_PATH in self.strategies:
            ...
            if fast_decision and fast_decision.confidence_score >= fast_threshold:
                ...
                self._record_routing_metrics(query, fast_decision, time.time() - start_time, RoutingTier.FAST_PATH)
        ...
        if RoutingTier.SLOW_PATH in self.strategies:
            ...
            if slow_decision and slow_decision.confidence_score >= slow_threshold:
                ...
                self._record_routing_metrics(query, slow_decision, time.time() - start_time, RoutingTier.SLOW_PATH)
        ...
        if RoutingTier.LANGEXTRACT in self.strategies:
            ...
            if langextract_decision and langextract_decision.confidence_score >= langextract_threshold:
                self._update_cache(query, langextract_decision)
                self._record_routing_metrics(query, langextract_decision, time.time() - start_time, RoutingTier.LANGEXTRACT)
        ...
        if RoutingTier.FALLBACK in self.strategies:
            ...
            self._record_routing_metrics(query, fallback_decision, time.time() - start_time, RoutingTier.FALLBACK)
        ...
        self._record_routing_metrics(query, default_decision, time.time() - start_time, None)
        return default_decision
    
    async def _try_fast_path(self, query: str, context: Optional[Dict[str, Any]]) -> Optional[RoutingDecision]:
        """
        ...
        try:
            ...
            decision = await self.strategies[RoutingTier.FAST_PATH].route(query, context)
    ...
    async def _try_slow_path(self, query: str, context: Optional[Dict[str, Any]], 
                            fast_decision: Optional[RoutingDecision]) -> Optional[RoutingDecision]:
        """
        ...
        try:
            ...
            decision = await self.strategies[RoutingTier.SLOW_PATH].route(query, enhanced_context)
            ...
            if fast_decision and fast_decision.entities_detected:
                decision.entities_detected = (decision.entities_detected or []) + fast_decision.entities_detected
    ...
    async def _try_langextract(self, query: str, context: Optional[Dict[str, Any]]) -> Optional[RoutingDecision]:
        """
        ...
        try:
            ...
            decision = await self.strategies[RoutingTier.LANGEXTRACT].route(query, context)
    ...
    async def _try_fallback(self, query: str, context: Optional[Dict[str, Any]]) -> RoutingDecision:
        """
    ...
    def _check_cache(self, query: str) -> Optional[RoutingDecision]:
        ...
        if query in self.cache:
            ...
            # Get cache TTL
            if isinstance(self.config, dict):
                cache_ttl = self.config.get("cache_config", {}).get("cache_ttl_seconds", 300)
            else:
    ...
    def _update_cache(self, query: str, decision: RoutingDecision):
        ...
        if isinstance(self.config, dict):
            enable_caching = self.config.get("cache_config", {}).get("enable_caching", True)
        else:
    ...
    def _record_routing_metrics(self, query: str, decision: RoutingDecision, 
                               execution_time: float, tier: Optional[RoutingTier]):
        """
        ...
        if hasattr(self.config, 'monitoring_config'):
            ...
            metrics_batch_size = self.config.monitoring_config.get("metrics_batch_size", 100)
        else:
            enable_metrics = True
    ...
    def _flush_metrics(self):
        ...
        if self.metrics_buffer:
            ...
            avg_time = sum(m.execution_time for m in self.metrics_buffer) / len(self.metrics_buffer)
            ...
            logger.info(f"Metrics Summary - Avg Time: {avg_time:.3f}s, Success Rate: {success_rate:.2%}, "
    ...
    async def optimize_routing(self, training_data: Optional[List[Tuple[str, RoutingDecision]]] = None):
        """
        ...
        if hasattr(self.config, 'optimization_config'):
            enable_auto_opt = self.config.optimization_config.get("enable_auto_optimization", False)
        else:
        ...
        for tier_name, tier_stats in performance_data["tier_performance"].items():
            ...
            if tier_stats.get("success_rate", 1.0) < min_accuracy:
                logger.warning(f"Tier {tier_name} is underperforming with success rate: "
...
class TieredRouter(ComprehensiveRouter):
    ...
    def _check_tier_balance(self):
        ...
        if tier_percentages.get(RoutingTier.SLOW_PATH, 0) > 20:
            logger.warning(f"High Slow Path usage: {tier_percentages[RoutingTier.SLOW_PATH]:.1f}%. "
        ...
        if tier_percentages.get(RoutingTier.FALLBACK, 0) > 10:
            logger.warning(f"High Fallback usage: {tier_percentages[RoutingTier.FALLBACK]:.1f}%. "
    ...
    def get_tier_statistics(self) -> Dict[str, Any]:
        ...
        for tier in RoutingTier:
            ...
            stats[tier.value] = {
                ...
                "usage_percentage": (usage / self.query_count * 100) if self.query_count > 0 else 0
            }
