...
Remaining async/perf (all NEEDS_LIVE_BOUNDARY, more architectural): one-doc-per-segment Vespa feed → batch (embedding_generator/backend); `load_for_request` short-TTL cache; dashboard `evaluation.py` nested aggregate loop + missing timeouts. Then: request-bleed (shared cached agent overlay), VLM-descriptions-dropped, image_search multi-vector format, and the 10 hollow tests. (`schema_registry` regex, `tenant.py` cron name, `agent_dispatcher` signature-variant, `node_id` split) → async-blocking cluster (`document_agent` sync POST, sync DSPy on loop, one-doc-per-segment feed, leaked httpx client, unbounded `search_latencies`) → request-bleed (shared cached agent overlay) → VLM-descriptions-dropped → remaining → 10 hollow tests.
