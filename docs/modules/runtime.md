...
Error mapping: `VespaSearchDegraded` returns 503; `InferenceServiceUnavailableError` returns 503 naming the unconfigured service and the module it needs (an audio query with no `clap_embed` sidecar lands here); `ValueError` returns 404, 501 or 400 by message. Any other failure returns 500 with a JSON `detail` naming the agent, the exception type and the `request_id` — the traceback and the exception text stay in the runtime log, since backend URLs there can carry credentials.
