...
- Returns `GatewayOutput` with `complexity`, `modality`, `generation_type`, `routed_to`, `confidence`, `fast_path_confidence_threshold`, `gliner_threshold`, `entity_extraction_failed` (True on a GLiNER outage)
...
iterables are rejected at the routing boundary.
