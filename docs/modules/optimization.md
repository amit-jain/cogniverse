...
- `ArtifactManager.load_prompts` and `ArtifactManager.load_demonstrations` treat `DatasetNotFoundError` as an absent artifact and let other store `ValueError`s propagate, so a real store failure is not mistaken for "no optimized prompts" or "no demonstrations".
