...
> **Concurrency contract:** `replace_dataset` serializes same-name writers on a shared `DatasetStore` instance. If the delete/create path faults after the old dataset is gone, the previous frame is recreated before the exception propagates. Callers that bypass the shared store object still need their own exclusion.
