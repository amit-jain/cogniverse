...
- **Assertion strength** — run `ASSERTION_GUARD_BASE=<merge-base> uv run pytest tests/common/unit/test_assertion_strength_guard.py`. It fails on a net assertion loss in any `tests/` file and on a newly added skip, xfail, `is not None`, `>= 1`, or `> 0`. A hit is a blocker, not a waiver: either restore the strength or, if the expectation is genuinely stale, replace it with an equally exact pin on the new contract. (Local half of the `Test Integrity` CI job.)
- **Every touched failing test carries a verdict** — for each test the diff changes in response to a failure, the commit message or PR body names product-bug vs stale-test and the file:line that settles it. An edit that makes a test green without that verdict is rejected, even when the assertion count holds.
