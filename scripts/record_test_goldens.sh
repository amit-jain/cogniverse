...
#
# Goldens are regenerated by running pytest with RECORD_GOLDEN=1 against the
# live LM + Vespa + ColBERT + Phoenix stack; this script is the one-command
# wrapper. Most blocks are gitignored; the joint_trace and per_segment blocks
# are committed, so re-recording them is a change to commit.
#
