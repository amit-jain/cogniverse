...
class PhoenixDataManager:
    ...
    def backup(self, name: Optional[str] = None) -> str:
        ...
        logger.info(
            f"Backup created: {backup_file} ({backup_file.stat().st_size / 1024 / 1024:.2f} MB)"
        )
    ...
    def clean(self, older_than_days: int = 30, dry_run: bool = False):
        ...
        logger.info(
            f"Cleaning data older than {older_than_days} days (before {cutoff_date.isoformat()})"
        )
...
def main():
    ...
    # Analyze command
    subparsers.add_parser("analyze", help="Analyze data directory")
        ...
        if args.command == "backup":
            ...
            if backups:
                ...
                for backup in backups:
                    ...
                    print(
                        f"  {status} {backup['name']} - {backup['created_at']} ({size})"
                    )
            else:
            ...
            for category in [
                ...
                print(
                    f"  {category.capitalize():12} {data['count']:6} files, {data['size_mb']:8.2f} MB"
                )
