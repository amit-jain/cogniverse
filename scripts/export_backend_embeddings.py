...
class BackendEmbeddingExporter:
    ...
    def export_embeddings(
        ...
        if reduce_dims and len(embeddings) >= 3:  # Need at least 3 points
            ...
            reduced_embeddings = self._reduce_dimensions(
                embeddings_array, method=dim_reduction_method
            )
            ...
            if field_name in doc:
                ...
                if isinstance(embedding, dict):
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Could not decode binary embedding from {field_name}"
                        )
    ...
    def _reduce_dimensions(
        self, embeddings: np.ndarray, method: str = "umap", n_components: int = 2
    ) -> np.ndarray:
        ...
        logger.info(
            f"Reducing from {embeddings.shape[1]} to {n_components} dimensions..."
        )
...
def main():
    ...
    parser.add_argument(
        "--backend-url", default="http://localhost:8080", help="Backend URL"
    )
    parser.add_argument(
        "--base-schema",
        required=True,
    ...
    parser.add_argument("--filter-key", help="Filter field name (e.g., video_id)")
    parser.add_argument("--filter-value", help="Filter field value")
    parser.add_argument(
        "--profile",
    ...
    parser.add_argument(
        "--no-reduction", action="store_true", help="Skip dimension reduction"
    )
