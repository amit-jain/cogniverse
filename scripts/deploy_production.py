...
def test_api(api_url: str):
    ...
    test_cases = [
        {"query": "Show me how to cook pasta", "expected_modality": "video"},
        {
            "query": "Create a detailed report on climate change",
            "expected_modality": "text",
            "expected_type": "detailed_report",
        },
        {"query": "What's the summary of the research?", "expected_type": "summary"},
    ]
    ...
    for i, test_case in enumerate(test_cases, 1):
        try:
            ...
            response = requests.post(
                api_url,
                json={"user_query": test_case["query"], "include_reasoning": False},
                timeout=30,
            )
            ...
            print(
                f"    ✅ {modality}/{gen_type} (API: {api_latency}ms, E2E: {latency:.1f}ms)"
            )
            ...
            if "expected_modality" in test_case:
                if modality != test_case["expected_modality"]:
                    print(
                        f"    ⚠️ Expected modality: {test_case['expected_modality']}, got: {modality}"
                    )

            if "expected_type" in test_case:
                if gen_type != test_case["expected_type"]:
                    print(
                        f"    ⚠️ Expected type: {test_case['expected_type']}, got: {gen_type}"
                    )
