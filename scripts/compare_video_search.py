...
class VideoSearchComparison:
    ...
    def search_profile(
        self, profile: str, query: str, top_k: int = 10
    ) -> Dict[str, Any]:
    ...
    def search_agent(
        self, agent: Dict[str, Any], query: str, top_k: int = 10
    ) -> Dict[str, Any]:
        ...
        for query in queries:
            ...
            for agent in self.agents:
                ...
                if "error" in result:
                    ...
                    print(
                        f"✅ Success: {result['num_results']} results for profile {result['profile']}"
                    )

                    # Show top 3 results
                    for i, hit in enumerate(result["results"][:3]):
                        source_id = hit.get(
                            "source_id", hit["document_id"].split("_")[0]
                        )
                        if "temporal_info" in hit and hit["temporal_info"]:
                            start_time = hit["temporal_info"]["start_time"]
                            print(
                                f"  {i+1}. {source_id} @ {start_time:.1f}s (score: {hit['score']:.3f})"
                            )
                        else:
    ...
    def _compare_results(self, query: str, results: Dict[str, Any]):
        ...
        for i in range(len(agent_names)):
            for j in range(i + 1, len(agent_names)):
                ...
                videos1 = set(
                    r.get("source_id", r["document_id"].split("_")[0])
                    for r in successful[agent1]["results"][:10]
                )
                videos2 = set(
                    r.get("source_id", r["document_id"].split("_")[0])
                    for r in successful[agent2]["results"][:10]
                )
        ...
        for name, res in successful.items():
            ...
            if scores:
                print(
                    f"  {name}: min={min(scores):.3f}, max={max(scores):.3f}, avg={np.mean(scores):.3f}"
                )
...
def main():
    ...
    parser.add_argument(
        "--agents", nargs="+", help="Agent configurations (name:url:port:profile)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output",
        default="video_search_comparison.json",
