...
class VLMModel:
    ...
    def generate_description(self, request: dict) -> dict:
        ...
        if not frame_path and not remote_frame_path and not frame_base64:
            return {
                "error": "No frame_path, remote_frame_path, or frame_base64 provided in request"
            }

        # Get prompt (detailed for better analysis)
        prompt = request.get(
            "prompt",
            "Provide a detailed description of this video frame, including objects, people, actions, scene setting, and visual details.",
        )
        ...
        try:
            ...
            if remote_frame_path:
                ...
                print(
                    f"📁 Created temp file: {temp_filename} ({len(frame_data)} bytes)"
                )
            ...
            state = image_qa.run(image_path=temp_filename, question=prompt)
            ...
            try:
                ...
                print(
                    ...
                    else f"📝 Generated description: {description}"
                )
                ...
                if not description or description.strip() == "":
                    ...
                    print(
                        f"   State keys: {list(state.keys()) if hasattr(state, 'keys') else 'No keys'}"
                    )

            except KeyError:
                print(
                    f"⚠️ No 'answer' key in state. Available keys: {list(state.keys()) if hasattr(state, 'keys') else 'Not a dict'}"
                )
            ...
            if (
                not frame_path and not remote_frame_path
            ):  # Only cleanup if we created the temp file
                try:
    ...
    def upload_and_process_frames(self, request: dict) -> dict:
        ...
        try:
            ...
            extracted_files = list(Path(extract_dir).glob("*.jpg")) + list(
                Path(extract_dir).glob("*.png")
            )
            ...
            for frame_file in extracted_files:
                ...
                try:
                    ...
                    # Execute the function
                    state = image_qa.run(image_path=str(frame_file), question=prompt)
            ...
            duration = round((time.monotonic_ns() - start) / 1e9, 2)
            print(
                f"✅ Batch processing completed in {duration}s: {len(results)} descriptions"
            )
...
def upload_app():
    ...
    async def upload_frames(request: Request):
        ...
        try:
            ...
            print(
                f"✅ Upload {upload_id} completed in {duration:.2f}s: {len(extracted_files)} frames extracted"
            )
...
def test_vlm(frame_path: Optional[str] = None):
    ...
    payload = json.dumps(
        {"frame_base64": frame_base64, "prompt": "What do you see in this video frame?"}
    )
