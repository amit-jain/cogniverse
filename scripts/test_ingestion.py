...
def main():
    parser = argparse.ArgumentParser(
        description="Run ingestion tests with marker filtering"
    )
    ...
    parser.add_argument(
        "--local-only", action="store_true", help="Run local-only tests"
    )
    ...
    # Heavy model control
    parser.add_argument(
        "--exclude-heavy", action="store_true", help="Exclude heavy model tests"
    )
    parser.add_argument(
        "--include-heavy", action="store_true", help="Include heavy model tests"
    )

    # Coverage options
    parser.add_argument(
        "--coverage-fail-under", type=int, default=80, help="Coverage threshold"
    )
