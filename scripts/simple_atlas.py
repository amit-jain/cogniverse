...
def load_data(path):
    ...
    if "x" not in df.columns or "y" not in df.columns:
        if "embedding" in df.columns and len(df) > 0:
            ...
            st.error(
                "No embedding column found in the data. Cannot create visualization."
            )
            st.stop()
...
with st.sidebar:
    ...
    if categorical_cols:
        color_by = st.selectbox("Color by", options=categorical_cols, index=0)
    else:
        color_by = None
...
with tab1:
    ...
    for i, row in df.iterrows():
        ...
        if "video_title" in df.columns and pd.notna(row.get("video_title")):
            ...
            if "frame_description" in df.columns and pd.notna(
                row.get("frame_description")
            ):
        ...
        if (
            ...
            and pd.notna(row.get("relevance_score"))
    ...
    if has_query:
        ...
        if "z" not in query_df.columns or query_df["z"].isna().any():
            ...
            mean_z = (
                doc_df["z"].mean() if "z" in doc_df.columns and len(doc_df) > 0 else 0
            )
            query_df["z"] = mean_z + 1.0  # Slightly elevated for visibility

        # Get hover texts for documents
        doc_hover_texts = [
            hover_texts[i] for i, is_doc in enumerate(doc_mask) if is_doc
        ]
        doc_color_values = [
            color_values[i] for i, is_doc in enumerate(doc_mask) if is_doc
        ]
                    ...
                    data=[
                        go.Scatter3d(
                            ...
                            marker=dict(
                                ...
                                showscale=True,
                                colorbar=dict(
                                    title=colorbar_title, thickness=15, len=0.7, x=1.02
                                ),
        ...
        if not query_df.empty:
            ...
            for q_idx, (idx, row) in enumerate(query_df.iterrows()):
                ...
                similarity_cols = [
                    col for col in row.index if col.startswith("query_similarity_")
                ]
                if similarity_cols:
                ...
                if similarity_col in doc_df.columns:
                    top_k = min(
                        3, len(doc_df)
                    )  # Show top 3 for each query to avoid clutter
                        ...
                        fig.add_trace(
                            go.Scatter3d(
                                ...
                                line=dict(
                                    color=rgba,
                                    width=1.5
                                    * similarity,  # Thicker lines for higher similarity
                                    dash="dot",
                                ),
                                showlegend=False,
                                hoverinfo="skip",
...
with tab3:
    ...
    if "video_title" in df.columns:  # Video frame schema
        ...
        for col in df.columns:
            if (
                df[col].dtype == "object"
                and col not in priority_cols
                and col not in ["embedding", "x", "y", "z"]
            ):
