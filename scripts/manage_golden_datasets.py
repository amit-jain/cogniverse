...
class GoldenDatasetManager:
    ...
    def list_datasets(self) -> List[Path]:
        ...
        return sorted(
            json_files + csv_files, key=lambda x: x.stat().st_mtime, reverse=True
        )
    ...
    def load_dataset(self, path: Path) -> Dict[str, Dict]:
        ...
        if path.suffix == ".json":
            ...
            for _, row in df.iterrows():
                query = row["query"]
                expected_videos = (
                    row["expected_videos"].split(",") if row["expected_videos"] else []
                )
                dataset[query] = {
    ...
    def merge_datasets(
        self, datasets: List[Dict[str, Dict]], strategy: str = "union"
    ) -> Dict[str, Dict]:
        """
            ...
            for dataset in datasets:
                for query, data in dataset.items():
                    if query not in merged:
                        ...
                        merged[query]["expected_videos"] = list(
                            existing_videos | new_videos
                        )

                        # Merge relevance scores
                        merged[query]["relevance_scores"].update(
    ...
    def get_statistics(self, dataset: Dict[str, Dict]) -> Dict[str, any]:
        ...
        stats = {
            ...
            "total_videos": sum(
                len(d.get("expected_videos", [])) for d in dataset.values()
            ),
            "queries_with_scores": sum(
                1 for d in dataset.values() if d.get("relevance_scores")
            ),
...
def main():
    ...
    export_parser = subparsers.add_parser(
        "export", help="Export dataset as Python code"
    )
