...
class GoldenDatasetGenerator:
    ...
    def __init__(
        ...
        self.tenant_id = require_tenant_id(tenant_id, source="GoldenDatasetFromTraces")
    ...
    def identify_challenging_queries(
    ...
    def create_golden_dataset(
        self, challenging_queries: List[Tuple[str, Dict]]
    ) -> Dict[str, Dict]:
        """
    ...
    def save_golden_dataset(
        self, dataset: Dict[str, Dict], output_path: Optional[Path] = None
    ) -> Path:
        """
    ...
    def generate_report(self, dataset: Dict[str, Dict]) -> str:
        ...
        if dataset:
            ...
            report.append(
                f"  Score Range: {min(avg_scores):.3f} - {max(avg_scores):.3f}"
            )
            ...
            for i, (query, data) in enumerate(sorted_queries[:5], 1):
                report.append(
                    f'  {i}. "{query}" (score: {data["avg_score"]:.3f}, seen: {data["occurrences"]}x)'
                )
...
async def async_main():
    ...
    parser.add_argument(
        "--hours", type=int, default=48, help="Hours back to analyze (default: 48)"
    )
    parser.add_argument(
        "--min-occurrences",
        type=int,
    ...
    parser.add_argument("--output", type=str, help="Output path for golden dataset")
    parser.add_argument("--csv", action="store_true", help="Also save as CSV")
    parser.add_argument(
        "--dry-run", action="store_true", help="Don't save files, just print report"
    )
