    ...
    def _stop_python(self):
        ...
        if self.process:
            ...
            try:
                ...
                # Poll for process termination using utility function for consistent delays
                max_retries = 5
                process_terminated = False

                for attempt in range(max_retries):
                    # Wait with exponential backoff before checking
                    if attempt > 0:  # Skip first wait - check immediately after SIGTERM
                        ...
                        wait_for_retry_backoff(
                            ...
                            base_delay=0.5,
                            max_delay=10.0,
                            description=f"process termination check (PID {pid})",
                        )
                    ...
                    try:
                        ...
                        if attempt < max_retries - 1:
                            logger.debug(
                                f"Process {pid} still running, retry {attempt + 1}/{max_retries}"
                            )
...
def main():
    ...
    # Stop command
    _ = subparsers.add_parser("stop", help="Stop Phoenix server")

    # Restart command
    _ = subparsers.add_parser("restart", help="Restart Phoenix server")

    # Status command
    _ = subparsers.add_parser("status", help="Get Phoenix server status")
