...
from cogniverse_dashboard.evaluation.core.schema_analyzer import SchemaAnalyzer
...
class DocumentSchemaAnalyzer(SchemaAnalyzer):
    ...
    def can_handle(self, schema_name: str, schema_fields: dict[str, Any]) -> bool:
        ...
        # Check schema name
        if any(
            term in schema_name.lower()
            for term in ["document", "text", "article", "page"]
        ):
            return True
    ...
    def analyze_query(
        ...
        for doc_type in doc_types:
            if (
                f"type:{doc_type}" in query_lower
                or f"filetype:{doc_type}" in query_lower
            ):
...
class ImageSchemaAnalyzer(SchemaAnalyzer):
    ...
    def can_handle(self, schema_name: str, schema_fields: dict[str, Any]) -> bool:
        ...
        if any(
            term in schema_name.lower()
            for term in ["image", "photo", "picture", "visual"]
        ):
            return True
