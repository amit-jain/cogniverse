...
class VespaPyClient:
        ...
        while current.parent != current:
            if (current / "configs" / "schemas").exists():
                ...
                schema_path = (
                    ...
                    / "configs"
                    / "schemas"
                    / f"{self.base_schema_name}_schema.json"
                )
                break
            current = current.parent
        else:
            raise RuntimeError(
                "Cannot find project root with configs/schemas directory"
            )
    ...
    def process(self, doc: Document) -> Dict[str, Any]:
        ...
        required_embeddings = self._strategy_processor.get_required_embeddings(
            self.base_schema_name
        )
        field_names = self._strategy_processor.get_embedding_field_names(
            self.base_schema_name
        )
        ...
        # Float embeddings
        if (
            required_embeddings.get("needs_float", False)
            and "embedding" in all_processed_embeddings
        ):
        ...
        # Binary embeddings
        if (
            required_embeddings.get("needs_binary", False)
            and "embedding_binary" in all_processed_embeddings
        ):
            binary_field = field_names.get("binary_field", "embedding_binary")
            processed_embeddings[binary_field] = all_processed_embeddings[
                "embedding_binary"
            ]
        ...
        if (
            "audio_transcript" in doc.metadata
            and "audio_transcript" in self.schema_fields
        ):
        ...
        if doc.id.endswith("_0_0") or doc.id.endswith(
            ...
            if "embedding" in fields:
                if (
                    isinstance(fields["embedding"], dict)
                    and "values" in fields["embedding"]
                ):
        ...
        try:
            ...
            for doc in documents:
                ...
                if self.logger.isEnabledFor(logging.DEBUG):
                    self.logger.debug(
                        f"Document {doc_id} fields: {list(fields.keys())}"
                    )

                feed_data.append({"id": doc_id, "fields": fields})
