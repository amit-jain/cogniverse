...
class VespaVideoSearchClient:
    ...
    def __init__(
        ...
        self.vespa_schema = os.environ.get("VESPA_SCHEMA") or self.config.get(
            "schema_name"
        )
    ...
    def _init_query_encoder(self):
        ...
        try:
            ...
            if profile:
                ...
                self.logger.info(
                    f"Initialized query encoder for {self.vespa_schema} using {profile}"
                )
            else:
                self.logger.warning(
                    f"No query encoder mapping for schema: {self.vespa_schema}"
                )
        ...
        try:
            ...
            if check_schema:
                ...
                self.vespa_app.query(
                    yql=f"select * from {check_schema} where true limit 1", hits=1
                )
            else:
                # Generic health check without schema
                self.vespa_app.query(
                    yql="select * from sources * where true limit 1", hits=1
                )
            return True
        except Exception as e:
    ...
    def search(
        ...
        if not schema:
            raise ValueError(
                "No schema specified. Provide schema in search params or configure schema_name in config."
            )
        ...
        try:
            ...
            raise ValueError(
                f"Invalid ranking strategy '{ranking}'. Available: {available}"
            )
            ...
            if strategy_info["requires_embeddings"] != "No":
                if self.query_encoder and query_text:
                    ...
                    if not query_text:
                        ...
                        raise RuntimeError(
                            f"No query encoder available for schema {self.vespa_schema}"
                        )

        # Validate strategy inputs
        validation_errors = self.validate_strategy_inputs(
            ranking, query_text, embeddings
        )
        if validation_errors:
            raise ValueError(
                f"Strategy validation failed: {'; '.join(validation_errors)}"
            )
    ...
    def _execute_strategy_search(
        ...
        yql = self._build_base_yql(
            strategy, start_date, end_date, query_text, schema=search_schema
        )
        ...
        if strategy in [RankingStrategy.BM25_ONLY, RankingStrategy.BM25_NO_DESCRIPTION]:
            ...
            RankingStrategy.FLOAT_FLOAT,
            RankingStrategy.HYBRID_FLOAT_BM25,
            RankingStrategy.HYBRID_BM25_FLOAT,
            ...
            if is_single_vector_schema and embeddings.ndim == 1:
                ...
                body.update(
                    {"ranking": strategy.value, "input.query(qt)": float_embedding}
                )
            ...
            if strategy in [
                ...
                body["model.defaultIndex"] = (
                    "default"  # Use fieldset for BM25 component
                )

        elif strategy in [
            RankingStrategy.BINARY_BINARY,
            RankingStrategy.HYBRID_BINARY_BM25,
            RankingStrategy.HYBRID_BM25_BINARY,
            ...
            if is_single_vector_schema and embeddings.ndim == 1:
                ...
                body.update({"ranking": strategy.value, "input.query(qtb)": binary_hex})
            else:
                # Multi-token embeddings for patch-based models
                binary_embedding = {
                ...
                body.update(
                    {"ranking": strategy.value, "input.query(qtb)": binary_embedding}
                )
    ...
    def _build_base_yql(
        ...
        if desc_field and strategy not in [
            RankingStrategy.BM25_NO_DESCRIPTION,
            RankingStrategy.HYBRID_FLOAT_BM25_NO_DESC,
            RankingStrategy.HYBRID_BINARY_BM25_NO_DESC,
            RankingStrategy.HYBRID_BM25_BINARY_NO_DESC,
            RankingStrategy.HYBRID_BM25_FLOAT_NO_DESC,
        ...
        if transcript_field and strategy in [
            RankingStrategy.BM25_ONLY,
            RankingStrategy.BM25_NO_DESCRIPTION,
            RankingStrategy.HYBRID_FLOAT_BM25,
            RankingStrategy.HYBRID_BINARY_BM25,
            RankingStrategy.HYBRID_BM25_BINARY,
    ...
    def hybrid_search(
        ...
        body = {
            ...
            "ranking": {
                ...
                "features": {"query(qt)": query_tensor},  # Visual for recall
            },
            "hits": top_k,
        }
    ...
    def _execute_search(
        self, body: Dict[str, Any], search_type: str
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for key in ["input.query(qt)", "input.query(qtb)"]:
                if key in log_body and isinstance(log_body[key], dict):
                    ...
                    sample_embeddings = {}
                    for i, (token_key, embedding) in enumerate(
                        list(log_body[key].items())[:2]
                    ):
                        sample_embeddings[token_key] = (
                            embedding[:3] if isinstance(embedding, list) else embedding
                        )
                    log_body[key] = (
                        f"{{sample: {sample_embeddings}, total_tokens: {len(log_body[key])}}}"
                    )
            ...
            self.logger.info(
                f"{search_type}: Found {len(response.hits)} results in {query_time:.3f}s"
            )
            ...
            for hit in response.hits:
                result = {
                    ...
                    or hit["fields"].get(
                        "segment_id"
                    ),  # Handle both frame_id and segment_id
    ...
    def get_video_summary(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for video_data in video_summaries.values():
            ...
            video_data["avg_relevance"] = (
                video_data["total_relevance"] / video_data["frame_count"]
            )
