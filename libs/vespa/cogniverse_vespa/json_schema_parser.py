...
class JsonSchemaParser:
    ...
    def parse_function(self, func_config: Dict[str, Any]) -> Function:
        ...
        return Function(name=func_config["name"], expression=func_config["expression"])
    ...
    def parse_fieldset(self, fieldset_config: Dict[str, Any]) -> FieldSet:
        ...
        return FieldSet(name=fieldset_config["name"], fields=fieldset_config["fields"])
    ...
    def parse_document(self, doc_config: Dict[str, Any]) -> Document:
        ...
        fields = [
            self.parse_field(field_config) for field_config in doc_config["fields"]
        ]
    ...
    def load_schema_from_json_file(self, json_file_path: str) -> Schema:
        ...
        try:
            ...
            self.logger.info(
                f"Successfully loaded schema '{schema.name}' from {json_file_path}"
            )
            return schema
