...
class StrategyAwareProcessor:
                ...
                for s in schema_strategies.values():
                    for input_name, input_type in s.get("inputs", {}).items():
                        if tensor_name in input_name:
                            tensor_info[tensor_name] = self._parse_tensor_type(
                                input_type
                            )
    ...
    def _parse_tensor_type(self, tensor_type: str) -> Dict[str, Any]:
        ...
        return {"type": tensor_element_type, "dimensions": dims}
    ...
    def format_document(
        ...
        document = {"video_id": video_id, **metadata}  # frame_id, timestamps, etc.
        ...
        if embeddings is not None:
            ...
            if requirements["needs_binary"]:
                ...
                if binary_embeddings.ndim == 2:
                    document["embedding_binary"] = self._format_patch_embeddings(
                        binary_embeddings
                    )
                else:
