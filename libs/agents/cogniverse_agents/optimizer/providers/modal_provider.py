...
class ModalModelProvider(ModelProvider):
    ...
    def call_model(
        ...
        response = requests.post(
            self.deployed_services["inference_endpoint"], json=request_data, timeout=120
        )
    ...
    def deploy_model_service(
        self, model_id: str = None, config: Optional["SystemConfig"] = None, **kwargs
    ) -> Dict[str, str]:
        ...
        try:
            ...
            while i < len(output_lines):
                ...
                if "=>" in line and "Created web function" in line:
                    ...
                    if "https://" in line:
                        ...
                        i + 1 < len(output_lines) and "https://" in output_lines[i + 1]
                    ):
                    ...
                    if url:
                        ...
                        if url.startswith("https://"):
                            ...
                            if (
                                "serve" in line
                                or "--general-inference-service-serve" in url
                            ):
                                service_urls["inference_endpoint"] = (
                                    url  # vLLM serve endpoint
                                )
            ...
            try:
                ...
                config.set(
                    "inference.modal_endpoint", service_urls["inference_endpoint"]
                )
                ...
                print(
                    f"✅ Updated config.json with Modal endpoint: {service_urls['inference_endpoint']}"
                )
            except Exception as e:
            ...
            try:
                health_response = requests.get(
                    service_urls["health_endpoint"], timeout=30
                )
                if health_response.status_code == 200:
                    print("✅ Service health check passed")
                else:
                    print(
                        f"⚠️ Service health check failed: {health_response.status_code}"
                    )
            except Exception as e:
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        try:
            response = requests.get(
                self.deployed_services["health_endpoint"], timeout=10
            )
            if response.status_code == 200:
            ...
            return {"status": "error", "provider": "modal", "error": str(e)}
...
class ModalArtifactProvider(ArtifactProvider):
    ...
    def upload_artifact(self, local_path: str, remote_path: str) -> bool:
        ...
        try:
            print(
                f"📤 Uploading {local_path} to Modal volume {self.volume_name}:{remote_path}"
            )

            cmd = ["modal", "volume", "put", self.volume_name, local_path, remote_path]
    ...
    def download_artifact(self, remote_path: str, local_path: str) -> bool:
        ...
        try:
            ...
            cmd = ["modal", "volume", "get", self.volume_name, remote_path, local_path]
