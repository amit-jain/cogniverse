...
def generate_teacher_examples(
    teacher_lm: dspy.LM, num_examples: int = 50
) -> List[dspy.Example]:
    """
    ...
    with dspy.context(lm=teacher_lm):
        ...
        for i in range(num_examples):
            ...
            if random.random() > 0.7:  # 30% chance of having history
                conversation_history = (
                    f"User previously asked about {random.choice(topics)}"
                )

            try:
                # Get routing decision from teacher
                result = router(
                    conversation_history=conversation_history, user_query=query
                )
...
def evaluate_routing_accuracy(
    ...
    for example in test_set:
        try:
            ...
            # Check both correct
            if (
                prediction.search_modality == example.routing_decision.search_modality
                and prediction.generation_type
                == example.routing_decision.generation_type
            ):
                correct_both += 1
...
if __name__ == "__main__":
    ...
    print(
        f"Optimized Overall Accuracy: {results['metrics']['optimized']['overall_accuracy']:.2%}"
    )
