...
class ModelClient:
    ...
    def _initialize_providers(self):
        ...
        for role in ["teacher", "student"]:
            if role in self.config:
                ...
                if provider_type == "modal":
                    ...
                    self.model_providers[role] = ProviderFactory.create_model_provider(
                        "modal", provider_config
                    )
                    ...
                    raise ValueError(
                        f"Unsupported provider for {role}: {provider_type}"
                    )
        ...
        if artifact_type == "modal":
            ...
            self.artifact_providers["primary"] = (
                ProviderFactory.create_artifact_provider("modal", provider_config)
            )
        elif artifact_type == "local":
            provider_config = artifact_config.get("local", {})
            self.artifact_providers["primary"] = (
                ProviderFactory.create_artifact_provider("local", provider_config)
            )
        else:
    ...
    def deploy_hosted_models(self) -> Dict[str, str]:
        ...
        for role in ["teacher", "student"]:
            if role in self.config and self.config[role]["provider"] in [
                "modal",
                "local",
            ]:
    ...
    def upload_artifacts(
        self,
        local_path: str,
        remote_path: str = "/artifacts/unified_router_prompt_artifact.json",
    ) -> bool:
        """Upload artifacts using the configured provider."""
        if "primary" in self.artifact_providers:
            return self.artifact_providers["primary"].upload_artifact(
...
class ModelClientLM(DSPyLMProvider):
    ...
    def __init__(self, client: ModelClient, model_type: str):
        ...
        if model_type in client.model_providers and client.model_providers[model_type]:
            super().__init__(
                client.model_providers[model_type],
                client.config[model_type]["model"],
                model_type,
            )
        else:
...
class OptimizationOrchestrator:
    ...
    def __init__(self, config_path: str = "config.json"):
        ...
        # create_default_config_manager() which uses COGNIVERSE_CONFIG env var
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        if not opt_config.get("enabled", False):
            raise ValueError(
                "Optimization is disabled in config. Set 'optimization.enabled' to true."
            )

        # Check optimization type
        if opt_config.get("type") != "dspy":
            raise ValueError(
                f"Unsupported optimization type: {opt_config.get('type')}. Only 'dspy' is supported."
            )
            ...
            for i, query_data in enumerate(queries):
                try:
                    if isinstance(query_data, dict):
                        conversation_history = query_data.get(
    ...
    def _generate_diverse_queries(self) -> List:
        ...
        actions = ["cook pasta", "fix a bicycle", "learn guitar", "code in Python"]
    ...
    def _run_provider_optimization(
        ...
        artifacts = {
            ...
            "metrics": {
                ...
                "improvement": {
                    "modality": optimized_metrics["modality_accuracy"]
                    - baseline_metrics["modality_accuracy"],
                    "generation": optimized_metrics["generation_accuracy"]
                    - baseline_metrics["generation_accuracy"],
                    "overall": optimized_metrics["overall_accuracy"]
                ...
                artifacts["demonstrations"].append(
                    {
                        ...
                        "routing_decision": {
                            "search_modality": demo.get("routing_decision", {}).get(
                                "search_modality", ""
                            ),
                            "generation_type": demo.get("routing_decision", {}).get(
                                "generation_type", ""
                            ),
                        },
                    }
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Agentic Router Optimization Orchestrator"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--setup-only",
        action="store_true",
        help="Only setup services, don't run optimization",
    )
    parser.add_argument(
        "--test-models", action="store_true", help="Test model connections"
    )
