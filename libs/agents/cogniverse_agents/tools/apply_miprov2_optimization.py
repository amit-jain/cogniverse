...
def load_miprov2_results(
    results_file: str = "dspy_miprov2_optimization_results.json",
) -> Dict[str, Any]:
...
def get_best_configuration(results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    best_config = {"model": None, "labels": None, "threshold": None, "accuracy": 0.0}
...
def apply_to_query_analyzer():
    ...
    print(
        "📝 To use in QueryAnalyzer, update your config loading to include this override"
    )
...
def demonstrate_usage():
    ...
    examples = [
        ...
        (
            "Search all content from last week",
    ...
    for query, expected in examples:
        print(f"\nQuery: '{query}'")
        print(
            f"Expected: video={expected['video']}, text={expected['text']}, temporal={expected['temporal']}"
        )
