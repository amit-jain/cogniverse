...
class EnhancedTemporalExtractor:
    ...
    def resolve_temporal_pattern(self, pattern: str) -> Dict[str, Any]:
        ...
        def resolve_last_week():
            return {
                "start_date": (today - datetime.timedelta(weeks=1)).strftime(
                    "%Y-%m-%d"
                ),
        ...
        def resolve_two_weeks_ago():
            return {
                "start_date": (today - datetime.timedelta(weeks=2)).strftime(
                    "%Y-%m-%d"
                ),
        ...
        def resolve_last_month():
            return {
                "start_date": (today - datetime.timedelta(days=30)).strftime(
                    "%Y-%m-%d"
                ),
                "end_date": today.strftime("%Y-%m-%d"),
        ...
        def resolve_beginning_of_month():
            return {
                "start_date": today.replace(day=1).strftime("%Y-%m-%d"),
                "end_date": (
                    today.replace(day=1) + datetime.timedelta(days=6)
                ).strftime("%Y-%m-%d"),
