...
class QueryRewriter:
        ...
        try:
            ...
            for strategy_name, strategy_func in self.enhancement_strategies.items():
                try:
                    ...
                    enhancement_results[strategy_name] = {
            ...
            if rel.get("confidence", 0) > 0.6:
                ...
                if relation in ["uses", "using"]:
                    relationship_terms.extend(
                        [f"{subject} with {obj}", f"{obj} in {subject}"]
                    )
                elif relation in ["plays", "playing"]:
                    relationship_terms.extend(
                        [f"{subject} {relation}", f"{relation} {obj}"]
                    )
                elif relation in ["shows", "showing", "demonstrates"]:
                    relationship_terms.extend(
                        [f"{obj} demonstration", f"{subject} example"]
                    )
    ...
    def _add_semantic_context(
        ...
        if any(et in entity_types for et in ["TECHNOLOGY", "PRODUCT"]):
            ...
            if "algorithm" in query_lower:
                tech_terms.extend(
                    ["computational method", "software algorithm", "AI technique"]
                )
        ...
        return {"enhanced_query": enhanced_query, "terms": semantic_terms}
    ...
    def _optimize_boolean_logic(
        ...
        for rel in relationships:
            if rel.get("confidence", 0) > 0.6:
                ...
                relationship_groups[relation_type].extend(
    ...
    def _expand_with_synonyms(
        ...
        return {"enhanced_query": enhanced_query, "terms": synonym_terms}
    ...
    def _generate_enhancement_metadata(
        ...
        quality_factors = {
            ...
            "relationship_richness": len(relationships)
            / 5.0,  # Normalize by expected max
    ...
    def _classify_query_domain(self, query: str, entities: List[Dict[str, Any]]) -> str:
        """
        ...
        # Sports + Technology
        if (
            "SPORT" in entity_types or "sport" in query_lower
        ) and "TECHNOLOGY" in entity_types:
    ...
    def _calculate_complexity_increase(self, original: str, enhanced: str) -> float:
        """
    ...
    async def enhance_query_with_relationships(
        ...
        try:
            ...
            if entities is None or relationships is None:
                relationship_data = (
                    await self.relationship_tool.extract_comprehensive_relationships(
                        query, entity_labels
                    )
                )
                ...
                complexity_indicators = relationship_data.get(
            ...
            complete_result = {
                ...
                "semantic_expansions": enhancement_prediction_data[
                    "semantic_expansions"
                ],
                "relationship_phrases": enhancement_prediction_data[
                    "relationship_phrases"
                ],
                "enhancement_strategy": enhancement_prediction_data[
