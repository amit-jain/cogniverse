...
class ProfilePerformanceOptimizer:
    ...
    TEMPORAL_KEYWORDS = {
        'when', 'before', 'after', 'during', 'while', 'timeline', 'sequence',
        'first', 'last', 'beginning', 'end', 'start', 'finish', 'recent', 'old'
    }
    
    # Spatial keywords for query classification
    SPATIAL_KEYWORDS = {
        'where', 'location', 'place', 'here', 'there', 'near', 'far', 'around',
        'inside', 'outside', 'left', 'right', 'top', 'bottom', 'scene'
    }
    
    # Object keywords for query classification
    OBJECT_KEYWORDS = {
        'object', 'thing', 'item', 'person', 'people', 'face', 'animal', 'car',
        'building', 'furniture', 'device', 'tool', 'what', 'who', 'which'
    }
    ...
    def __init__(self, model_dir: Optional[Path] = None):
        ...
        logger.info(f"Initialized ProfilePerformanceOptimizer (model_dir: {self.model_dir})")
    ...
    def extract_query_features(self, query_text: str) -> QueryFeatures:
        ...
        return QueryFeatures(
            ...
            has_temporal_keywords=any(kw in query_lower for kw in self.TEMPORAL_KEYWORDS),
    ...
    async def extract_training_data_from_phoenix(
        ...
        search_spans = spans_df[
            spans_df['name'].str.contains('search|eval', case=False, na=False)
        ] if 'name' in spans_df.columns else pd.DataFrame()
        ...
        quality_cols = [col for col in search_spans.columns if any(
            metric in col.lower() for metric in ['ndcg', 'score', 'quality']
        )]
        query_cols = [col for col in search_spans.columns if 'query' in col.lower() or 'input' in col.lower()]
    ...
    def train(
        ...
        **xgb_params
        ...
        logger.info(
            f"Training complete - Train: {train_acc:.3f}, Test: {test_acc:.3f}"
        )
