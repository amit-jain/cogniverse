...
@contextmanager
def search_span(tenant_id: str, query: str, top_k: int = 10, ranking_strategy: str = "default", 
                profile: str = "unknown", backend: str = "vespa"):
    ...
    with manager.span("search_service.search", tenant_id=tenant_id, attributes=attributes) as span:
...
@contextmanager  
def encode_span(tenant_id: str, encoder_type: str, query_length: int = 0, query: str = ""):
    ...
    with manager.span(f"encoder.{encoder_type.lower()}.encode", tenant_id=tenant_id, attributes=attributes) as span:
        start_time = time.time()
        try:
            yield span
...
@contextmanager
def backend_search_span(tenant_id: str, backend_type: str = "vespa", schema_name: str = "unknown",
                       ranking_strategy: str = "default", top_k: int = 10, 
                       has_embeddings: bool = False, query_text: str = ""):
    ...
    with manager.span("search.execute", tenant_id=tenant_id, attributes=attributes) as span:
        start_time = time.time()
        try:
            yield span
            span.set_attribute("latency_ms", (time.time() - start_time) * 1000) 
...
def add_search_results_to_span(span, results):
    ...
    if results:
        span.set_attribute("top_score", results[0].score if hasattr(results[0], 'score') else 0)
...
def with_telemetry(span_name: str, 
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if not tenant_id:
                logger.warning(f"No {tenant_id_param} found in {func.__name__}, skipping telemetry")
