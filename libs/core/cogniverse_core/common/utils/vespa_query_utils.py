...
def build_binary_query_params(
    ...
    for index in range(len(binary_query_tensors)):
        ...
        nn_operators.append(
            f"({{targetHits:{target_hits}}}nearestNeighbor(colpali_binary, {nn_argument}))"
        )
...
def build_binary_query_params_ranking(
    binary_query_tensors: Dict[str, str], hits: int = 10
) -> Dict[str, Any]:
    """
...
def build_float_query_params(
    float_query_tensors: Dict[str, List[float]], hits: int = 10
) -> Dict[str, Any]:
    """
...
def build_hybrid_query_params(
    float_query_tensors: Dict[str, List[float]], text_query: str = "", hits: int = 10
) -> Dict[str, Any]:
    """
...
def create_dummy_query_tensors(
    num_tokens: int = 2, embedding_dim: int = 128
) -> torch.Tensor:
    """
