...
class RootCauseAnalyzer:
    ...
    @staticmethod
    def _trace_identity_set(traces: list[Any]) -> set[int]:
        return {id(trace) for trace in traces}
    ...
    def analyze_failures(
        ...
        failed_ids = self._trace_identity_set(failed_traces) if failed_traces else set()
        ...
        if include_performance and successful_traces:
            ...
            performance_degraded = [
                t for t in successful_traces if t.duration_ms > threshold
            ]

            # Log threshold info for debugging
            logger.info("RCA Performance Analysis Debug:")
            logger.info(f"  Total traces analyzed: {len(traces)}")
            logger.info(f"  Successful traces: {len(successful_traces)}")
            logger.info(
                f"  Duration range: {min(durations):.2f}ms - {max(durations):.2f}ms"
            )
            logger.info(
                f"  Duration percentiles: P50={np.percentile(durations, 50):.2f}ms, P95={np.percentile(durations, 95):.2f}ms, P99={np.percentile(durations, 99):.2f}ms"
            )
            logger.info(
                f"  Performance threshold (P{performance_threshold_percentile}): {threshold:.2f}ms"
            )
            ...
            if performance_degraded:
                ...
                for trace in sample_slow:
                    logger.info(
                        f"    - {trace.operation}: {trace.duration_ms:.2f}ms (>{threshold:.2f}ms)"
                    )
    ...
    def _analyze_performance_patterns(
        ...
        slow_ids = self._trace_identity_set(slow_traces)
        normal_durations = []
        for trace in normal_traces:
            if id(trace) not in slow_ids:
        ...
        patterns["latency_distribution"] = {
            ...
            "slowdown_factor": (
                np.mean(slow_durations) / np.mean(normal_durations)
                if normal_durations
                else 0
            ),
        }
    ...
    def _analyze_temporal_patterns(
        ...
        failed_ids = (
            ...
            else self._trace_identity_set(failed_traces)
        )
    ...
    def _detect_failure_bursts(
        ...
        while i < len(sorted_failures):
            ...
            j = i
            while (
                j < len(sorted_failures) and sorted_failures[j].timestamp <= window_end
            ):
                ...
                bursts.append(
                    {
                        ...
                        "duration_minutes": (
                            window_failures[-1].timestamp - window_start
                        ).total_seconds()
                        / 60,
                        "trace_ids": [
                            t.trace_id for t in window_failures[:5]
                        ],  # Sample
                    }
                )
        ...
        for value, total_count in total_values.items():
            ...
            if total_count >= 5:  # Minimum sample size
                ...
                if expected_failures > 0 and expected_successes > 0:
                    chi_square = (
                        (failed_count - expected_failures) ** 2
                    ) / expected_failures
                    ...
                    if (
                        ...
                        and failure_rate > overall_failure_rate * 1.5
                    ):
                        patterns.append(
                            FailurePattern(
                                ...
                                confidence=min(
                                    correlation_strength * (total_count / 10), 1.0
                                ),
            ...
            if slowest_op:
                ...
                hypotheses.append(
                    RootCauseHypothesis(
                        ...
                        affected_traces=[
                            t.trace_id for t in slow_traces if t.operation == op_name
                        ][:5],
        ...
        if performance_analysis["slow_profiles"]:
            ...
            hypotheses.append(
                RootCauseHypothesis(
                    ...
                    affected_traces=[
                        t.trace_id for t in slow_traces if t.profile == profile_name
                    ][:5],
    ...
    def _compare_by_attribute(
        ...
        for value, traces in grouped_all.items():
            ...
            comparison[value] = {
                ...
                "p95_duration": (
                    np.percentile([t.duration_ms for t in traces], 95) if traces else 0
                ),
            }
                ...
                recommendations.append(
                    {
                        ...
                        "details": [f"Check {h.hypothesis}" for h in hypotheses],
    ...
    def _generate_markdown_report(self, analysis: dict[str, Any]) -> str:
        ...
        if analysis["recommendations"]:
            ...
            for rec in analysis["recommendations"]:
                report.append(
                    f"### {rec['recommendation']} ({rec['priority'].upper()} Priority)"
                )
        ...
        if "statistical_analysis" in analysis:
            ...
            if "profile_comparison" in stats:
                ...
                report.append(
                    "| Profile | Total | Failed | Failure Rate | Mean Duration |"
                )
                report.append(
                    "|---------|-------|---------|--------------|---------------|"
                )
