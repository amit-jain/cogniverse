        ...
        with self._lock:
            ...
            for attempt in range(self.config.max_retries):
                try:
                    ...
                    from cogniverse_evaluation.providers import get_evaluation_provider, reset_evaluation_provider
                    import asyncio
                    ...
                    logger.error(f"Failed to connect to telemetry provider (attempt {attempt + 1}): {e}")
                    if attempt < self.config.max_retries - 1:
                        delay = self.config.retry_delay_seconds * (self.config.retry_backoff_factor ** attempt)
                        wait_for_retry_backoff(
                            attempt,
            ...
            raise ConnectionError(f"Failed to connect to telemetry provider after {self.config.max_retries} attempts")
                ...
                if self.provider:
                    ...
                    try:
                        ...
                        with concurrent.futures.ThreadPoolExecutor() as pool:
                            _ = pool.submit(
                                lambda: asyncio.run(self.provider.telemetry.traces.get_spans(
                                    project="cogniverse-default", limit=1
                                ))
                            ).result()
                    except RuntimeError:
                        _ = asyncio.run(self.provider.telemetry.traces.get_spans(
                            project="cogniverse-default", limit=1
                        ))
    ...
    def get_traces_for_evaluation(
        ...
        def _fetch_spans(**kwargs):
            ...
            try:
                ...
                with concurrent.futures.ThreadPoolExecutor() as pool:
                    return pool.submit(
                        lambda: asyncio.run(self.provider.telemetry.traces.get_spans(**kwargs))
                    ).result()
            except RuntimeError:
