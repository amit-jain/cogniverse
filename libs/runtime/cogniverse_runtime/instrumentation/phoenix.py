...
class CogniverseInstrumentor(BaseInstrumentor):
    ...
    def _instrument(self, **kwargs):
        ...
        self._instrument_search_service()  # Backend-agnostic search instrumentation
    ...
    def _instrument_video_pipeline(self):
        ...
        try:
            ...
            self._original_methods[
                "src.processing.unified_video_pipeline.VideoIngestionPipeline.process_video_async"
            ] = original_process
                ...
                with tracer.start_as_current_span(
                    ...
                    try:
                        ...
                        result = await original_process(
                            pipeline_self, video_path, *args, **kwargs
                        )
    ...
    def _instrument_search_service(self):
        ...
        try:
            ...
            self._original_methods["src.app.search.service.SearchService.search"] = (
                original_search
            )
            ...
            def traced_search(service_self, query: str, *args, **kwargs):
                ...
                with tracer.start_as_current_span(
                    ...
                    attributes={
                        ...
                        "backend": getattr(
                            service_self.config, "search_backend", "vespa"
                        ),
                        "query": query_text,
                    ...
                    try:
                        ...
                        if (
                            "query_embeddings" in kwargs
                            and kwargs["query_embeddings"] is not None
                        ):
                            span.set_attribute("has_embeddings", True)
                            span.set_attribute(
                                "embedding_shape", str(kwargs["query_embeddings"].shape)
                            )
                        else:
                        ...
                        if result:
                            span.set_attribute(
                                "top_score",
                                result[0].score if hasattr(result[0], "score") else 0,
                            )
                            ...
                            for i, res in enumerate(result[:3]):
                                result_detail = {
                                    "rank": i + 1,
                                    "document_id": (
                                        res.document.id if res.document else "unknown"
                                    ),
                                    "video_id": (
                                        res.document.metadata.get(
                                            "source_id", "unknown"
                                        )
                                        if res.document
                                        else "unknown"
                            ...
                            span.add_event(
                                "search_results", {"top_3": str(top_3_results)}
                            )
        ...
        try:
                    ...
                    with tracer.start_as_current_span(
                        ...
                        try:
                            ...
                            embeddings = original_encode(
                                encoder_self, query, *args, **kwargs
                            )
                            ...
                            if embeddings is not None:
                                ...
                                if hasattr(embeddings, "shape"):
                                    ...
                                    span.set_attribute(
                                        "embedding_norm_std",
                                        float(
                                            np.std(np.linalg.norm(embeddings, axis=-1))
                                        ),
                                    )

                                    # Set output.value with just shape info
                                    span.set_attribute(
                                        "output.value", str(embeddings.shape)
                                    )
            ...
            for encoder_name, encoder_class in encoder_classes:
                if hasattr(encoder_class, "encode"):
                    ...
                    traced_encode = create_traced_encode(
                        encoder_name, original_encode, self.tracer
                    )
    ...
    def _instrument_routing_agent(self):
        ...
        try:
            ...
            if hasattr(RoutingAgent, "route_query"):
                ...
                self._original_methods[
                    "src.app.agents.routing_agent.RoutingAgent.route_query"
                ] = original_route
                    ...
                    with tracer.start_as_current_span(
                        ...
                        try:
                            ...
                            if isinstance(result, dict):
                                ...
                                span.set_attribute(
                                    "routing.num_workflow_steps", len(workflow_steps)
                                )
                                span.set_attribute(
                                    "routing.reasoning", reasoning[:500]
                            ...
                            span.set_attribute(
                                "routing.latency_ms", (time.time() - start_time) * 1000
                            )
                            span.set_status(Status(StatusCode.OK))
            ...
            if hasattr(RoutingAgent, "_call_agent"):
                ...
                self._original_methods[
                    "src.app.agents.routing_agent.RoutingAgent._call_agent"
                ] = original_call

                @wraps(original_call)
                async def traced_call_agent(
                    agent_self, agent_url: str, payload: dict, *args, **kwargs
                ):
                    with tracer.start_as_current_span(
                        ...
                        attributes={
                            ...
                            "input.value": json.dumps(payload, default=str)[
                                :1000
                        ...
                        try:
                            ...
                            result = await original_call(
                                agent_self, agent_url, payload, *args, **kwargs
                            )

                            # Record agent response details
                            if isinstance(result, dict):
                                span.set_attribute(
                                    "agent.response_size", len(str(result))
                                )
                                if "status" in result:
                                    span.set_attribute("agent.status", result["status"])
                                if "error" in result:
                                    span.set_attribute(
                                        "agent.error", result["error"][:500]
                                    )
